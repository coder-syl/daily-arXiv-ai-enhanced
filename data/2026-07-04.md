...
TL;DR: 本文提出AnchorSplat，一种端到端、无需原始图像的3D原生优化方法，用于提升3D Gaussian Splatting资产质量，显著优于现有方法且速度快达10⁵倍。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS资产常存在细节缺失和纹理噪声等问题，而基于2D图像处理的增强方法会导致多视角不一致且计算开销大；因此需要一种高效、3D原生且无需原始图像的优化方案。

Method: 提出AnchorSplat网络，直接在3D结构上操作，采用Point Anchor机制通过局部偏移约束保证几何一致性，并用单次乘法机制替代迭代稠密化，完全无需原始多视角图像。

Result: 在新构建的大规模基准3DGS-SR上达到SOTA性能，处理速度比传统优化方法快达10⁵倍，并在生成模型输出和真实扫描等不同分布数据上展现出强零样本泛化能力。

Conclusion: AnchorSplat为3DGS资产优化提供了一种高效、鲁棒且无需源图像的新范式，推动了3D内容增强的发展。
...
Abstract (中文翻译): 3D高斯泼溅（3DGS）已成为高质量渲染的一种强大表示方法。然而，现有的资产常常受到诸如细节缺失和纹理噪声等质量问题的限制。以往通过2D图像处理来增强这些资产的方法会引入多视角不一致性并带来高昂的计算成本。本文提出了一种新颖的3D原生优化范式——AnchorSplat。AnchorSplat是一种端到端的深度网络，直接在3D结构上进行操作，避免了传统3D-2D-3D流程中昂贵的优化开销。关键在于，AnchorSplat是一种严格无源（source-free）的解决方案，无需原始多视角图像。该方法的核心是点锚定机制（Point Anchor Mechanism），通过局部偏移约束强制实现几何一致性，从而缓解不适定映射和梯度混淆问题。此外，AnchorSplat以单次乘法机制取代了迭代稠密化过程。为了促进相关研究，我们构建了首个面向该任务的大规模基准数据集3DGS-SR。实验表明，AnchorSplat在3DGS-SR数据集上取得了最先进的结果，处理吞吐量比优化方法快高达10⁵倍。值得注意的是，AnchorSplat在包括生成模型输出和真实世界扫描在内的多种数据分布上均展现出强大的零样本泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有PAD模型在跨域场景下泛化能力不足，主要因为传统基于类别标签优化的提示学习无法对齐细粒度攻击相关语义，导致模型过拟合域特异性伪影而非可迁移攻击线索。

Method: 提出CPG-PAD框架，包含视觉概念驱动增强（VCE）模块和基于提示的概念注入（PCI）机制：VCE利用可解释AI技术自动发现与PAD相关的视觉概念并生成热力图；PCI通过视觉-提示解码器（VPD）和概念映射损失将这些概念融入提示空间，使提示与模型内部概念对齐。

Result: 在9个基准数据集上进行的大量实验表明，CPG-PAD在多源、少源和单源设置下均取得当前最优的跨域性能。

Conclusion: 通过将模型级概念引导引入提示学习过程，CPG-PAD能有效捕捉通用且域不变的攻击线索，并抑制数据集特定偏差，显著提升跨域泛化能力。
...
Abstract (中文翻译): 呈现攻击检测（PAD）是保护人脸识别系统免受打印照片、重放视频和3D面具等呈现攻击的关键防线。尽管已有显著进展，现有PAD模型仍因传感器、光照和攻击材料的变化而在未见域上泛化困难。近期的视觉-语言模型（VLMs）展现出强大的泛化能力，但其在PAD中的应用仍受限，因为通常在类别标签监督下优化的学习提示未能显式对齐细粒度的攻击相关视觉语义，导致所学表征往往过拟合域特定伪影，而非捕获可迁移的攻击线索。为解决此问题，我们提出了概念引导提示的呈现攻击检测方法（CPG-PAD），该框架将模型级概念引导引入提示学习过程。具体而言，我们设计了一个视觉概念驱动增强（VCE）模块，利用可解释人工智能（XAI）技术自动发现与PAD相关的视觉概念，并生成关联概念的热力图以提供局部细粒度引导。在这些热力图的指导下，基于提示的概念注入（PCI）机制通过视觉-提示解码器（VPD）和概念映射损失将这些概念整合进提示空间，使提示与模型内部概念空间对齐。该设计使CPG-PAD能够捕获可泛化且域不变的攻击线索，同时有效抑制数据集特定偏差。在九个基准数据集上的大量实验表明，CPG-PAD在多源、少源和单源设置下均持续达到当前最优的跨域性能。
...
Method: 提出KathaTrace协议，通过三种证据条件（仅文本、仅图像、图文结合）评估场景过渡的语义可恢复性，并构建包含25K样本的KathaBench数据集；定义语义轨迹差距（STG）指标衡量可视化过程中的语义损失；引入Semantic Compass探针用于生成后修复。

Result: 人类验证一致性高（Fleiss' kappa = 0.845）；在SOTA生成模型上测得平均STG为23.5 ± 1.3；Semantic Compass能有效提升故事板选择质量。

Conclusion: 视觉连贯性不等于语义连贯性，需专门评估语义轨迹保持能力；KathaTrace与STG为视觉叙事生成提供了新的诊断与改进路径。
...
Abstract (中文翻译): 视觉叙事是故事板、漫画、儿童媒体和电影预可视化的核心，观众仅凭图像即可理解故事。近期的生成模型（如StoryDiffusion）虽能生成视觉连贯的序列，但视觉连贯性并不能保证源故事中场景过渡的语义仍可被还原。现有基准主要评估视觉质量、内容忠实度和场景连贯性，却忽略了一种关键失效模式：即场景看似视觉连贯，但场景间的语义关联却已消失。我们提出了KathaTrace——一种与生成器无关的协议，用于诊断“语义轨迹崩溃”（即理解一个场景如何接续另一个场景所需的过渡语义丢失）。KathaTrace在三种证据条件下评估过渡语义：仅文本、仅图像、以及图文结合，并过滤掉模糊项。我们贡献了KathaBench-25K数据集，包含来自《伊索寓言》《五卷书》和《故事海》等经典文集的5,000个叙事、20,000个过渡及28,712个可恢复性问题。我们定义“语义轨迹差距”（Semantic Trajectory Gap, STG）为仅文本与仅图像条件下的可恢复性之差，用以衡量可视化过程中丢失的过渡语义。人类验证获得Fleiss' kappa = 0.845。在多个前沿生成器上的实验显示，其STG高达23.5 ± 1.3。此外，我们提出的“语义罗盘”（Semantic Compass）作为一种可操作性探针，利用KathaTrace信号进行生成后修复，有效提升了故事板的选择质量。
...
TL;DR: 本文提出一种输入感知的可扩展专家模块，通过在相似样本子集上训练专家，并结合输入感知专家选择机制与时空选择机制，提升视频行人重识别中细粒度特征的提取能力，在两个大规模数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理外观高度相似的行人身份时难以有效区分，因此需要更强的细粒度特征提取能力。传统模型对所有样本统一更新参数，无法专注于相似样本间的细微差异。

Method: 提出一种新颖的输入感知可扩展专家模块，包含两个核心机制：1）输入感知专家选择机制，在相似样本子集上动态激活特定专家；2）时空选择机制，增强专家对空间和时间维度上细粒度差异的敏感性。同时设计可扩展方案，支持按需添加新专家。

Result: 所提方法在两个大规模视频行人重识别数据集上取得了领先的性能表现。

Conclusion: 通过在相似样本子集上训练专用专家并引入动态选择机制，能显著提升模型对细粒度差异的建模能力，从而有效解决外观相似行人的重识别难题。
...
Abstract (中文翻译): 基于视频的行人重识别（Re-ID）旨在从图库视频片段中检索出与查询视频片段中相同身份的目标。为解决该问题，挖掘细粒度特征至关重要，尤其是在区分外观相似的身份时。本文提出一种新颖的输入感知可扩展专家模块，以增强模型探索细粒度信息的能力。不同于对数据集中每个样本统一更新网络参数的做法，我们旨在仅包含相似样本的特定子集上训练专家，从而提升其在这些相似样本中挖掘细粒度信息的能力。为实现这一目标，该模块融合了两种机制：输入感知专家选择机制和时空选择机制。前者针对相似样本子集动态激活一组专家，促使专家挖掘这些相似样本之间的细微差异；后者进一步增强专家对空间和时间维度上细粒度差异的敏感性，并允许专家根据不同的输入样本动态利用这些差异。此外，为支持该专家模块，我们设计了一种可扩展方案，使其能够在必要时灵活地添加新专家。实验结果表明，所提方法在两个大规模数据集上均取得了卓越的性能。
...
TL;DR: MapDreamer 是一种基于扩散模型的方法，能从单张航拍图像中自动生成具有显式拓扑结构的车道级矢量地图，显著提升地图生成的自动化水平和精度。


<details>
  <summary>Details</summary>
Motivation: 高精地图对自动驾驶至关重要，但传统生成方法依赖大量人工标注，难以规模化。因此需要一种能够自动、高效且准确地从航拍图像生成车道级矢量地图的方法。

Method: MapDreamer 结合变分自编码器（VAE）学习车道中心线及其拓扑关系的紧凑潜在表示，并使用基于 Transformer 的潜在扩散模型预测矢量图；通过交叉注意力机制在每一步去噪过程中注入密集航拍特征以对齐场景；引入车道数量模块与背景“幽灵车道”潜在变量防止槽位坍塌；采用滑动窗口全局图聚合策略拼接局部瓦片以构建城市尺度地图并保持连通性。

Result: 在 Argoverse 2 衍生的 UrbanLaneGraph 数据集上的实验表明，MapDreamer 在几何和拓扑保真度方面优于非生成式基线方法。

Conclusion: MapDreamer 提供了一种端到端、可扩展的高精地图自动生成方案，在自动驾驶地图构建中具有重要应用潜力。
...
Abstract (中文翻译): 高精地图生成对自动驾驶至关重要，但其大规模构建仍是一个劳动密集型过程。我们提出了 MapDreamer，一种生成式扩散模型，能够直接从单张航拍图像合成具有显式拓扑结构的车道级矢量地图。MapDreamer 利用变分自编码器学习车道中心线及其拓扑关系的紧凑潜在表示，并通过基于 Transformer 的潜在扩散模型预测图结构。为了使生成的地图与观测场景对齐，我们在每个去噪步骤中通过交叉注意力机制注入密集的航拍特征。为应对不同场景中车道数量的变化，我们提出了一种车道基数模块，并配合背景“幽灵车道”潜在变量——一种可学习的缓冲机制，以防止扩散过程中出现槽位坍塌。此外，我们引入了一种滑动窗口的全局图聚合策略，通过编码的车道边界将局部瓦片拼接成城市尺度的地图，同时保持连通性。在源自 Argoverse 2 的 UrbanLaneGraph 数据集上的实验表明，MapDreamer 在几何和拓扑保真度方面优于非生成式基线方法。
...
TL;DR: 本文提出MIBE框架，包含多主体交互基准（MIB）和评估器（MIE），用于解决多主体个性化图像生成中的身份遗漏、外观失真和交互错配问题，并提供与人类偏好高度一致的可靠评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的多主体个性化图像生成模型在准确渲染多个参考身份及其指定交互方面仍存在困难，且现有评估指标主要针对单主体场景，在多主体情况下无法有效反映生成质量，与人类偏好不一致。

Method: 作者构建了MIB基准，包含6万对VLM标注的Silver Set用于训练评估器，以及4千对双盲人工评估的Gold Set用于测试；并在此基础上训练轻量级评估器MIE，采用双头结构进行排序与诊断。

Result: MIE在Gold Set上展现出强大的泛化能力，整体与人类偏好的成对准确率达0.922（已见生成器0.982，未见生成器0.884），显著优于CLIP、DINO等基线指标。

Conclusion: 通过引入诊断性监督，MIE有效解决了传统评估指标在多主体场景下排名区分度下降和与人类偏好脱节的问题，为多主体图像生成提供了可靠的评估工具。
...
Abstract (中文翻译): 多主体个性化图像生成要求根据引导提示精确渲染所有请求的参考身份及其指定的交互关系。然而，当前最先进的模型在这一过程中仍面临挑战，经常遗漏主体、无法保留参考外观，或错误分配交互关系。此外，现有的评估指标主要针对单主体保真度设计，难以可靠地捕捉这些错误，在主体数量增加时排名区分度严重下降，且无法与人类偏好保持一致。为弥补这一空白，我们提出了多主体交互基准与评估器（MIBE），这是一个统一框架，包含多主体交互基准（MIB）和多主体交互评估器（MIE）。MIB通过解耦的数据机制系统覆盖多种关系类型和场景复杂度，包括一个用于可扩展指标训练的6万对VLM标注的Silver Set，以及一个涵盖多种先进生成器的4千对双盲人工评估Gold Set，其中Silver Set在不同VLM间的偏好一致性达到95.1%。为展示该基准的实用性，我们提出了MIE——一个轻量级、基于参考条件的评估器，仅使用Silver Set进行训练，并采用双头结构实现排序与诊断目标。MIE在Gold Set上展现出强大的跨生成器泛化能力，与人类偏好的整体成对准确率达到0.922，其中在已见生成器上为0.982，在未见生成器上为0.884。MIE显著优于包括CLIP和DINO变体在内的多种基线指标，证明了诊断性监督能够在传统评估器失效的情况下，有效保持排名区分度并与人类偏好对齐。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪模型在面对目标严重形变、复杂干扰、环境剧变或训练中未见类别时，泛化与在线适应能力不足，导致跟踪可靠性下降；而人类视觉系统能融合先验知识、空间几何与语义上下文实现稳健连续的感知，因此需向人类视觉机制靠拢。

Method: 提出一系列系统性方法，增强跟踪模型的目标判别能力、鲁棒自适应能力以及几何推理能力。

Result: 所提方法有效提升了通用目标跟踪（GOT）任务中模型在动态、不可预测场景下的鲁棒性和准确性。

Conclusion: 通过模拟人类视觉感知机制，可显著提升机器视觉跟踪系统的性能，缩小与人类视觉能力的差距。
...
Abstract (中文翻译): 人类视觉感知的核心在于能够对外部世界保持连续且一致的理解。通过将观察结果与积累的经验相结合，人类视觉系统能够持续适应目标及其周围环境的变化，同时在场景动态演变过程中保持稳健的视觉连续性。因此，人类视觉可以整合先验知识、空间几何结构和语义上下文，以理解复杂场景及其变化。作为计算机视觉中的一个核心问题，视觉目标跟踪旨在使机器感知更接近人类视觉感知，这一能力正是通用目标跟踪（Generic Object Tracking, GOT）任务的核心。在该任务中，视觉跟踪器仅在第一帧中通过一个任意指定目标的边界框进行初始化，并需在后续动态视觉流中持续定位该目标。然而，未来事件、观测结果和现实世界的变化本质上是不可预测的，因此模型的泛化能力和在线适应能力仍是瓶颈。当目标发生严重形变、受到复杂干扰物影响、遭遇显著环境变化，或属于训练阶段未见过的类别时，跟踪的可靠性会显著下降。本论文旨在通过提出一系列方法，系统性地增强跟踪模型的目标判别能力、鲁棒自适应能力以及几何推理能力，从而缩小机器视觉跟踪系统与人类视觉感知之间的差距。
...
TL;DR: 本文提出利用改进的YOLOv10模型结合辅助数据，自动检测树冠廊道摄像机拍摄视频中的褐吼猴，以提升城市扩张背景下对树栖物种保护措施的监测效率。


<details>
  <summary>Details</summary>
Motivation: 城市扩张导致森林生境破碎化，威胁树栖物种生存；树冠廊道虽可缓解该问题，但其有效性依赖于对动物使用的持续监测。传统相机陷阱方法产生大量误报图像，耗费人力，因此亟需高效的自动检测技术。

Method: 研究采用YOLOv10目标检测框架，通过引入不同比例的辅助数据进行微调，以自动识别相机陷阱视频中的褐吼猴（Alouatta guariba）个体。

Result: 结合辅助数据微调YOLOv10模型有效提升了对褐吼猴的自动检测性能，减少了人工审核负担。

Conclusion: 该自动检测方法为监测树冠廊道使用情况提供了高效工具，有助于减轻人类活动对野生动物栖息地的干扰，支持生物多样性保护。
...
Abstract (中文翻译): 城市扩张威胁全球生物多样性，尤其因森林生境破碎化而严重影响树栖物种。树栖动物在破碎化林斑之间的移动会增加死亡风险，从而危及其保护成效。在此背景下，架设树冠廊道可能是一种可行策略；然而，为确保其有效性，必须对树栖物种的使用情况进行持续监测，通常借助红外相机完成。但该方法常产生大量假阳性图像，耗费保护工作者大量时间进行人工审核。为此，计算机视觉算法可用于优化树冠廊道中目标物种的检测任务。本研究探索了利用相机陷阱所获视频自动检测褐吼猴（Alouatta guariba）的方法。鉴于训练此类算法需要大量标注图像，我们测试了引入辅助数据以改进检测模型的效果，并使用不同比例的辅助数据对YOLOv10框架进行微调。这些自动检测技术的改进有助于保护工作，为监测减轻人类干扰对动物栖息地影响的措施提供自动化工具。
...
TL;DR: 提出了一种基于图的Concept Bottleneck Model（G-CBM），通过非负矩阵分解进行无监督概念发现，并以图像级概念图表示这些概念，利用图注意力网络建模概念间依赖关系，在多个医学和通用图像数据集上取得优于基线模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有Concept Bottleneck Models（CBMs）通常依赖预定义概念词汇或监督标注，缺乏显式概念定位，并仅用单个图像级分数概括每个概念，忽略了空间重复性和概念间依赖关系。

Method: G-CBM采用非负矩阵分解（NMF）进行无监督概念发现，将发现的概念作为节点构建每张图像的概念图；通过区域特征与概念节点匹配实现概念定位并捕捉空间重复性；引入可调概念过滤阈值τ抑制弱区域特征；使用图注意力网络（GAT）对概念节点间的非线性依赖进行推理。

Result: 在ImageNet、HAM10000、PH2和Derm7pt数据集上，G-CBM相比ResNet-50基线平均相对AUC提升3.7%；概念过滤不仅提升预测性能，还能实现选择性使用概念（如PH2上仅用2/10概念达AUC 0.96，HAM10000上用3.8/9概念达AUC 0.92）；在皮肤镜基准上媲美需外部标注的监督方法；删除/插入分析表明所学概念排序能忠实反映模型预测。

Conclusion: G-CBM是一种无需监督标注、具备内在可解释性的框架，通过图结构有效建模概念及其空间与语义关系，在多个任务上兼具高性能与可解释性。
...
Abstract (中文翻译): 概念瓶颈模型（CBM）提供了一种内生可解释的替代方案，避免了事后解释的局限。然而，现有的CBM通常依赖预定义的概念词汇表或监督标注，缺乏显式的概念定位，并且仅用单一的图像级分数来概括每个概念，忽略了概念在空间上的重复出现以及概念之间的相互依赖关系。我们提出了一种基于图的概念瓶颈模型（G-CBM），该模型是一种内生可解释的框架，通过非负矩阵分解（NMF）进行无监督概念发现，并将所发现的概念表示为每张图像对应的概念图中的节点。G-CBM将区域级特征与这些概念节点进行匹配，从而实现概念定位并捕捉概念在整个图像中的重复出现情况，同时引入一个可调节的概念过滤阈值τ以抑制较弱的区域级特征。随后，图注意力网络（GAT）通过建模节点间的非线性依赖关系来进行概念层面的推理。在ImageNet、HAM10000、PH2和Derm7pt等多个数据集上，G-CBM相比ResNet-50基线模型平均实现了3.7%的相对AUC提升。概念过滤机制在提升预测性能的同时，还能促使模型选择性地使用部分概念：例如在PH2数据集上仅使用10个概念中的2个即达到0.96的AUC，在HAM10000上使用9个概念中的3.8个即达到0.92的AUC。在皮肤镜图像基准测试中，G-CBM的性能可与需要外部标注的监督方法相媲美。通过带有随机消融控制的删除/插入分析表明，所学习到的概念排序能够忠实地反映模型的预测结果。
...
TL;DR: 提出一种基于空中签名的3D行为认证方法“Sign in the Air to Unlock”，利用点-体素交叉注意力网络（PV-Net）从3D轨迹中建模局部运动与全局空间结构，在两个数据集上验证了其有效性，展示了在沉浸式环境中实现自然、安全身份认证的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有身份认证方式（如密码、PIN码、设备登录）破坏沉浸感且依赖外部硬件；而现有的3D行为认证方法（如手势、眼动、脑电）常需专用传感器或限制自然动作，难以在动态沉浸环境中实用。因此需要一种既安全又符合自然交互的新型认证接口。

Method: 设计名为Sign in the Air to Unlock的空中签名认证接口，用户可在3D空间中自然书写签名；开发点-体素交叉注意力网络（PV-Net），联合建模3D轨迹中的局部运动动态与全局空间结构以识别用户。

Result: 在公开数据集DeepAirSig（40名用户，1800个样本）上达到2.5%的等错误率（EER）；在新收集的VR数据集ImmAirSig（22名用户，880个样本，使用Meta Quest 2采集）上达到76%的分类准确率。

Conclusion: 该研究证明了基于3D行为的空中签名认证在沉浸式环境中兼具安全性与自然交互性，为未来用户中心的身份认证提供了可行路径。
...
Abstract (中文翻译): 沉浸式技术（如虚拟现实和增强现实，VR/AR）的显著进步及其在现代生活中的广泛应用，亟需一种安全、直观且与具身交互兼容的身份认证界面。传统的密码、PIN码和基于设备的登录方式会破坏沉浸感，并依赖外部硬件。近期针对3D环境的行为认证方法，如手势识别、眼动追踪和脑电图（EEG）技术，虽提供了有前景的替代方案，但往往需要专用传感器或限制用户的自然动作，从而在动态环境中可用性受限。本文提出“空中签名解锁”（Sign in the Air to Unlock）——一种空中签名界面，允许用户在3D空间中自然地进行签名，这是一种熟悉、个性化且可重复的手势。为实现该界面，我们设计了一种点-体素交叉注意力网络（PV-Net），能够从3D轨迹中联合建模局部运动动态与全局空间结构。该模型在两个数据集上进行了评估：公开的DeepAirSig数据集（40名用户，共1800个签名）以及新采集的ImmAirSig数据集（使用Meta Quest 2在沉浸式VR中收集，22名用户，共880个样本）。PV-Net在DeepAirSig上取得了2.5%的等错误率（EER），在ImmAirSig上达到了76%的分类准确率。这些结果凸显了3D行为认证界面在沉浸式环境中实现无缝、以用户为中心的身份认证的潜力，将安全性与自然交互融为一体。
