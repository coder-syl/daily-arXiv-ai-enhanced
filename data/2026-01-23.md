...
Method: 1. 评估包括传统数据增强和动态标签注入在内的预处理策略；  
2. 提出新架构FORTRESS，融合深度可分离卷积、自适应Kolmogorov-Arnold网络（KAN）和多尺度注意力机制；  
3. 探索少样本语义分割，采用带注意力机制的双向原型网络进行缺陷检测。

Result: 1. 预处理策略显著提升IoU和F1分数；  
2. FORTRESS在涵洞/污水管缺陷数据集上达到SOTA性能，同时大幅减少参数量和计算开销；  
3. 少样本方法在有限标注数据下仍取得令人满意的评估指标表现。

Conclusion: 在标注数据稀缺的实际场景中，通过改进训练数据或调整模型架构，均可有效提升涵洞与污水管道缺陷的自动分割性能。所提三种方法为基础设施智能检测提供了实用且高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型在视觉-语言任务中的成功，研究者开始关注其在生物识别（特别是异质人脸识别）中的应用潜力。然而，尚缺乏对其在该任务中性能的系统性评估。

Method: 作者在多种跨模态场景（如VIS-NIR、VIS-SWIR、VIS-THERMAL）下，对多个开源MLLM进行了基准测试，并采用生物识别标准协议和指标（如Acquire Rate、EER、TAR）进行评估。

Result: 实验结果表明，尽管MLLM近期取得了显著进展，但在具有挑战性的跨光谱条件下，其性能与传统人脸识别系统相比仍存在巨大差距。

Conclusion: 当前的MLLM在异质人脸识别任务中仍存在明显局限性，在将其部署到实际人脸识别系统前，必须进行严格的生物识别评估。
...
TL;DR: CURE is an error-aware curriculum learning framework that enhances medical vision-language models by improving visual grounding and reducing hallucinations without extra data.


<details>
  <summary>Details</summary>
Motivation: Medical vision-language models often produce radiology reports with poor visual grounding and factual inconsistencies, undermining their reliability in clinical settings.

Method: CURE fine-tunes a multimodal instructional model using public datasets through three tasks: phrase grounding, grounded report generation, and anatomy-grounded report generation. It employs dynamic curriculum learning that prioritizes harder samples based on model performance to enhance alignment.

Result: CURE improves grounding accuracy by +0.37 IoU, increases report quality by +0.188 CXRFEScore, and reduces hallucinations by 18.6%.

Conclusion: CURE is a data-efficient framework that significantly boosts both grounding accuracy and factual reliability of generated radiology reports.
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视角锥束CT重建因采样不足难以恢复高频解剖细节，而传统CNN方法偏向学习低频信息，无法有效重建精细结构。
...
Conclusion: DuFal通过联合建模空间与频域信息，有效解决了稀疏视角CT重建中高频细节丢失的问题，为医学图像重建提供了新思路。
...
TL;DR: 本文提出一种偏差引导的提示学习框架，结合视觉-语言模型与统计偏差评分机制，在仅使用少量正常样本的情况下提升异常区域检测性能。


<details>
  <summary>Details</summary>
Motivation: 少样本正常样本下的异常检测（FNSAD）因监督信息有限和缺陷类型多样而极具挑战；现有基于CLIP等视觉-语言模型的方法在正常与异常提示之间区分度不足，且缺乏有效的像素级异常评分机制。

Method: 提出偏差引导的提示学习框架：使用可学习的上下文向量替代固定提示前缀，共享于正常与异常提示；引入异常特定后缀以实现类别感知对齐；结合Top-K多示例学习（MIL）的偏差损失，将图像块特征建模为对正常分布的高斯偏差，从而赋予显著偏离的图像块更高异常分数。

Result: 在MVTecAD和VISA基准上，该方法在像素级检测性能上优于PromptAD及其他基线方法；消融实验验证了可学习提示、偏差评分和Top-K MIL策略的有效性。

Conclusion: 所提方法有效融合了视觉-语言模型的语义能力与统计偏差的可靠性，显著提升了少样本异常检测的定位精度与可解释性。
...
TL;DR: 该论文提出了一种基于传感器物理原理的统一NeRF框架，利用单次曝光的模糊低动态范围（LDR）图像及其对应事件数据，实现高质量的高动态范围（HDR）清晰新视角合成。
...
Method: 作者提出一个统一的、基于传感器物理的NeRF框架：1）用NeRF直接建模HDR域中的真实3D场景辐射，并模拟物理世界中到达传感器像素的原始HDR光线；2）引入逐像素RGB映射场，将渲染值与输入LDR图像的传感器记录值对齐；3）设计新的事件映射场，连接物理场景动态与实际事件传感器输出；4）联合优化两个映射场与NeRF网络，利用事件中的时空动态信息提升HDR清晰3D表示学习。

Result: 在自建和公开数据集上的实验表明，该方法在仅使用单次曝光模糊LDR图像和对应事件数据的情况下，实现了当前最优的去模糊HDR新视角合成效果。

Conclusion: 通过将传感器物理机制显式建模进NeRF框架，并结合事件数据的时空信息，该方法有效解决了模糊LDR图像在极端光照下难以恢复HDR和清晰3D结构的问题，显著提升了新视角合成的质量。
...
TL;DR: 用Vision Transformer替代U-Net作为属性中和框架的编码器，可在保留诊断准确率的同时更有效地减少胸部X光图像中的性别与年龄属性泄露。


<details>
  <summary>Details</summary>
Motivation: 胸部X光分类器常因依赖性别和年龄等捷径特征而对少数群体造成系统性误诊；现有基于卷积编码器的像素级属性中和方法无法在临床可用的编辑强度下完全消除属性泄露。

Method: 在属性中和框架中，将U-Net卷积编码器替换为数据高效的DeiT-S Vision Transformer，在ChestX-ray14数据集上训练，并在11个编辑强度下生成中和图像；使用独立AI判别器评估属性泄露程度，用ConvNet评估疾病预测性能。

Result: 在中等编辑强度（alpha=0.5）下，ViT中和器将性别识别AUC降至约0.80，比原U-Net方法低约10个百分点（且训练轮次减半）；15种疾病预测的宏观ROC AUC与原始图像相差不超过5个百分点，最差子群体AUC仍接近0.70。

Conclusion: 基于全局自注意力机制的视觉模型能更有效抑制属性泄露，同时保持临床实用性，为构建更公平的胸部X光AI提供了可行路径。
...
TL;DR: LASAGNA 是一个统一框架，能同时生成具有逼真背景和高质量透明前景（含阴影、反射等效果）的图像及其分层表示，支持多种条件输入，实现高可控性和一致性编辑。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型在编辑特定图像元素时难以保持可控性和一致性；分层表示虽具灵活性，但现有方法常无法生成具有合理合成关系的图层，且前景缺乏真实视觉效果（如阴影、反射）。

Method: 提出 LASAGNA 框架，从文本提示、前景、背景及位置掩码等多种条件输入中学习正确的图像合成方式，同时生成逼真背景与带真实视觉效果的透明前景。构建新数据集 LASAGNA-48K（含干净背景和带物理真实效果的 RGBA 前景）及首个分层编辑基准 LASAGNABENCH。

Result: LASAGNA 能在多个图像层上同时生成高度一致且连贯的结果，支持多样化的后期编辑应用，并准确保留对象身份与视觉效果。

Conclusion: LASAGNA 有效解决了图像分层生成中的可控性、一致性和真实感问题，所提出的 LASAGNA-48K 数据集和 LASAGNABENCH 基准将公开发布以促进社区研究。
...
TL;DR: 提出一种仅利用手背图像的双流增量编码器方法，通过对比动态手与放松姿态的特征，在手指严重遮挡情况下显著提升手部姿态估计精度，并支持新型交互方式。


<details>
  <summary>Details</summary>
Motivation: 第一人称视角下的手部姿态估计常因手指频繁遮挡而受限，现有方法依赖完整手部几何信息和大型模型，难以在遮挡场景下保持鲁棒性。

Method: 引入双流增量编码器，利用密集视觉特征提取器挖掘手背皮肤形变信息，通过对比动态手与基线放松姿态的特征来学习姿态。

Result: 在手指遮挡≥50%的场景下，仅使用裁剪的手背图像，该方法相比当前最优技术将平均关节角度误差（MPJAE）降低了18%。

Conclusion: 该方法不仅提升了遮挡场景下下游任务（如食指捏合与点击估计）的可靠性，还支持无需可见运动的等距力检测等新交互范式，同时减小了模型规模。
...
<details>
  <summary>Details</summary>
Motivation: 将多模态大语言模型（MLLMs）泛化到新的视频领域对现实应用至关重要，但受限于标注数据稀缺；现有上下文学习方法依赖大量标注样本，在工业或手术等专业场景中难以实施。

Method: 提出VIOLA框架：1）采用密度-不确定性加权采样策略，在严格标注预算下选择兼具多样性、代表性和信息量的样本；2）构建混合池并引入置信度感知检索与提示机制，有效利用无标签数据，避免噪声传播。

Result: 在9个多样化基准和4种MLLM上的实验表明，该方法在低资源设置下显著优于多种基线，以极低标注成本实现稳健适应。

Conclusion: VIOLA通过最小标注与无标签数据的协同利用，为MLLM在新视频领域的高效、低成本部署提供了有效解决方案。
