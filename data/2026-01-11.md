...
TL;DR: 本文提出了一种两阶段对齐框架，通过构建分层细粒度的评估标准，并引入复杂偏好优化（CPO）方法，显著提升了扩散模型在绘画生成任务中与人类专家知识的对齐程度和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的后训练对齐方法依赖于简化的信号（如标量奖励或二元偏好），难以捕捉复杂、分层且细粒度的人类专业知识。

Method: 首先与领域专家共同构建一个树状结构的分层细粒度图像质量评估标准；然后提出两阶段对齐框架：第一阶段通过监督微调（SFT）将领域知识注入辅助扩散模型，第二阶段提出复杂偏好优化（CPO）方法，扩展DPO以同时最大化正向属性概率并最小化负向属性概率，从而将目标扩散模型与非二元、分层的评估标准对齐。

Result: 在绘画生成领域的实验表明，所提出的CPO方法显著提升了生成质量和与专家知识的对齐效果。

Conclusion: 该研究为扩散模型与细粒度、分层人类专业知识的对齐提供了新途径，展示了CPO在提升生成质量方面的有效性。
...
Abstract (中文翻译): 扩散模型的后训练对齐依赖于简化的信号，例如标量奖励或二元偏好，这限制了其与复杂、分层且细粒度的人类专业知识的对齐能力。为解决此问题，我们首先与领域专家共同构建了一套分层、细粒度的评估标准，将图像质量分解为多个组织成树状结构的正向与负向属性。在此基础上，我们提出了一种两阶段对齐框架：第一阶段通过监督微调（Supervised Fine-Tuning）将领域知识注入一个辅助扩散模型；第二阶段引入复杂偏好优化（Complex Preference Optimization, CPO），该方法扩展了DPO，使目标扩散模型能够对齐我们的非二元、分层标准。具体而言，我们将对齐问题重新表述为利用辅助扩散模型同时最大化正向属性概率并最小化负向属性概率。我们在绘画生成领域实例化了该方法，并基于所构建的标准，使用带有细粒度属性标注的绘画数据集进行了CPO训练。大量实验表明，CPO显著提升了生成质量和与专家知识的对齐程度，为细粒度标准对齐开辟了新途径。
...
TL;DR: 本文提出一种基于RGB空间中五元像素强度组合的新文本嵌入方法，可在单个像素内完整编码一个字符，避免了传统LSB/MSB、变换域或深度学习方法中的多像素操作或高计算开销，并在图像质量指标上表现出无显著失真。


<details>
  <summary>Details</summary>
Motivation: 现有文本图像隐写方法（如LSB、MSB、PVD、变换域、量化、边缘区域方法及深度学习等）普遍存在依赖多像素强度翻转、引入噪声、编码解码确定性强或计算复杂度高等问题，亟需一种高效且低失真的新方法。

Method: 该方法利用RGB三通道中各自五个可控的像素强度变化，构成最多125种五元像素强度组合，并将这些组合映射到字母、数字、空格及常用特殊字符；每个RGB像素可完整嵌入一个文本符号。

Result: 通过MSE、MAE、SNR、PSNR、SSIM、直方图比较和热力图分析等多种指标评估，嵌入后图像无明显失真；相比传统方法，该方法在单像素内完成字符嵌入，提升了嵌入效率并降低了计算开销。

Conclusion: 所提五元像素强度组合方法在保持图像视觉质量的同时，实现了高效、低复杂度的文本信息嵌入，优于现有多种隐写技术。
...
Abstract (中文翻译): 本文提出了一种利用RGB空间中五元像素强度组合将文本数据嵌入图像的新技术。现有方法主要依赖于最低有效位（LSB）与最高有效位（MSB）操作、像素值差异（PVD）、RGB通道的空间扰动、变换域方法、量化方法、基于边缘与区域的方法，以及近期兴起的深度学习和生成式AI技术，在图像空间域中隐藏文本信息。其中大多数方法依赖于多个像素的像素强度翻转（如LSB及其组合方法）或变换系数，常导致噪声形式的失真。此外，多数现有方法的编解码过程是确定性的，而深度学习和生成式AI等高级模型则计算开销较大。所提出的方法基于RGB空间中的五元像素强度组合，即在R、G、B每个通道中使用五种受控的不同像素强度变化，从而形成最多125种不同的像素强度组合。这些组合被映射到文本符号，能够表示大小写字母、数字、空格及常用特殊字符。通过MSE、MAE、SNR、PSNR、SSIM、直方图比较和热力图分析等多种指标对原始图像与嵌入图像进行评估，结果表明图像未出现显著失真。此外，该方法通过在单个RGB像素内编码完整文本符号，相较于通常需要多个像素或多步处理的LSB/MSB方法，以及计算开销更高的变换域和学习型方法，实现了更高的嵌入效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一多模态生成架构通常依赖显式的模态切换，在生成推理文本后再手动切换至图像生成，这种分离式、顺序式的推理过程限制了跨模态耦合，无法实现自动化的多模态生成。

Method: 采用离线、奖励加权的后训练方法，利用完全自生成的合成数据，探索联合文本-图像生成对T2I性能的影响、各模态在后训练中的重要性，以及不同后训练数据策略的效果。

Result: 在四个多样化的T2I基准上实现了多模态图像生成性能的提升；使用针对特定局限性的定向数据集优于通用图像-标题语料库或与基准对齐的数据；同时对两个模态进行奖励加权和精心设计后训练数据是有效的。

Conclusion: 通过奖励加权的后训练和策略性设计的合成数据，可以有效实现文本与图像的统一生成，增强模型在多模态图像生成任务中的表现。
...
Abstract (中文翻译): 最近，能够联合生成文本和图像的统一多模态生成架构已成为文本到图像（T2I）合成的一个有前景的方向。然而，许多现有系统依赖显式的模态切换，在生成推理文本之后再手动切换到图像生成。这种分离的、顺序式的推理过程限制了跨模态耦合，并阻碍了自动多模态生成。本研究探索通过后训练实现完全统一的文本-图像生成，使模型能够在单次推理过程中自主地从文本推理过渡到视觉合成。我们考察了联合文本-图像生成对T2I性能的影响，以及在后训练过程中各模态的相对重要性。此外，我们还探索了不同的后训练数据策略，结果表明，针对特定局限性设计的定向数据集相比广泛的图像-标题语料库或与基准对齐的数据能取得更优的结果。通过使用完全自生成的合成数据进行离线、奖励加权的后训练，我们的方法在四个多样化的T2I基准上均提升了多模态图像生成性能，证明了对两种模态进行奖励加权以及策略性设计后训练数据的有效性。
...
TL;DR: ReHyAt 是一种混合注意力机制，结合了 softmax 注意力的高质量与线性注意力的高效性，实现了常量内存使用和线性复杂度，显著降低训练成本并保持生成质量，适用于长视频和端侧部署。


<details>
  <summary>Details</summary>
Motivation: 现有基于 Transformer 的视频扩散模型依赖二次复杂度的注意力机制，难以扩展到长序列；亟需一种兼顾生成质量和计算效率的注意力机制。

Method: 提出 ReHyAt（Recurrent Hybrid Attention），融合 softmax 注意力与线性注意力，支持分块递归重构和常量内存使用，并通过轻量蒸馏从现有 softmax 模型高效迁移知识。

Result: 在 VBench 和 VBench-2.0 以及人类偏好研究中，ReHyAt 达到 SOTA 视频质量，注意力复杂度从二次降至线性，训练成本降至约 160 GPU 小时。

Conclusion: ReHyAt 在保持高质量的同时显著提升效率，为未来双向 softmax 模型提供可复用的高效训练范式，推动长视频和设备端生成的实用化。
...
Abstract (中文翻译): 近期视频扩散模型的研究趋势转向基于 Transformer 的架构，在视频生成方面取得了最先进的成果，但其注意力机制具有二次复杂度，严重限制了对更长序列的可扩展性。我们提出了 ReHyAt（Recurrent Hybrid Attention，递归混合注意力）机制，将 softmax 注意力的高保真度与线性注意力的高效性相结合，实现分块递归重构和恒定内存占用。与同期仅使用线性注意力的 SANA Video 不同，ReHyAt 的混合设计能够高效地从现有的基于 softmax 的模型中进行知识蒸馏，将训练成本降低两个数量级至约 160 GPU 小时，同时在生成质量上具有竞争力。我们提出的轻量级蒸馏与微调流程，可作为未来先进双向 softmax 模型的通用方案。在 VBench 和 VBench-2.0 基准以及人类偏好研究中的实验表明，ReHyAt 在将注意力复杂度从二次降低至线性的同时，实现了最先进的视频生成质量，为长时长和设备端视频生成提供了实用的可扩展性。项目页面见 https://qualcomm-ai-research.github.io/rehyat。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅模型在中大型场景中存储需求高，限制了其在云和流媒体服务中的部署；当前基于标量量化的渐进压缩方法未能充分利用高维特征向量间的相关性，影响率失真性能。

Method: 采用残差向量量化（RVQ）替代传统标量量化方法，并设计了一个由多分辨率哈希网格引导的自回归熵模型，用于精确预测每个传输索引的条件概率，从而高效压缩粗略层与细化层。

Result: 所提方法能更有效地捕捉高维特征相关性，在保持高质量视图合成的同时显著降低比特率。

Conclusion: 通过引入残差向量量化和自回归熵建模，该工作在3D高斯泼溅的渐进压缩中实现了更高的率失真效率，为实际部署提供了可行方案。
...
Abstract (中文翻译): 近期在3D高斯泼溅方面的进展已实现高质量的实时新视角合成。然而，这些模型在中大型场景中具有较高的存储需求，阻碍了其在云端和流媒体服务中的部署。一些最新的渐进压缩技术依赖于渐进掩码和标量量化方法，利用空间上下文模型来降低高斯属性的比特率。尽管有效，但标量量化可能无法最优地捕捉高维特征向量之间的相关性，从而可能限制率失真性能。  
在本研究中，我们提出了一种新颖的3D高斯泼溅渐进式编解码器，用更强大的残差向量量化方法替代传统方法以压缩图元特征。我们的核心贡献是一种由多分辨率哈希网格引导的自回归熵模型，能够准确预测每个连续传输索引的条件概率，从而高效压缩粗略层和细化层。
...
TL;DR: 在五个孟加拉国图像数据集上的实验表明，使用微调的迁移学习（尤其是ResNet-18）显著优于从头训练的自定义CNN和特征提取方法，在准确率上提升3%至76%，其中ResNet-18在Road Damage BD数据集上达到100%准确率；尽管自定义CNN参数更少、训练更快，但迁移学习在复杂或小样本任务中表现更优。


<details>
  <summary>Details</summary>
Motivation: 为实际应用中如何根据数据集特性、计算资源和性能需求选择合适的深度学习方法提供依据，特别是在资源有限或数据量较小的场景下比较自定义CNN与预训练模型的优劣。

Method: 对自定义CNN与两种主流预训练模型（ResNet-18和VGG-16）进行对比，分别采用特征提取和迁移学习（微调）两种策略，在五个来自孟加拉国的不同图像分类数据集上进行评估。

Result: 迁移学习结合微调在所有数据集上均优于自定义CNN和特征提取方法，准确率提升3%–76%；ResNet-18微调在Road Damage BD数据集上达到100%准确率；自定义CNN参数量更小（3.4M vs. 11–134M），在简单任务上训练效率更高。

Conclusion: 对于复杂或训练数据有限的图像分类任务，采用预训练模型进行迁移学习（特别是微调）是更优选择；而当计算资源受限或任务较简单时，自定义轻量CNN可能更具实用性。
...
Abstract (中文翻译): 本研究对自定义卷积神经网络（CNN）与流行的预训练架构（ResNet-18 和 VGG-16）进行了全面的对比分析，分别采用特征提取和迁移学习两种方法。我们在五个来自孟加拉国的多样化图像分类数据集上评估了这些模型，包括Footpath Vision、Auto Rickshaw Detection、Mango Image Classification、Paddy Variety Recognition和Road Damage Detection。实验结果表明，在不同数据集上，结合微调的迁移学习方法始终优于从头构建的自定义CNN和特征提取方法，准确率提升幅度在3%至76%之间。值得注意的是，经过微调的ResNet-18在Road Damage BD数据集上达到了完美的100%准确率。尽管自定义CNN在模型规模（340万参数，而预训练模型为1100万至1.34亿参数）和简单任务的训练效率方面具有优势，但在复杂分类任务或训练数据有限的情况下，采用迁移学习的预训练模型表现更为出色。本研究为从业者根据数据集特性、计算资源和性能需求选择合适的深度学习方法提供了实用指导。
...
TL;DR: 本文提出PackCache，一种无需训练的KV缓存管理方法，通过条件锚定、跨帧衰减建模和空间保留位置编码，在统一自回归视频生成中显著提升长序列生成效率。


<details>
  <summary>Details</summary>
Motivation: 统一自回归模型在多模态任务（如文本、图像、视频）中依赖KV缓存机制以降低注意力计算复杂度，但KV缓存随生成长度线性增长，成为推理效率和生成长度的主要瓶颈，尤其在视频生成中更为严重。

Method: 作者分析发现KV缓存中的token具有明显的时空特性：文本和条件图像token作为持久语义锚点持续获得高注意力，而对先前帧的注意力随时间距离自然衰减。基于此，提出PackCache方法，包含三个协同机制：条件锚定保留语义参考、跨帧衰减建模按时间距离分配缓存预算、空间保留位置嵌入在缓存移除时维持3D结构一致性。

Result: PackCache在48帧长序列视频生成中实现1.7–2.2倍的端到端加速；在最昂贵的最后四帧生成中，在A40和H200上分别达到2.6倍和3.7倍加速。

Conclusion: PackCache有效缓解了统一自回归视频生成中KV缓存膨胀问题，显著提升长序列生成效率，展现出推动更长视频生成的潜力。
...
Abstract (中文翻译): 统一自回归模型是一种基于Transformer的框架，将多样化的多模态任务（例如文本、图像、视频）统一为共享词元空间下的单一序列建模问题。这类模型依赖KV缓存机制将注意力计算复杂度从O(T²)降至O(T)；然而，KV缓存大小随生成词元数量线性增长，迅速成为限制推理效率和生成长度的主要瓶颈。统一自回归视频生成也继承了这一限制。我们的分析揭示，KV缓存中的词元表现出明显的时空特性：(i) 文本和条件图像词元作为持久的语义锚点，始终获得高注意力；(ii) 对先前帧的注意力随时间距离自然衰减。基于这些观察，我们提出了PackCache——一种无需训练的KV缓存管理方法，通过三种协同机制动态压缩KV缓存：保留语义参考的条件锚定、根据时间距离分配缓存预算的跨帧衰减建模，以及在缓存移除时维持连贯3D结构的空间保留位置嵌入。在效率方面，PackCache在48帧长序列上实现了1.7–2.2倍的端到端生成加速，展现出其在支持更长序列视频生成方面的强大潜力。值得注意的是，在受KV缓存持续膨胀影响最大、因而计算成本最高的视频最后四帧生成中，PackCache在A40和H200上分别实现了2.6倍和3.7倍的加速。
...
TL;DR: 本文利用EMOCA提取的3D面部表情与姿态系数，在分心驾驶场景下分析压力反应，发现其中41个系数在基线与压力阶段表现出与生理指标相当的一致性变化；进一步提出基于Transformer的时序建模框架，并通过跨模态注意力融合EMOCA与生理信号，在压力识别任务中取得最优性能（AUROC 92%，准确率86.7%）。


<details>
  <summary>Details</summary>
Motivation: 由于压力具有主观性且人们可自主控制面部表情，从面部视频中可靠地识别压力极具挑战；现有方法多依赖面部动作单元（Facial Action Units），而解耦的3D面部几何信息的作用尚未充分探索。

Method: 使用EMOCA模型提取3D面部表情与姿态系数，通过配对假设检验分析其在基线与压力阶段的变化；构建基于Transformer的时序建模框架，评估单模态、早期融合及跨模态注意力等融合策略。

Result: 56个EMOCA系数中有41个在压力阶段表现出一致且显著的变化，效果可比生理指标；跨模态注意力融合EMOCA与生理信号取得最佳性能（AUROC 92%，准确率86.7%），EMOCA与注视信息融合也表现优异（AUROC 91.8%）。

Conclusion: 3D面部几何特征在压力识别中具有重要价值，结合时序建模与跨模态注意力机制能有效提升识别性能。
...
Abstract (中文翻译): 从面部视频中可靠地识别压力具有挑战性，这源于压力的主观特性以及人们对面部表情的自主控制能力。尽管大多数方法依赖于面部动作单元（Facial Action Units），但解耦的3D面部几何信息在压力识别中的作用尚未得到充分探索。本文通过使用EMOCA模型提取的3D表情与姿态系数，在分心驾驶情境下分析压力反应。基线阶段与压力刺激阶段之间的配对假设检验表明，56个系数中有41个呈现出与生理指标相当的、具有阶段特异性的稳定压力响应。在此基础上，我们提出了一种基于Transformer的时序建模框架，并评估了单模态、早期融合以及跨模态注意力等多种融合策略。实验结果显示，EMOCA特征与生理信号通过跨模态注意力融合取得了最佳性能（AUROC达92%，准确率为86.7%），而EMOCA与注视信息的融合也表现优异（AUROC为91.8%）。该研究突显了时序建模与跨模态注意力机制在压力识别中的有效性。
...
TL;DR: 本文研究了如何通过少量配对样本，将主要在RGB图像上预训练的流匹配基础模型（FLUX.1 Kontext）利用低秩适配（LoRA）技术，高效地转化为跨光谱翻译器（如RGB到红外或合成孔径雷达图像），并验证了生成的合成数据能有效提升下游目标检测任务的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉领域的基础模型主要在RGB数据上训练，但许多安全关键应用（如自动驾驶、遥感）依赖红外（IR）和合成孔径雷达（SAR）等非可见光模态。如何利用现有的强大RGB基础模型来支持这些稀缺数据的非可见光模态，是一个重要挑战。

Method: 作者从FLUX.1 Kontext模型出发，插入低秩适配（LoRA）模块，并仅使用每个域100对配对图像（KAIST数据集用于RGB-to-IR，M4-SAR数据集用于RGB-to-SAR）进行微调。该方法能将RGB图像翻译成像素对齐的IR/SAR图像，从而复用现有的RGB标注框来训练纯目标模态下的检测器。

Result: 实验发现，在50个保留样本上计算的LPIPS指标是下游检测性能（mAP）的强代理指标。使用基于LPIPS选出的最佳LoRA适配器，由外部RGB数据集（LLVIP, FLIR ADAS）生成的合成IR图像提升了KAIST数据集上的行人检测性能；合成SAR图像与少量真实SAR数据结合，显著提升了M4-SAR数据集上的基础设施检测性能。

Conclusion: 对流匹配基础模型进行少量样本的LoRA适配，是为非可见光模态提供基础模型级别支持的一条有效且有前景的路径。
...
Abstract (中文翻译): 视觉领域的基础模型主要在RGB数据上进行训练，而许多安全关键型应用则依赖于红外（IR）和合成孔径雷达（SAR）等非可见光模态。我们研究了是否可以将一个主要在RGB图像上预训练的单一流匹配基础模型，仅通过少量共测样本重新用于跨光谱翻译，并探究由此生成的合成数据能否增强下游检测任务。我们从FLUX.1 Kontext模型出发，插入低秩适配（LoRA）模块，并在两个场景下仅使用每域100对配对图像进行微调：在KAIST数据集上实现RGB到IR的转换，以及在M4-SAR数据集上实现RGB到SAR的转换。经过适配的模型能够将RGB图像翻译成像素对齐的IR/SAR图像，使我们能够复用现有的边界框标注，并纯粹在目标模态下训练目标检测模型。在一系列LoRA超参数网格搜索中，我们发现仅基于50个保留样本对计算出的LPIPS指标是下游性能的强有力代理指标：更低的LPIPS值始终能预测出YOLOv11n在IR和SAR上以及DETR在KAIST IR测试数据上更高的mAP。使用基于最佳LPIPS选出的LoRA适配器，由外部RGB数据集（LLVIP, FLIR ADAS）生成的合成IR图像改善了KAIST IR行人检测效果，而合成SAR图像在与有限的真实SAR数据结合时，显著提升了M4-SAR上的基础设施检测性能。我们的结果表明，对流匹配基础模型进行少量样本的LoRA适配，是为非可见光模态提供基础模型级别支持的一条有前景的途径。
...
Method: 作者设计了一个自定义CNN并从头训练，同时在相同实验条件下使用迁移学习对VGG-16、ResNet-50和MobileNet等主流架构进行训练，并通过准确率、精确率、召回率和F1分数等指标进行评估。

Result: 预训练CNN在分类准确率和收敛速度方面始终优于自定义CNN，尤其在训练数据有限时；而自定义CNN则以更少参数和更低计算复杂度实现了具有竞争力的性能。
...
Abstract (中文翻译): 卷积神经网络（CNN）在图像分类任务中展现出卓越的性能；然而，在从头设计自定义CNN与采用成熟的预训练架构之间如何选择，仍是一个重要的实际考量。本文对一种自定义设计的CNN与若干广泛使用的深度学习架构（包括VGG-16、ResNet-50和MobileNet）进行了对比分析。自定义CNN从零开始开发和训练，而主流架构则在相同的实验设置下采用迁移学习方式进行训练。所有模型均使用准确率、精确率、召回率和F1分数等标准性能指标进行评估。实验结果表明，当训练数据有限时，预训练CNN在分类准确率和收敛速度方面始终优于自定义CNN；然而，自定义CNN在参数数量显著更少、计算复杂度更低的情况下，仍展现出具有竞争力的性能。本研究揭示了模型复杂度、性能与计算效率之间的权衡，并为图像分类问题中选择合适的CNN架构提供了实用见解。
