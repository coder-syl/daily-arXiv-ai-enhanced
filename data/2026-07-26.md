...
TL;DR: 本文提出 RealVDeblur，一种用于真实场景视频去模糊的高效生成框架，通过基于3D高斯泼溅和高帧率视频构建逼真模糊合成数据，并结合改进的视频扩散先验与一阶段蒸馏策略，在多个真实数据集上实现高质量、语义保真且时间一致的去模糊效果，并提升下游3D重建鲁棒性。
...
Method: 1）构建基于3D高斯泼溅（3DGS）和高帧率视频的大规模物理真实模糊合成流程；2）采用视频扩散先验进行恢复，禁用VAE中的时间压缩并使用逐帧编码以适应帧间模糊变化；3）将多步扩散采样蒸馏为单步生成器以支持长视频高效推理；4）引入无需训练的Temporal Window Mask机制，在恒定内存下稳定超出训练长度的推理。

Result: 在多个真实世界基准测试中，RealVDeblur展现出优异的感知质量、语义保真度和时间一致性，并在严重运动模糊条件下显著提升下游3D重建的鲁棒性。

Conclusion: RealVDeblur通过物理真实的合成数据与高效的扩散蒸馏架构，有效提升了真实视频去模糊的泛化能力与实用性，为下游视觉任务提供了更可靠的输入。
...
Abstract (中文翻译): 由于多样的运动模式、复杂的退化过程以及逼真训练数据的稀缺，真实世界视频去模糊仍然具有挑战性，然而鲁棒的恢复对于移动成像和3D重建等下游任务至关重要。本文提出了\textbf{RealVDeblur}，一种高效的生成式框架，旨在提升在各种真实拍摄条件下的野外鲁棒性。首先，我们基于场景级3D高斯泼溅（3DGS）资产和高帧率视频构建了一个大规模、物理真实的模糊合成流程，生成涵盖相机抖动和物体运动模糊的逼真训练数据。其次，利用视频扩散先验进行恢复；为了更好地适应帧间模糊变化，我们在VAE中禁用了时间压缩，并采用逐帧编码策略。为实现在长视频上的实际部署，我们将多步扩散采样蒸馏为高效的单步生成器，并引入一种无需训练的时序窗口掩码（Temporal Window Mask），在恒定内存占用下稳定超出训练长度的推理过程。在多种真实世界基准上的大量实验表明，该方法在未见过的视频上展现出卓越的感知质量、语义保真度和时间一致性，并在严重运动模糊条件下显著提升了下游3D重建的鲁棒性。项目页面：https://rbjin.github.io/RealVDeblur
...
TL;DR: 本文提出了一种针对CAD模型B-Rep数据的自监督预训练方法Masked Topology Modeling（MTM），结合对比学习与掩码重建目标，在多个基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏以可编辑、参数化B-Rep格式提供的CAD数据集，开发适用于该领域的数据高效学习方法变得尤为重要。

Method: 提出Masked Topology Modeling（MTM）任务，利用B-Rep特有的面邻接图结构，通过掩码部分边并预测其凸性和曲线类型；同时结合MoCo风格的动量队列对比学习、BFS连通面区域掩码重建目标，并在ABC数据集和新生成的数据集上进行预训练。

Result: 在多个CAD相关基准任务上展现出强大的性能。

Conclusion: 所提出的自监督预训练策略能有效利用B-Rep结构信息，显著提升下游任务表现，为CAD领域数据高效学习提供了新途径。
...
Abstract (中文翻译): 计算机辅助设计（CAD）无处不在：几乎所有现代物体都是使用可编辑的CAD工具设计的。然而，由于缺乏以其原生可编辑和参数化格式——边界表示（B-Rep）形式提供的CAD数据集，开发适用于该领域的数据高效方法变得愈发重要。我们提出了一种新的自监督预训练任务——掩码拓扑建模（Masked Topology Modeling, MTM），该方法利用B-Rep所特有的面邻接图结构，要求编码器重建该图。MTM对图中一部分边进行掩码，并训练一个小型预测头，基于编码器在消息传递后的面特征，预测每条被掩码边的凸性及曲线类型。我们将MTM与一种基于B-Rep感知增强的MoCo风格动量队列对比学习方法、以及基于BFS连通面区域的掩码重建目标相结合，并在ABC数据集和我们新构建的过程生成数据集上进行预训练，在多个基准任务上取得了优异的性能。
...
TL;DR: Axolotl3D 是一种多模态、遮挡感知的 3D 生成模型，通过联合利用图像、可见性掩码、相机参数和部分点云，在多样条件下实现可控且一致的 3D 形状补全。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 生成模型通常假设输入图像完整可见且仅来自单视角，难以应对多视角、遮挡或编辑等复杂场景；尽管已有方法分别解决这些问题，但缺乏统一框架支持在多种条件信号下进行可控的 3D 补全。

Method: 提出 Axolotl3D 模型，联合条件化图像、可见性掩码、相机参数和部分点云：点云作为几何锚点促进形状忠实补全，相机参数确保共享 3D 坐标系下的多视角一致性，并通过统一训练策略从大规模 3D 数据中合成多样条件组合以实现跨模态推理。

Result: 在 Toys4K 和 OmniObject3D 数据集上，Axolotl3D 在干净与遮挡设置下均达到 SOTA 性能，并在真实世界重建和几何一致编辑任务中表现优异。

Conclusion: Axolotl3D 提供了一个统一、可控且鲁棒的多模态 3D 生成框架，有效应对遮挡、多视角及编辑需求，推动了 3D 内容生成的实用性和灵活性。
...
Abstract (中文翻译): 近期的 3D 生成模型利用大规模先验和扩散架构，能够从单张图像生成高质量几何。然而，这些方法假设物体完全可见且仅使用单视角输入，限制了其在多视角、遮挡或编辑场景中的应用。尽管已有研究分别应对这些挑战，但尚缺乏一个统一框架来支持在多样化条件信号下进行可控的 3D 补全。我们提出了 Axolotl3D，一种多模态且对遮挡敏感的 3D 生成模型，可同时以图像、可见性掩码、相机参数和部分点云为条件。其中，点云作为几何锚点促进形状的忠实补全，而相机参数则确保在共享 3D 坐标系中实现多视角一致性。通过统一的训练策略，该模型能从大规模 3D 数据中合成多种条件组合，从而实现鲁棒的跨模态推理。在 Toys4K 和 OmniObject3D 上的实验表明，该方法在干净和遮挡设置下均达到当前最优性能，并在真实世界重建和几何一致编辑任务中取得出色结果。
...
TL;DR: 提出ODeform，一种基于神经常微分方程的方法，用于建模可变形物体在3D空间中的连续4D动态，无需离散时间步且计算高效。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖离散时间步，要么计算开销大，难以满足实时应用需求，因此需要一种能高效建模连续形变的新方法。

Method: 将3D点云和物理条件（如材质属性）映射到统一的潜在空间，并通过求解常微分方程来建模该嵌入空间中的连续形变流。

Result: 在未见过的物理参数配置上表现出优于基线方法的运动预测精度，并成功迁移到真实3D捕获的新形状物体，支持动态插值与外推。

Conclusion: ODeform能够高效、准确地建模可变形物体的连续动态，适用于仿真与机器人等实际应用场景。
...
Abstract (中文翻译): 对连续物体形变进行建模对于许多计算机视觉和机器人任务（如操作与仿真）至关重要。现有方法依赖于基于学习的方法或物理模拟器来建模形状形变，但这些方法要么使用离散时间步，要么计算开销过大，难以满足实时应用的需求。我们提出了ODeform，这是一种将神经常微分方程（Neural ODE）扩展至3D空间中可变形物体连续4D动态建模的新方法。我们的方法将3D点云和物理条件（如材料属性）转换到一个统一的潜在空间中，并通过对时间上的常微分方程进行求解，在所学嵌入空间内将形变建模为连续流，从而无需离散时间步的同时保持计算效率。我们在未见过的物理参数配置上评估了该方法，结果表明其运动预测精度优于基线方法。实验进一步证明，该方法能成功迁移到具有新形状的真实3D捕获物体上，并能有效实现所学动态的插值与外推。我们的代码和数据将公开发布。
...
TL;DR: 本文提出一种空间锚定的概念瓶颈模型（SG-CBM），利用粗略病灶标注作为弱监督信号，提升医学影像中概念预测的空间可信度和诊断性能。


<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型在医学影像中的可解释性受限于监督信号的质量与粒度，导致概念激活可能源于无关区域，缺乏空间忠实性。

Method: 作者提出SG-CBM，利用病灶粗略勾画生成两类临床相关区域（病灶内部用于形态学概念、后方声影带用于后方现象），通过分组空间锚定目标训练概念图，并结合线性瓶颈分类器保持语义忠实性。

Result: 在五折分层组交叉验证中，SG-CBM提升了诊断AUROC和概念macro-AUROC，并显著增强了概念证据的空间对齐；通过“训练污染/测试干净”压力测试量化了监督质量对诊断与空间忠实性的影响。

Conclusion: 结果强调在可部署的医疗AI系统中，需重视监督数据质量的设计并进行系统性的可信度验证。
...
Abstract (中文翻译): 概念瓶颈模型通过以人类可理解的概念作为中介来提供“设计即解释”的预测，但在医学影像中，其可信度常受限于可用监督信号的质量与粒度。特别是，预测出的概念激活可能由无关区域驱动，导致空间上不忠实的解释。我们研究了一种以数据为中心的空间锚定概念瓶颈模型（SG-CBM），该模型利用粗略的病灶勾画作为弱监督信号，以鼓励解剖上合理的概念证据。针对乳腺超声图像，我们从每个病灶掩码中提取两个临床驱动的区域：(i) 用于形态学相关概念的病灶内感兴趣区域，以及(ii) 用于后方声学现象的后方声影带。我们使用分组空间锚定目标训练概念图，并通过线性瓶颈分类器保持语义忠实性。在五折分层组交叉验证中，所提出的SG-CBM不仅提升了诊断AUROC和概念macro-AUROC，还显著增强了概念证据的空间对齐程度。我们还进行了“训练污染/测试干净”的标注质量压力测试，以量化监督质量对诊断性能和空间忠实性的影响。总体而言，结果凸显了在可部署的医疗AI系统中，监督设计需关注数据质量，并需对可信度进行系统性验证。
...
TL;DR: 提出一种结合最优传输条件流匹配与几何潜空间过滤的隐私保护肺部CT切片生成框架，在Image-CLEFmed GANs 2026挑战赛中取得良好隐私-真实感平衡。


<details>
  <summary>Details</summary>
Motivation: 在医学图像生成中，既要保证合成图像的真实性，又要防止患者隐私泄露（如记忆化、成员推断和身份重识别），现有方法难以兼顾两者。

Method: 采用最优传输条件流匹配进行隐私导向训练，并引入“监督者”后处理流程：利用自编码器嵌入、行列式点过程（DPP）和Stein核稀疏化，在学习到的几何潜空间中过滤生成样本。

Result: 最佳模型在Image-CLEFmed GANs 2026挑战赛中获得隐私保护得分0.549和FID 0.3290，显著降低最近邻记忆化和成员推断泄露，但患者重识别分数仍较高。

Conclusion: 仅防止直接图像复制不足以消除深层次的解剖身份信息，未来需探索更深层的隐私保护机制以应对医学图像生成中的身份泄露问题。
...
Abstract (中文翻译): 我们提出了一种用于合成肺部CT切片生成的隐私保护框架，该框架为Image-CLEFmed GANs 2026挑战赛而开发。该方法结合了最优传输条件流匹配（Optimal Transport Conditional Flow Matching）与面向隐私的训练策略，并引入了一个生成后的“监督者”（Supervisor）流程：该流程利用自编码器嵌入、行列式点过程（Determinantal Point Processes）和Stein核稀疏化（Stein Kernel Thinning），在学习到的几何潜空间中对生成候选图像进行过滤。官方结果显示，该方法在真实性与隐私性之间取得了良好的平衡，其中表现最佳的模型获得了0.549的隐私保护得分（Privacy Preservation Score），并在视觉保真度方面具有竞争力，FID值为0.3290。尽管所提出的几何过滤机制显著降低了最近邻记忆化和成员推断泄露，但持续存在的患者重识别分数表明，仅防止直接图像复制尚不足以消除更深层次的患者特异性解剖身份信息，这凸显了未来隐私保护医学图像生成领域的一个重要研究方向。
...
TL;DR: 提出U-CFR框架，在推理时通过不确定性引导自动生成伪点击，实现模型自主修正分割结果，提升点击效率与边界精度。


<details>
  <summary>Details</summary>
Motivation: 现有交互式图像分割方法通常需要大量用户点击或依赖收敛缓慢的被动优化策略，难以高效准确地完成标注。

Method: 提出Uncertainty-Guided Cascade Forward Refinement（U-CFR）框架：设计边界感知的不确定性评分，融合分割不确定性、轮廓梯度和显式边缘预测，生成指向模糊边界的内部伪点击；采用双头网络结构（共享编码器-解码器主干），包含分割头和边缘头，在推理阶段级联式地利用伪点击逐步优化掩码。

Result: 在标准数据集上实验表明，U-CFR提升了点击效率、初始掩码质量和边界准确性，在Berkeley等挑战性数据集上减少超过10%所需点击数。

Conclusion: U-CFR提供了一种更智能高效的交互式图像标注方法，通过不确定性驱动的自修正机制显著减少人工干预。
...
Abstract (中文翻译): 交互式图像分割对于高效图像标注至关重要；然而，现有方法通常需要大量修正点击，或依赖收敛缓慢的被动优化策略。我们提出了不确定性引导级联前向精炼（U-CFR），这是一种新颖的推理阶段框架，使模型能够在每次用户交互后自主进行自我修正。U-CFR引入了一种边界感知的不确定性评分，该评分融合了分割不确定性、轮廓梯度和显式边缘预测，以指导内部伪点击的放置。这些自动生成的点击针对最模糊的边界区域，在无需额外人工输入的情况下提供强有力的修正信号。为支持这一过程，我们设计了一个具有共享编码器-解码器主干的双头网络：分割头确保区域一致性，而边缘头则强化边界对齐。在推理过程中，U-CFR启动一系列级联精炼步骤，每个阶段都利用不确定性驱动的伪点击逐步优化掩码。在标准基准数据集上的实验表明，所提出的U-CFR提高了点击效率、初始掩码质量和边界准确性。在Berkeley等具有挑战性的数据集上，它减少了超过10%所需的点击次数，实现了更智能、更高效的交互式标注。
...
TL;DR: 本文指出当前开放世界视频异常检测（OWVAD）评估方法未能有效衡量模型对用户指定异常定义的响应能力，提出“定义盲区”问题，并引入新的条件化评估指标和DeCoS评分规则以改进评估与性能。


<details>
  <summary>Details</summary>
Motivation: 现有OWVAD评估方法主要关注异常与正常帧的区分，忽视了模型是否真正根据用户提供的异常定义来调整检测结果，导致模型即使对定义不敏感也能获得高分。

Method: 作者将动态定义评估分解为目标异常vs正常检测与目标异常vs其他异常区分两部分，揭示评估偏差；进而提出三个定义条件化评估指标（DC-Disc、DC-DetΔ、DC-SelΔ）及DeCoS定义对比评分规则。

Result: 在UCF-Crime、XD-Violence和MSAD数据集上的实验表明，多个强基线模型虽能定位异常片段，但对定义的遵循能力弱；使用DeCoS后，在新指标上AUROC提升显著（最高达28.3点）。

Conclusion: OWVAD应被评估为基于定义的异常评分任务，而非仅在不同提示标签下的异常检测，强调评估需反映模型对具体异常定义的响应能力。
...
Abstract (中文翻译): 开放世界视频异常检测（OWVAD）旨在检测符合用户指定异常定义的事件。这一要求比通用的异常定位更强：在同一视频中，改变定义应导致被判定为异常的时间区域发生变化。我们发现，当前的OWVAD评估在很大程度上未能有效分离这种条件行为。标准的视频异常检测（VAD）指标和动态定义协议往往被“目标异常与正常帧”的区分能力所主导，使得模型即使几乎对查询定义不敏感，也能获得较高的分数。我们将这种失效模式称为“定义盲区”。为解释为何该问题被忽视，我们将动态定义评估分解为目标异常vs正常检测与目标异常vs其他异常区分两个部分，发现在常见VAD基准中，前者权重高出7.2至26.8倍。基于此诊断，我们提出了三种定义条件化的评估指标：DC-Disc、DC-DetΔ和DC-SelΔ，它们逐步消除正常帧、通用异常和多事件选择带来的捷径。在UCF-Crime、XD-Violence和MSAD上的实验表明，多个强大的VAD、OWVAD及通用视觉语言模型基线虽能定位异常时刻，但在遵循定义方面表现微弱，其对定义变化的响应幅度常接近于零。为验证该问题可被解决，我们进一步提出了DeCoS——一种定义对比评分规则，通过减去跨定义共享的异常证据来增强定义特异性。DeCoS使最强基线在DC-Disc上提升7.3–16.0 AUROC点，在DC-DetΔ上提升15.5–28.3点。总体而言，我们的结果表明，OWVAD应被评估为定义条件化的异常评分任务，而非在不同提示标签下的异常检测。
...
TL;DR: 3D-GIMP 提出一种结合单视角生成修复与多视角纹理传播的新方法，用于 3D Gaussian Splatting 中的高保真物体移除，在保持细节和一致性的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于迭代扩散模型的 3D 场景编辑方法计算开销大、难以保留锐利细节，且易因“幻觉漂移”导致多视角不一致，产生结构伪影。

Method: 3D-GIMP 仅在关键参考视图上执行一次生成式修复作为外观先验，并通过一种 3D 感知的 PatchMatch 算法将该纹理传播至其余所有视角，避免逐帧扩散的随机性。

Result: 实验表明，3D-GIMP 在修复质量上与多视角扩散方法相当，同时在渲染速度和视角一致性方面表现更优。

Conclusion: 通过优先保证重建一致性而非迭代生成，3D-GIMP 能在任意分辨率下保留高频细节并实现数学上一致的 3D 重建。
...
Abstract (中文翻译): 近期 3D 场景编辑的进展利用迭代扩散模型来更新输入视角。然而，该过程计算开销大，且难以生成清晰的细节。同时，“幻觉漂移”常导致多视角不一致，在渲染新视角时产生结构伪影。为解决这一问题，我们提出了 3D-GIMP（3D Gaussian Inpainting Meets Patch Matching），这是一种用于 3D Gaussian Splatting 中高保真物体移除的新型混合范式。3D-GIMP 不再对每个视角进行扩散，而是在一个关键参考视角上执行单次生成式修复，作为外观先验。随后，我们引入一种 3D 感知的 PatchMatch 算法，通过对应匹配将这些参考纹理传播到其余所有视角，有效绕过了逐帧扩散的随机性。通过优先考虑重建一致性而非迭代生成，3D-GIMP 能在任意分辨率下保留高频细节，同时确保数学上一致的 3D 重建。实验表明，3D-GIMP 不仅在修复质量上与以往使用多视角扩散的方法具有竞争力，而且在渲染速度和视角一致性方面表现更优。
...
TL;DR: 本文研究VR头显中虹膜识别的挑战，评估现有质量指标的局限性，利用生成模型处理VR数据特有难题，并通过多模态融合（虹膜+眼周）显著提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 传统虹膜识别协议在VR设备常见的非受控采集条件下表现不佳，需针对VR环境下的生物特征认证进行专门研究。

Method: (1) 在VRBiom数据集上评估ISO/IEC 29794-6虹膜质量指标；(2) 使用生成模型解决离轴注视、光照不均和镜面反射等问题；(3) 进行单模态（虹膜、眼周）及多模态分数级融合识别。

Result: 发现部分质量指标（如边缘充足度）在VR数据上失效；图像调整主要提升眼周识别效果；多模态融合相比单模态虹膜识别将等错误率（EER）降低约11%。

Conclusion: VR环境下的虹膜识别需定制化方法，结合眼周信息的多模态融合是提升认证性能的有效途径。
...
Abstract (中文翻译): 虚拟现实（VR）头戴设备（例如Meta Quest、Apple Vision Pro）因其在模拟环境中与物理世界实现快速、无摩擦的交互而提供无缝的用户体验。此类设备中的用户身份认证依赖于虹膜等生物特征线索。然而，在VR数据典型的非受控采集条件下，传统的虹膜识别协议可能并不充分。本研究探讨了三个关键方面：（1）在VRBiom数据集上评估ISO/IEC 29794-6虹膜质量指标并分析其局限性；（2）利用生成模型应对离轴注视、光照不均和镜面反射等数据特定挑战；（3）执行单模态（虹膜、眼周）识别及多模态分数级融合（虹膜+眼周）。我们观察到，某些指标（如边缘充足度）在VR采集的数据上失效；而图像调整主要有利于眼周识别，多模态融合相比单模态虹膜识别将等错误率（EER）降低了约11%。为确保可复现性，我们将在论文被接收后公开评估脚本。
