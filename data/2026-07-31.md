...
TL;DR: 提出一种统计方法，通过字符原型自动量化历史印刷书籍之间的字体相似性，并用于大规模文献学分析。


<details>
  <summary>Details</summary>
Motivation: 传统上依赖专家人工比对字体进行文献学研究效率低下，难以应对大量历史印刷品；因此需要一种可扩展的自动化方法来辅助或加速这一过程。

Method: 从自动提取的字符图像中聚类并对齐生成字符原型，以此定义任意两本书之间的字体距离；并采用a contrario统计框架评估该距离的显著性。

Result: 在17世纪西班牙戏剧小册子的大规模分析中，该方法成功实现了罗马体与意大利体的自动比对，经专家验证后发现了新的印刷者归属，并修正了原有归属。

Conclusion: 该方法展现出推动更大规模数字文献学研究的潜力，突破了以往人工视觉检查的局限。
...
Abstract (中文翻译): 我们提出了一种统计方法，用于量化印刷历史书籍之间字体的相似性，从而为文献学分析提供一种加速工具。该方法利用从自动提取的字符图像中通过聚类和对齐得到的字符原型，定义任意两本书之间的字体距离。为了生成可操作的结果，我们开发了一个a contrario统计框架，用于解释所计算字体距离的显著性。我们将该方法应用于17世纪西班牙印刷戏剧小册子的文献学研究，其数量超出了人类专家系统性视觉检查的能力范围。我们的方法能够自动比较从不同书籍中提取的罗马体和意大利体字体。经过人类专家验证后，该方法已促成新印刷者的发现，并修正了先前的印刷者归属。这一成功强烈表明，我们的方法有望实现比以往更大规模的数字文献学研究。
...
TL;DR: 提出OVEarth-Bench，一个更全面、多样化的开放词汇地球观测基准，涵盖广泛的类别和多样的查询形式，并在统一的零样本协议下评估现有方法，发现当前方法性能仍有限，MLLM方法表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇地球观测（EO）基准通常覆盖的类别词汇狭窄或查询形式有限，难以全面评估模型能力。因此需要构建一个类别覆盖更广、查询形式更多样的新基准。

Method: 构建OVEarth-Bench基准，从类别广度（包含正负表达的层次化类别）和查询多样性（词汇、指代表达和推理查询）两方面扩展评估；在统一的零样本协议下支持掩码和边界框定位任务，并对通用和EO专用方法进行广泛评估。

Result: 评估结果表明：(1) 当前方法性能仍有限，但更广的类别覆盖可带来更稳定的模型排名；(2) 基于MLLM的方法整体表现最强；(3) EO专用方法普遍不如通用模型，很少达到最优水平。

Conclusion: 未来开放词汇EO方法设计应借鉴这些发现，同时强调需开发更真实、多样、高质量且大规模的基准以实现可靠评估。作者已公开数据和评估工具包。
...
Abstract (中文翻译): 开放词汇地球观测（EO）旨在根据自然语言描述而非固定标签集来定位地理空间概念。然而，现有的基准通常仅涵盖狭窄的类别词汇或有限的查询形式。为填补这一空白，我们提出了OVEarth-Bench，从两个方向扩展了现有评估：一是类别广度，通过包含正负表达的广泛层次化类别覆盖；二是查询多样性，涵盖词汇、指代和推理类查询。该基准在统一的零样本协议下支持掩码和边界框定位任务。我们评估了大量通用及EO专用方法，结果表明：（1）当前方法的性能仍然有限，而更广泛的类别覆盖能带来更稳定的模型排名；（2）基于多模态大语言模型（MLLM）的方法整体表现最强；（3）EO专用方法普遍弱于通用模型，且很少能达到最优方法的水平。这些发现为未来开放词汇EO方法的设计提供了指导，并凸显了开发更真实、多样、高质量且大规模基准以实现可靠评估的重要性。我们的数据和评估工具包已发布于 https://earth-insights.github.io/OVEarth-bench。
...
TL;DR: 本文提出VETO，一种针对现代图像编辑模型（如FLUX.2）的新型抗编辑防护方法，通过干扰其联合注意力机制来阻止对参考图像的滥用，并引入新基准VetoBench评估防护效果。


<details>
  <summary>Details</summary>
Motivation: 现有抗编辑防御方法主要针对旧式扩散模型中的参考图像语义瓶颈，但无法有效应对基于联合注意力机制的新一代图像编辑模型，这些模型能实现更自由、更广泛的对象重上下文化，从而带来更大的滥用风险。

Method: 提出VETO，一种微妙的“抗编辑斗篷”，通过扰动现代编辑模型在联合注意力块中读取源图像的方式，破坏其对参考图像信息的提取；同时构建新评测基准VetoBench，涵盖局部编辑和全局上下文迁移两类任务。

Result: 在两个主流编辑模型和三个基准上，VETO均优于现有防御方法，并在保护效果与图像保真度之间取得更优平衡。

Conclusion: VETO有效应对了新一代图像编辑模型带来的安全挑战，VetoBench为未来抗编辑技术提供了更全面的评估标准。
...
Abstract (中文翻译): 强大且易于获取的图像编辑模型（如FLUX.2）的兴起，使高保真编辑变得广泛可用。这些模型的能力已不仅限于局部修改，还能将对象和身份从原图中提取并在全新场景中重新上下文化。通过允许提示词和生成词直接关注参考图像的token，现代模型模糊了传统编辑与文本到图像合成之间的界限。这种扩展的生成自由度也扩大了潜在滥用的空间，因为有害变换不再局限于可预测的一组局部编辑。现有的抗编辑防御方法旨在破坏旧式扩散模型中参考图像编码的语义瓶颈，然而，新型编辑器通过联合注意力块提炼参考信息，往往能绕过这些保护机制。因此，我们提出了VETO——一种微妙的抗编辑“斗篷”，通过干扰现代模型读取源图像的内部机制来实现防护。此外，鉴于现有编辑基准大多未充分测试全面的重上下文化能力，我们引入了VetoBench，该基准不仅评估传统局部编辑下的防御效果，还涵盖更广泛的上下文迁移场景。在两个当代编辑模型和三个基准上的实验表明，VETO始终优于现有防御方法，并在防护强度与图像保真度之间实现了更好的权衡。
...
TL;DR: 本文提出了首个能从单张插图端到端生成完整Live2D模型的系统，包含有序RGBA图层、每层的变形网格以及参数驱动的关键姿态顶点偏移。


<details>
  <summary>Details</summary>
Motivation: Live2D是动漫角色和虚拟形象主流的2D动画格式，但其制作过程繁琐耗时，需数周手动完成图层分离、遮挡补全、网格放置和关键帧设定。目前尚无生成式方法能端到端产出此类结构化资产。

Method: 系统分为两个阶段：第一阶段在Live2D感知的器官级分类体系下，将分层分解建模为分层扩散过程，生成带隐藏区域补全的有序RGBA图层栈；第二阶段仅基于每层Alpha通道构建内容贴合的三角网格，并联合预测所有图层的关键姿态位移场——每个顶点作为一个token，通过跨图层自注意力机制，将位移分解为有界方向和对数幅度。

Result: 在50个未见过的角色上，第二阶段在无教师强制的真实生成条件下，顶点方向余弦达到0.768（中位数0.828）。联合预测显著优于独立预测，而单纯扩大网络规模（112倍）并无提升。此外，由于网格由Alpha通道生成，服装图层可通过自然语言指令重绘纹理，同时复用原有网格和动画数据。

Conclusion: 该工作首次实现了从单图到完整Live2D结构化资产的端到端生成，显著简化了创作流程，并贡献了首个标准化基准Live2D-Bench及包含8,884个模型的带监督Live2D语料库。
...
Abstract (中文翻译): Live2D是动漫角色和虚拟化身主流的2D角色动画格式，它将每个角色表示为一组由逐层网格形变驱动的RGBA图层堆叠。尽管其在虚拟直播、手机游戏和交互式角色中被广泛使用，但制作一个Live2D模型仍需耗费数周时间进行手动图层分离、遮挡区域补全、网格放置和关键帧设定，且此前尚无任何生成式方法能够端到端地生成此类结构化资产。我们提出了首个系统，可从单张插图生成Live2D运行时所需的所有结构化信息：有序的RGBA图层、每层的变形网格，以及使角色运动的参数驱动关键姿态顶点偏移。第一阶段在Live2D感知的器官级分类体系下，将分层分解视为一种分层扩散过程，生成带有隐藏区域补全的有序RGBA图层堆叠。第二阶段仅根据每层的Alpha通道为其构建内容贴合的三角网格，然后联合预测所有图层的关键姿态位移场：每一层的每个顶点均为一个token，自注意力机制跨越图层边界，且每个位移被分解为一个有界方向和一个对数幅度。与独立预测相比，联合预测使得结果成为一个连贯的角色而非各自合理但割裂的部分，这是我们最大的改进；而将网络规模扩大112倍则毫无收益。在50个未见过的角色上，在无教师强制的真实生成条件下，第二阶段达到了0.768的逐顶点方向余弦（中位数为0.828）。由于图层的网格源自其Alpha通道，因此服装图层可通过自然语言指令重新纹理化，同时字节级复用原有的网格和预测动画。我们还贡献了该任务首个标准化基准Live2D-Bench，以及一个包含8,884个模型并带有图层和动画监督的Live2D语料库。
...
TL;DR: 提出一种名为共享语义码本蒸馏（SSCD）的新方法，用于在无配对、异构医学图像模态间进行跨模态知识蒸馏，通过共享离散码本对齐教师与学生模型的语义分布，在OCT→眼底照相和CT→胸片任务上显著提升学生模型性能。


<details>
  <summary>Details</summary>
Motivation: 在医学图像分析中，跨模态知识蒸馏面临两个主要挑战：不同模态数据通常来自不同患者群体（无配对），且特征空间几何不兼容，导致传统的实例级蒸馏和直接特征匹配失效。

Method: 提出Shared Semantic Codebook Distillation (SSCD)：将教师和学生模态的图像表示为一个共享的、与模态无关的离散码本上的概率分布；通过全局和类别条件对齐这些分布来传递知识，无需配对样本或可比原始特征；码本通过指数移动平均在线更新，并利用熵正则化和死码重启机制保持多样性；推理时仅保留学生编码器和分类器。

Result: 在OCT到眼底图像的视网膜疾病分类任务中，学生模型macro-F1从64.5提升至70.2；在CT到胸片的肺炎分类任务中，macro-F1从73.8提升至76.3，均优于所有基线蒸馏方法。

Conclusion: SSCD有效解决了无配对异构医学图像模态间的知识迁移难题，显著提升了轻量级学生模型的诊断性能，具有良好的部署潜力。
...
Abstract (中文翻译): 跨模态知识蒸馏能够将强大但计算成本高的教师模态中的诊断知识迁移到更廉价且易于部署的学生模态。然而在医学图像分析中，这两种模态通常是非配对的：它们来自不同的患者队列，且占据几何上不兼容的特征空间。这使得基于实例的蒸馏无效，直接的特征匹配也不可靠。为应对这些挑战，我们提出了共享语义码本蒸馏（Shared Semantic Codebook Distillation, SSCD），该方法通过一个共享的离散码本比较教师与学生的表征。每张图像被表示为一个通用的、与模态无关的词汇表上的分布，知识通过在全局和类别条件下对齐这些跨模态分布进行迁移，无需配对样本或可直接比较的原始特征。码本通过指数移动平均在线演化，并通过熵正则化和死码重启机制保持多样性。在推理阶段，所有教师端和码本模块均被丢弃，仅保留学生编码器和分类器。在两个异构非配对场景下——OCT到眼底图像的视网膜疾病分类和CT到胸部X光片的肺炎分类——SSCD分别将学生模型的macro-F1分数从64.5提升至70.2，以及从73.8提升至76.3，均优于所有评估的蒸馏基线方法。代码和预训练模型已开源：https://github.com/DillanImans/SSCD-unpaired-distillation。
...
TL;DR: 本文提出了PanDent，一个基于专家验证的细粒度牙位标注的大规模口腔全景片（OPG）基准数据集，用于评估多模态大语言模型（MLLMs）在牙科影像中的临床一致性。实验表明当前MLLMs虽能生成流畅报告，但在细粒度定位和牙位诊断上存在显著错误；在PanDent上微调可显著提升模型的临床一致性。


<details>
  <summary>Details</summary>
Motivation: 现有对多模态大语言模型（MLLMs）在牙科全景片（OPG）上的评估缺乏细粒度、临床可靠的基准，无法反映专家解读，限制了模型在真实临床场景中的应用与评估。

Method: 构建PanDent数据集：包含9,524张高质量OPG图像，由经验丰富的牙医提供结构化标注，并经口腔颌面放射科专家验证；基于临床定义的报告逻辑，将结构化发现转化为临床一致的自由文本报告，用于评估MLLMs生成报告的临床一致性。在多种MLLMs上进行实验，并在PanDent上进行微调以验证其有效性。

Result: 当前MLLMs虽能生成流畅的报告，但在细粒度牙位定位和诊断上存在严重错误，缺乏临床一致性；在PanDent上微调后，模型在视觉定位准确性和诊断正确性方面显著提升，输出更接近专家解读。

Conclusion: PanDent为评估MLLMs在牙科影像中的细粒度临床推理能力提供了严格基准，同时也是推动临床可信牙科AI发展的重要资源。
...
Abstract (中文翻译): 目前，在牙科全景X光片（口腔曲面断层片，OPG）中对多模态大语言模型（MLLMs）的准确评估受限于缺乏能够反映专家解读的细粒度、临床可靠的基准数据集。本研究提出了PanDent——一个大规模、临床可信的OPG基准数据集，其建立在经专家验证的细粒度牙位标注之上。该数据集包含9,524张高质量OPG图像，每张图像均配有由经验丰富的牙医提供的全面结构化标注，并进一步由一名口腔颌面放射科专家验证，从而为牙位级别的诊断与推理提供临床可靠的监督信号。研究利用临床医生定义的报告逻辑，从经专家验证的发现中构建出临床一致的放射学报告，明确建立了结构化临床证据与自由文本描述之间的对应关系。这一设计使得评估MLLMs所生成的报告不仅在语言上连贯，而且在临床上与专家验证的牙位发现保持一致成为可能。作者在多种MLLMs上进行了实验，包括最先进的（SOTA）闭源模型、通用领域的开源模型以及医学专用模型。结果表明，当前的MLLMs虽能生成流畅的报告，却难以产生临床一致的描述，在细粒度定位和牙位诊断方面存在显著错误。在PanDent上进行微调可显著提升结构与语言之间的一致性，大幅提高视觉定位准确率和诊断正确性，使模型输出更接近专家的牙科解读。这些结果确立了PanDent作为评估MLLMs牙位级临床推理能力的严格基准，同时也使其成为推动临床可信牙科人工智能发展的宝贵资源。
...
TL;DR: 提出VideoCoCo框架，利用可执行Blender代码作为中间表示，显著提升文本到视频模型在物理一致性方面的表现。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频模型难以生成物理上一致的动态效果，因其需从高度压缩的文本提示中隐式推断时间演化；现有思维链方法所用的中间表示通常不可执行或时间稀疏，限制了对完整时空过程的实例化与控制。

Method: 引入VideoCoCo双引擎代理框架：编码代理根据文本生成可执行的Blender程序以显式定义场景及其时间演化；仿真引擎运行该程序生成确定性的时空草稿；生成视频引擎基于该草稿进行条件编辑，产出逼真视频。同时构建VideoCoCo-3K数据集用于训练视频编辑器。

Result: 在PhyGenBench上将OmniWeaving基线从0.475提升至0.558，在VBench-2.0上从52.18提升至77.88，两项基准均取得最佳平均得分。

Conclusion: 可执行代码是一种有效、可控且可检验的中间表示，能显著改善物理一致的视频生成。
...
Abstract (中文翻译): 文本到视频模型已实现卓越的视觉质量，但仍难以生成物理上一致的动态效果，因为场景的时间演化必须从高度压缩的文本提示中隐式推断。现有的思维链方法引入了中间计划或视觉状态，但这些表示通常不可执行或时间上稀疏，限制了其对完整时空过程的实例化与控制能力。为解决此问题，我们提出了VideoCoCo——一种基于代理的双引擎框架，其中可执行的Blender代码充当过程级思维链。给定文本提示，编码代理合成一个Blender程序，显式指定场景及其时间演化；可执行仿真引擎运行该程序，生成确定性的时空草稿；随后，生成式视频引擎通过草稿条件编辑将该草稿转化为逼真视频。这种分解将过程级推理与高保真视觉实现分离。为使视频编辑器适应模拟草稿，我们构建了VideoCoCo-3K数据集，包含草稿-指令-目标三元组。VideoCoCo在PhyGenBench上将OmniWeaving基线从0.475提升至0.558，在VBench-2.0上从52.18提升至77.88，在两个基准上均取得最佳平均得分。结果表明，可执行代码为物理一致的视频生成提供了一种高效、可控且可检验的中间表示。
...
TL;DR: 本文提出一种高效的单源可迁移攻击方法（IGME），用于语义分割模型，通过链式可微攻击组件共享梯度计算，并采用路径平均方向提升稳定性，在保持良好迁移性的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有针对语义分割的可迁移对抗攻击多依赖多个代理模型进行集成，计算成本高；本文旨在探索一种仅需单个源模型、高效且具备良好迁移能力的替代方案。

Method: 将可迁移攻击构建为可微攻击组件的链式计算，共享源模型的梯度；并引入类似积分梯度的路径平均方向作为经验性稳定策略，以缓解链式组合带来的更新不稳定性。

Result: 在Pascal VOC和Cityscapes数据集上的实验表明，所提方法IGME在CNN和Transformer架构的分割模型上均实现了与单源基线相当的迁移性能，同时运行时间优于模型集成攻击。

Conclusion: IGME仅需访问一个源模型，即可在迁移性与效率之间取得良好平衡，为语义分割任务提供了一种实用的高效可迁移攻击方案。
...
Abstract (中文翻译): 语义分割模型容易受到可迁移对抗扰动的影响，但在密集预测模型上评估可迁移攻击的计算成本较高。现有的集成攻击方法通常依赖多个代理模型，增加了计算开销，对分割任务而言尤为困难。本文研究了一种高效的单源替代方案，用于语义分割的可迁移攻击。我们将可迁移攻击构造成可微攻击组件上的链式计算，从而共享昂贵的源模型梯度计算。为了减少链式组合引入的更新不稳定性，我们进一步采用一种类似积分梯度的路径平均方向作为经验性稳定启发策略。在Pascal VOC和Cityscapes数据集上的实验评估了该方法在基于CNN和Transformer的分割模型上的迁移性与效率权衡。结果表明，IGME在仅需访问一个源模型的前提下，相比单源基线方法具有有竞争力的迁移性能，且运行时间优于模型集成攻击。
...
TL;DR: 提出了一种名为ProgFormer的分层体素空间扩散Transformer模型，用于纵向脑MRI预测，在保留全局结构一致性的同时捕捉细微的局部病变进展。


<details>
  <summary>Details</summary>
Motivation: 现有方法在预测未来脑结构MRI时存在局限：潜在空间方法因压缩-重建过程导致信息损失，而直接体素空间方法则因使用统一路径建模整体结构与细微变化，易使局部病灶变化被掩盖。

Method: ProgFormer采用双路径架构：粗路径基于3D patch tokens进行整体体积预测，建模全局结构和纵向上下文；细路径以粗路径表示为时空锚点，在patch内进行体素级细化。两路径通过条件流匹配在体素空间联合估计速度场，无需单独训练图像自编码器，并通过欧拉积分从高斯噪声生成未来扫描。

Result: 在ADNI、AIBL和OASIS三个基准数据集上的成对和轨迹设置实验表明，ProgFormer优于多种当前最先进的方法。
...
Abstract (中文翻译): 预测大脑未来的结构MRI具有挑战性，因为纵向变化通常很细微且局限于特定解剖区域，而大多数个体特异性脑结构在时间上保持稳定。因此，有效的模型应在保持全局脑结构一致性的同时，对细粒度的疾病进展保持敏感。现有的潜在空间方法虽然提升了计算效率，但在压缩-重建过程中存在信息损失。相比之下，直接体素空间方法避免了潜在重建，但通常使用统一的预测路径来同时建模脑结构和与进展相关的变化，导致细微的局部变化容易被占主导地位的稳定脑结构所掩盖。为解决这些问题，我们提出了ProgFormer——一种用于纵向脑MRI预测的分层体素空间扩散Transformer。ProgFormer使用粗路径从3D patch tokens执行主要的体积预测，该路径建模整体脑结构和纵向上下文；细路径则利用粗路径的表示作为时空锚点，在单个patch内进行体素级细化。两条路径通过条件流匹配在体素空间中联合估计速度场，从而实现端到端预测，无需单独学习图像自编码器。最终的未来扫描通过在一系列欧拉步长上对估计的速度场进行积分，从高斯噪声生成。在ADNI、AIBL和OASIS三个广泛使用的基准数据集上，无论是成对还是轨迹设置，大量实验结果均表明该方法优于多种当前最先进的方法。
...
TL;DR: 本文提出Drawing-Recode框架，从光栅格式的2D CAD图纸中恢复参数化CAD序列，通过图像编码器提取几何特征、文本识别模块解析标注，并利用交叉注意力与新提出的标注对齐损失（AGL）显式关联标注与几何信息，最终由大语言模型生成结构化参数化CAD代码（SPCC），在真实工业扫描图纸上表现优异。


<details>
  <summary>Details</summary>
Motivation: 在数字化转型前积累的大量光栅格式2D CAD图纸难以直接用于零件复现和制造自动化；现有方法要么仅处理矢量图，要么局限于特定领域，且未能显式将尺寸标注与几何信息关联，限制了其在3D参数化CAD序列恢复中的应用。

Method: 提出Drawing-Recode框架：使用图像编码器提取几何特征，独立文本识别模块识别标注，通过交叉注意力机制和新设计的Annotation Grounding Loss（AGL）显式对齐标注与几何信息，再将融合特征输入大语言模型（LLM），生成结构化参数化CAD代码（SPCC）。

Result: 实验表明，Drawing-Recode优于现有基线方法，并在模拟工业环境的扫描图纸上保持鲁棒性。

Conclusion: Drawing-Recode有助于工业场景中光栅2D CAD图纸的数字化，推动零件复现与制造自动化。
...
Abstract (中文翻译): 从数字化转型之前积累的光栅格式二维计算机辅助设计（CAD）图纸中恢复参数化CAD序列，对于零件复现和制造流程自动化具有重要意义。然而，现有研究要么仅处理矢量图纸，要么局限于特定领域，且未能显式地将尺寸标注与几何信息关联起来，从而限制了其在三维参数化CAD序列恢复中对尺寸信息的有效利用。我们提出了Drawing-Recode框架，该框架能够从光栅格式的二维CAD图纸生成参数化CAD序列（即CAD代码）。Drawing-Recode通过图像编码器提取几何特征，并通过独立的文本识别模块识别标注，然后利用交叉注意力机制及我们提出的标注对齐损失（Annotation Grounding Loss, AGL）显式地将标注与几何信息对齐。所得特征随后被输入大型语言模型（LLM），以生成结构化参数化CAD代码（Structured Parametric CAD Code, SPCC）格式的CAD代码。实验表明，Drawing-Recode优于现有基线方法，并在模拟工业条件的扫描图纸上表现出良好的鲁棒性。我们期望Drawing-Recode能助力工业环境中光栅二维CAD图纸的数字化，促进零件复现与制造自动化。
