...
Motivation: 提升对人脸识别系统在对抗攻击下的脆弱性理解，并为法证分析和安全测试提供可解释、高隐蔽性的对抗样本生成与检测方法。

Method: 使用FGSM生成针对身份分类器的对抗噪声，通过扩散模型进行反向扩散、高斯平滑和自适应亮度校正以增强不可感知性；将优化后的补丁应用于人脸图像，并利用ViT-GPT2生成身份语义描述；采用感知哈希和分割技术检测对抗样本。

Result: 所提方法在保持视觉自然性的同时成功规避人脸识别系统，SSIM达0.95，并能有效分析身份分类、表情识别及语义描述在对抗条件下的变化。

Conclusion: 该端到端流程不仅有效生成高隐蔽性对抗补丁，还支持法证文档生成与对抗样本检测，为人脸生物识别系统的安全性评估提供了实用工具。
...
Abstract (中文翻译): 本研究提出了一种端到端的流程，用于生成、优化和评估对抗性补丁，以攻击面部生物识别系统，适用于法证分析与安全测试。我们采用FGSM方法生成针对身份分类器的对抗噪声，并利用扩散模型通过反向扩散、高斯平滑和自适应亮度校正来增强补丁的不可感知性，从而实现合成对抗补丁的隐蔽规避。优化后的补丁被应用于人脸图像，以测试其在保持自然视觉特征的同时规避识别系统的能力。此外，采用Vision Transformer（ViT）-GPT2模型为对抗图像生成语义描述，以支持对身份规避与识别攻击的法证解释与文档记录。该流程评估了在对抗条件下身份分类、图像描述结果以及面部身份验证和表情识别系统中的漏洞。我们进一步利用感知哈希和图像分割技术，实现了对对抗补丁和对抗样本的有效检测与分析，取得了0.95的SSIM指标。
...
TL;DR: LCF3D是一种新型多模态融合框架，通过结合RGB图像的2D检测与LiDAR点云的3D检测，利用后期融合减少LiDAR误检，并通过级联融合恢复漏检目标，在KITTI和nuScenes数据集上显著提升了对行人、骑行者等难检测类别的性能，同时具备良好的域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中准确地定位三维物体（如行人、骑行者和其他车辆）至关重要。尽管自动驾驶车辆通常同时使用RGB相机和LiDAR传感器以提升检测性能，但如何有效融合这两种模态的数据进行3D目标检测仍是一个挑战。

Method: 提出LCF3D框架，结合RGB图像上的2D检测器与LiDAR点云上的3D检测器。该方法采用两种融合策略：(i) 后期融合，通过将LiDAR 3D检测结果与RGB 2D检测结果匹配，滤除未匹配的LiDAR检测以减少误报；(ii) 级联融合，为未匹配的RGB检测生成新的3D视锥提议，以恢复LiDAR漏检的目标。

Result: 实验表明，LCF3D在不同训练与测试域的传感器配置下具有良好的域泛化能力。在KITTI数据集上，对行人和骑行者等类别有显著性能提升；在nuScenes数据集上，对摩托车和自行车也有明显改进。

Conclusion: LCF3D通过多模态融合有效提升了3D目标检测的准确性和鲁棒性，尤其在处理难检测类别和跨域场景方面表现突出，为自动驾驶感知系统提供了实用解决方案。
...
Abstract (中文翻译): 在自动驾驶中，准确地定位行人、骑行者及其他车辆等三维物体至关重要。为了确保高检测性能，自动驾驶车辆通常将RGB相机与LiDAR传感器结合使用，但如何有效融合这两种数据源以实现3D目标检测仍具挑战性。我们提出了LCF3D，一种新颖的传感器融合框架，将RGB图像上的2D目标检测器与LiDAR点云上的3D目标检测器相结合。通过利用多模态融合原则，我们弥补了LiDAR目标检测网络中的不准确性。该方案结合了两个关键原则：(i) 后期融合，通过将LiDAR的3D检测结果与RGB的2D检测结果进行匹配，并滤除未匹配的LiDAR检测，从而减少LiDAR的误报；(ii) 级联融合，通过为未匹配的RGB检测生成新的3D视锥区域提议，以恢复LiDAR漏检的目标。实验表明，LCF3D在域泛化方面具有优势，能够有效应对训练域与测试域之间传感器配置不同的情况。LCF3D相比纯LiDAR方法取得了显著改进，尤其在KITTI数据集上的行人和骑行者等具有挑战性的类别，以及nuScenes数据集上的摩托车和自行车类别上表现突出。代码可从以下网址下载：https://github.com/CarloSgaravatti/LCF3D。
...
TL;DR: 本研究比较了DenseNet121和EfficientNet-B0两种CNN模型在儿童肺炎X光图像自动诊断中的性能，发现EfficientNet-B0在准确率、F1分数和MCC等指标上均优于DenseNet121，并结合Grad-CAM与LIME提升模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 儿童肺炎仍是全球主要致病和致死原因之一，亟需高效准确的辅助诊断工具；深度学习在医学影像分析中展现出巨大潜力，尤其在胸部X光解读方面。

Method: 使用包含5,863张儿童胸片的公开数据集，经归一化、尺寸调整和数据增强后，采用ImageNet预训练权重对DenseNet121和EfficientNet-B0进行微调；评估指标包括准确率、F1分数、MCC和召回率，并利用Grad-CAM和LIME进行可解释性分析。

Result: EfficientNet-B0表现更优（准确率84.6%、F1 0.8899、MCC 0.6849），DenseNet121次之（准确率79.7%、F1 0.8597、MCC 0.5852）；两模型召回率均超0.99，Grad-CAM与LIME可视化显示关注区域符合临床相关肺部区域。

Conclusion: EfficientNet-B0在性能和计算效率上更具优势，适合临床部署；结合可解释性技术可提升AI辅助诊断的透明度与可信度。
...
Abstract (中文翻译): 背景：肺炎仍是全球儿童发病和死亡的主要原因，凸显了对准确高效诊断支持工具的需求。深度学习在医学图像分析中展现出强大潜力，尤其是在胸部X光片解读方面。本研究比较了两种最先进的卷积神经网络（CNN）架构在儿童肺炎自动检测中的表现。方法：使用一个包含5,863张儿童胸部X光图像的公开数据集。图像经过归一化、尺寸调整和数据增强以提升泛化能力。在相同训练设置下，利用ImageNet预训练权重对DenseNet121和EfficientNet-B0进行微调。性能评估指标包括准确率、F1分数、马修斯相关系数（MCC）和召回率。同时采用梯度加权类激活映射（Grad-CAM）和局部可解释模型无关解释（LIME）实现模型可解释性，以可视化影响预测结果的图像区域。结果：EfficientNet-B0优于DenseNet121，准确率达到84.6%，F1分数为0.8899，MCC为0.6849；而DenseNet121的准确率为79.7%，F1分数为0.8597，MCC为0.5852。两个模型的召回率均高于0.99，表明其对肺炎检测具有高敏感性。Grad-CAM和LIME可视化结果显示，模型关注区域与临床相关的肺部区域一致，支持了模型决策的可靠性。结论：与DenseNet121相比，EfficientNet-B0提供了更均衡且计算效率更高的性能，是临床部署的有力候选方案。结合可解释性技术有助于提升人工智能辅助儿童肺炎诊断的透明度与可信度。
...
TL;DR: 本文提出 NanoSD，一种从 Stable Diffusion 1.5 蒸馏而来的轻量级扩散模型家族，通过联合优化 U-Net 和 VAE 的结构，在保持生成先验的同时实现边缘设备上的实时推理，并在多个图像恢复任务上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有轻量级扩散模型通常仅压缩 U-Net 或缩短扩散轨迹，破坏了潜在流形并限制了跨任务泛化能力；同时，完整扩散流程在边缘设备上计算开销过大，难以部署。

Method: 通过网络剪枝、特征级生成式蒸馏和结构化架构缩放，对 U-Net 与 VAE 编解码器进行全流水线协同设计，构建 Pareto 最优的 NanoSD 模型族。

Result: NanoSD 在参数量（130M–315M）和延迟（低至 20ms）之间取得良好平衡，在超分、去模糊、人脸修复和单目深度估计等任务上优于现有轻量扩散模型，兼具感知质量和实际部署能力。

Conclusion: NanoSD 构建了一个适用于边缘设备的通用轻量扩散基础模型家族，实现了真实场景下的实时视觉生成与恢复。
...
Abstract (中文翻译): 潜在扩散模型（如 Stable Diffusion 1.5）提供了强大的生成先验，在图像恢复任务中极具价值，但其完整流程计算开销过大，难以部署于边缘设备。现有的轻量级变体主要通过压缩去噪 U-Net 或缩短扩散轨迹来减小模型规模，但这会破坏底层潜在流形，限制其在单一任务之外的泛化能力。我们提出了 NanoSD——一个从 Stable Diffusion 1.5 蒸馏而来的帕累托最优扩散基础模型家族，通过网络剪枝、特征级生成式蒸馏以及针对 U-Net 与 VAE 编解码器的结构化架构缩放进行全流水线协同设计。该方法在保留生成先验的同时，生成了一系列在精度-延迟-模型大小前沿上占据不同运行点的模型（例如参数量为 1.3 亿至 3.15 亿，在移动端 NPU 上可实现实时推理，最低延迟达 20 毫秒）。我们发现，单纯减少参数量并不一定带来硬件效率的提升，并进一步分析表明，架构平衡性、特征路由机制和潜在空间的保留共同决定了真实的端侧延迟。当作为即插即用的主干网络时，NanoSD 在图像超分辨率、图像去模糊、人脸修复和单目深度估计等多个任务上均达到当前最优性能，在感知质量和实际部署能力方面均优于以往的轻量级扩散模型。NanoSD 建立了一个适用于边缘设备的通用扩散基础模型家族，可用于实时视觉生成与恢复任务。
...
TL;DR: 本文提出了一种名为UniHash的双分支哈希框架，结合点对点（pointwise）和成对（pairwise）两种训练范式的优势，在已见和未见类别上均实现优越的图像检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度哈希方法通常局限于单一训练范式：点对点方法在已见类别上表现优异，而成对方法在未见类别上泛化能力更强。为兼顾两者优势，作者提出统一哈希框架以平衡两类场景下的检索效果。

Method: UniHash包含两个互补分支：基于中心的点对点分支和成对分支。通过一种新颖的哈希码学习方法实现双向知识迁移，包括互学习损失以对齐哈希表示，以及Split-Merge Mixture of Hash Experts (SM-MoH)模块增强跨分支表示交换。

Result: 在CIFAR-10、MSCOCO和ImageNet上的大量实验表明，UniHash在已见与未见类别的图像检索任务中均达到当前最优性能。

Conclusion: UniHash有效融合了点对点与成对哈希方法的优点，显著提升了模型在已见和未见类别上的检索能力，理论分析与实验结果共同验证了其有效性。
...
Abstract (中文翻译): 在现代图像检索系统中，实现对已见类别和未见类别的有效检索至关重要。已见类别的检索确保对已知类别的精确识别，而未见类别的检索则促进模型在有限监督下对新类别的泛化能力。然而，大多数现有的深度哈希方法局限于单一的训练范式——要么是点对点（pointwise）范式，要么是成对（pairwise）范式，前者在已见类别上表现优异，后者在未见类别上泛化能力更强。为克服这一局限，本文提出了统一哈希（Unified Hashing, UniHash），一种双分支框架，融合两种范式的优点，以在已见和未见类别上实现均衡的检索性能。UniHash包含两个互补分支：一个遵循点对点范式的基于中心的分支，以及一个遵循成对范式的分支。本文引入了一种新颖的哈希码学习方法，以实现分支间的双向知识迁移，从而提升哈希码的判别性和泛化能力。该方法采用互学习损失对齐哈希表示，并引入Split-Merge Mixture of Hash Experts（SM-MoH）模块以增强跨分支的哈希表示交换。理论分析证实了UniHash的有效性，且在CIFAR-10、MSCOCO和ImageNet上的大量实验表明，UniHash在已见和未见图像检索场景中均持续达到最先进的性能。
...
TL;DR: 本文提出了一种名为ViSIL的新评估指标，用于衡量多模态视频摘要（如关键帧+文本）的信息损失，解决了传统指标无法跨模态评估的问题。ViSIL基于视觉语言模型推理，能统一比较不同结构的摘要，并在视频问答任务中与人类和模型表现显著相关，还能优化信息损失与处理速度之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标（如BLEU、ROUGE）无法有效衡量多模态视频摘要（结合关键帧和文本）的信息覆盖程度，因为它们难以跨模态（如文本段落与关键帧序列）进行比较。

Method: 提出Video Summary Information Loss (ViSIL)评分，这是一个基于信息论的框架，利用视觉语言模型（VLM）推理来量化摘要未能捕捉的视频信息，从而衡量信息损失。

Result: ViSIL评分与人类及VLM在视频问答（VQA）任务上的表现呈显著统计相关性；利用ViSIL进行摘要选择，可在不增加处理负担的情况下，在VQA准确率上比纯文本摘要提升7%，并建立帕累托最优前沿。

Conclusion: ViSIL提供了一个统一、有效的多模态视频摘要评估方法，不仅更贴合实际任务表现，还能指导高效摘要生成，在信息保真度与计算效率之间取得更好平衡。
...
Abstract (中文翻译): 多模态视频描述将密集的视频内容浓缩为关键帧与自然语言相结合的结构化格式。通过构建连贯的多模态摘要，该方法为生成式人工智能提供了丰富的语义依据，并可作为高效检索的轻量级代理。然而，传统的BLEU或ROUGE等指标无法量化跨模态（例如将一段文本与一系列关键帧进行比较）的信息覆盖程度。为此，我们提出了视频摘要信息损失（ViSIL）评分，这是一种基于信息论的框架，通过视觉语言模型（VLM）推理来量化摘要未能捕获的视频信息。通过衡量信息损失，ViSIL成为一个统一的评估指标，能够直接比较结构迥异的多模态摘要格式。实验结果表明，ViSIL评分与人类及VLM在视频问答（VQA）任务中的表现具有统计显著的相关性。此外，ViSIL还能用于摘要选择，以优化信息损失与处理速度之间的权衡，在不增加处理负载的情况下，其构建的帕累托最优前沿在VQA准确率上比纯文本摘要高出7%。
...
<details>
  <summary>Details</summary>
Motivation: 改进CLIP通常需从头训练数十亿样本，成本高昂；作者探索是否仅用现有自监督数据集即可提升开源CLIP模型的通用性能。

Method: TuneCLIP包含两个关键阶段：(1) 基于理论分析的热身阶段，恢复优化统计量以减少冷启动偏差；(2) 微调阶段，采用新对比损失缓解对假负样本对的惩罚。

Result: 在多种架构和规模下一致提升性能，例如SigLIP (ViT-B/16) 在ImageNet及分布外基准上提升最多+2.5%，在DataComp上提升+1.2%。

Conclusion: TuneCLIP为高效后预训练适配提供了新强基线，显著提升开源CLIP模型的通用下游任务表现。
...
Abstract (中文翻译): CLIP已成为多模态表征学习的基石，但提升其性能通常需要在数十亿样本上从头开始训练，成本极高。我们提出一个不同的问题：能否仅利用现有的自监督数据集来提升开源CLIP模型在各类下游任务中的性能？与针对单一任务的监督微调不同，我们的目标是提升模型在多种任务上的通用性能。然而，正如我们的实验和先前研究所示，直接对开源CLIP模型应用标准训练协议往往会导致性能下降。为此，我们提出了TuneCLIP——一种自监督微调框架，有效克服了性能退化问题。TuneCLIP包含两个关键组件：(1) 受理论分析启发的热身阶段，通过恢复优化统计量以减少冷启动偏差；(2) 微调阶段，通过优化一种新的对比损失函数，减轻对假负样本对的惩罚。大量实验表明，TuneCLIP在不同模型架构和规模下均能持续提升性能。尤其值得注意的是，该方法显著提升了SigLIP（ViT-B/16）等领先开源模型的性能，在ImageNet及其相关分布外基准上最高提升+2.5%，在竞争激烈的DataComp基准上提升+1.2%，为高效的后预训练适配设立了新的强基线。
...
TL;DR: VibrantSR 是一种基于 Sentinel-2 卫星影像的生成式超分辨率模型，可从 10 米分辨率影像生成 0.5 米冠层高度模型，在美国西部 22 个生态区评估中 MAE 为 4.39 米，优于多个现有卫星产品，虽略逊于航拍方法，但具备大范围、高频次监测能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于航拍影像的冠层高度建模方法受限于采集频率低和时间不规律，难以支持大范围、持续性的森林监测与碳核算。因此需要一种能利用广泛可用、定期获取的卫星数据（如 Sentinel-2）进行高分辨率冠层高度估计的方法。

Method: 提出 VibrantSR 框架，一种生成式超分辨率方法，利用全球可获取的 Sentinel-2 季节性合成影像，从 10 米分辨率输入生成 0.5 米冠层高度模型（CHM）。

Result: 在美国西部 22 个 EPA 第三级生态区使用空间分离验证集评估，VibrantSR 对 ≥2 米冠层高度的平均绝对误差（MAE）为 4.39 米，优于 Meta（4.83 米）、LANDFIRE（5.96 米）和 ETH（7.05 米）等卫星基准方法；尽管基于航拍的 VibrantVS（MAE 2.71 米）精度更高，但 VibrantSR 具备操作可行性。

Conclusion: VibrantSR 能够在不依赖昂贵且稀疏的航拍数据的前提下，实现大陆尺度的森林动态监测与碳核算，具有实际应用价值。
...
Abstract (中文翻译): 我们提出了 VibrantSR（Vibrant 超分辨率），这是一种生成式超分辨率框架，用于从 10 米分辨率的 Sentinel-2 影像中估算 0.5 米分辨率的冠层高度模型（CHM）。与依赖航拍影像的方法不同——后者受限于采集频率低且时间不规律，VibrantSR 利用全球可获取的 Sentinel-2 季节性合成影像，能够以季节至年度的时间频率进行一致监测。在美国西部 22 个 EPA 第三级生态区上，使用空间分离的验证集进行评估，VibrantSR 在冠层高度 ≥2 米时的平均绝对误差（MAE）为 4.39 米，优于 Meta（4.83 米）、LANDFIRE（5.96 米）和 ETH（7.05 米）等基于卫星的基准方法。尽管基于航拍的 VibrantVS（MAE 为 2.71 米）仍具有精度优势，但 VibrantSR 无需依赖成本高昂且时间稀疏的航拍数据，即可实现大陆尺度的业务化森林监测与碳核算。
...
TL;DR: 本文提出 MedVL-SAM2，一个统一的 3D 医学多模态模型，同时支持报告生成、视觉问答（VQA）和多种分割任务（语义、指代与交互式），通过结合图像级推理与像素级感知，在多个任务上达到 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言模型在图像级文本任务上表现良好，但在实现细粒度视觉定位和三维空间推理方面仍存在挑战，尤其缺乏能统一这些能力的通用框架。

Method: 提出 MedVL-SAM2 模型，整合图像级推理与像素级感知，采用基于 SAM2 的体积分割模块；通过两阶段训练：先在大规模 3D CT 图文对上预训练以对齐视觉与语言表征，再联合优化语言理解和分割目标。

Result: 模型在报告生成、VQA 和多种 3D 分割任务上均达到最先进性能，并展现出可靠的 3D 视觉定位、可控的交互式分割和强大的跨模态推理能力。

Conclusion: 高阶语义推理与精确 3D 定位可在统一的 3D 医学 VLM 中共同实现，MedVL-SAM2 为此提供了一个有效且通用的框架。
...
Abstract (中文翻译): 近期医学视觉-语言模型（VLM）在图像级文本中心任务（如报告生成和视觉问答 VQA）上取得了显著进展。然而，在 3D 医学 VLM 中实现细粒度视觉定位和体素级空间推理仍然具有挑战性，尤其是在试图将这些能力统一到单一、可泛化的框架中时。为解决这一问题，我们提出了 MedVL-SAM2——一个统一的 3D 医学多模态模型，能够同时支持报告生成、VQA 以及多范式分割任务，包括语义分割、指代表达分割和交互式分割。MedVL-SAM2 通过专为 3D 医学影像设计的统一架构，融合了图像级推理与像素级感知，并引入基于 SAM2 的体积分割模块，以实现精确的多粒度空间推理。该模型采用多阶段训练流程：首先在大规模 3D CT 图文对语料库上进行预训练，以对齐体素视觉特征与放射学语言嵌入；随后利用全面的 3D CT 分割数据集，联合优化语言理解与分割目标。这种联合训练使模型能够通过语言、点或框提示进行灵活交互，从而将高层视觉推理与空间精确定位统一起来。我们的统一架构在报告生成、VQA 和多种 3D 分割任务上均取得了最先进的性能。大量分析进一步表明，该模型具备可靠的 3D 视觉定位能力、可控的交互式分割能力以及鲁棒的跨模态推理能力，证明了高层语义推理与精确 3D 定位可以在统一的 3D 医学 VLM 中协同实现。
...
<details>
  <summary>Details</summary>
Motivation: 大型视频扩散和流模型虽然在高质量视频生成方面取得显著成果，但其多步采样过程效率低下，限制了在实时交互应用中的使用。因此，亟需一种能有效压缩模型推理步骤、同时保留生成质量的蒸馏方法。

Method: TMD框架将扩散模型的多步去噪轨迹与少步概率转移过程对齐，每一步转移由轻量级条件流建模。方法将预训练扩散主干分解为主干（提取语义表示）和流头（执行内部流更新）两部分，并通过分布匹配蒸馏训练学生模型，在每个转移步骤中展开流头。

Result: 在Wan2.1 1.3B和14B文本到视频模型上的实验表明，TMD在生成速度与视觉质量之间实现了灵活且优越的权衡，在相近推理成本下，其生成结果在视觉保真度和提示遵循方面优于现有蒸馏模型。

Conclusion: TMD是一种高效且有效的视频扩散模型蒸馏框架，能够显著加速视频生成过程，同时维持高质量输出，为实时交互式视频生成应用提供了可行路径。
...
Abstract (中文翻译): 大型视频扩散模型和流模型在高质量视频生成方面取得了显著成功，但由于其低效的多步采样过程，它们在实时交互应用中的使用仍受到限制。在本研究中，我们提出了“转移匹配蒸馏”（Transition Matching Distillation, TMD），这是一种将视频扩散模型蒸馏为高效少步生成器的新框架。TMD的核心思想是将扩散模型的多步去噪轨迹与一个少步概率转移过程相匹配，其中每一步转移被建模为一个轻量级的条件流。为了实现高效蒸馏，我们将原始扩散主干网络分解为两个组件：（1）主干部分，包含大部分早期层，在每个外部转移步骤中提取语义表示；（2）流头部分，由最后几层组成，利用这些表示执行多次内部流更新。给定一个预训练的视频扩散模型，我们首先为其引入一个流头，并将其适配为条件流映射。然后，我们在每个转移步骤中对带有流头展开的学生模型应用分布匹配蒸馏。在Wan2.1 1.3B和14B文本到视频模型上的大量实验表明，TMD在生成速度与视觉质量之间提供了灵活而强大的权衡。特别是在相近的推理成本下，TMD在视觉保真度和提示遵循方面优于现有的蒸馏模型。项目页面：https://research.nvidia.com/labs/genair/tmd
