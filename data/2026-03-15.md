...
Motivation: 现有基于模仿学习训练的视觉-语言-动作（VLA）模型在动态环境中可靠性差，面对分布外（OOD）情况时容易失败，亟需一种能实时检测异常并触发干预机制的方法以提升系统鲁棒性。

Method: 提出RC-NF模型，利用归一化流对任务感知的机器人状态与物体轨迹进行解耦建模；仅使用正样本进行无监督训练，并通过概率密度函数在推理阶段计算异常分数；同时构建LIBERO-Anomaly-10基准用于评估。

Result: RC-NF在所有异常类型上均优于现有方法，在仿真和真实环境中均表现出色；作为即插即用模块集成到VLA模型（如pi0）中，可在100毫秒内提供OOD信号，支持状态回滚或任务重规划。

Conclusion: RC-NF显著增强了VLA机器人系统在动态环境中的鲁棒性与适应能力，为实际部署提供了可靠保障。
...
Abstract (中文翻译): 视觉-语言-动作（VLA）模型的最新进展使机器人能够执行日益复杂的任务。然而，通过模仿学习训练的VLA模型在动态环境中难以可靠运行，且在分布外（Out-of-Distribution, OOD）条件下经常失效。为解决这一问题，我们提出了机器人条件归一化流（Robot-Conditioned Normalizing Flow, RC-NF），这是一种用于机器人异常检测与干预的实时监控模型，确保机器人的状态和物体的运动轨迹与任务一致。RC-NF在归一化流框架内对任务感知的机器人状态和物体状态进行解耦处理，仅需正样本即可完成无监督训练，并在推理阶段通过概率密度函数计算精确的机器人异常分数。我们进一步提出了LIBERO-Anomaly-10基准，包含三类机器人异常，用于仿真评估。实验表明，RC-NF在所有异常类型上的监测性能均优于以往方法。真实世界实验显示，RC-NF可作为即插即用模块与VLA模型（例如pi0）集成，提供实时OOD信号，在必要时实现状态级回滚或任务级重规划，响应延迟低于100毫秒。这些结果表明，RC-NF显著提升了基于VLA的机器人系统在动态环境中的鲁棒性和适应能力。
...
TL;DR: 本文提出Geometry-Grounded Point Transformer（GGPT），通过引入稀疏几何引导增强前馈网络的三维重建效果，在保持几何一致性的同时提升细节精度和纹理缺失区域的完整性。


<details>
  <summary>Details</summary>
Motivation: 现有前馈网络在稀疏视角三维重建中缺乏显式的多视角约束，导致几何不一致性和细粒度精度不足。

Method: 首先改进基于稠密特征匹配和轻量几何优化的Structure-from-Motion流程，以高效估计相机位姿和部分点云；然后构建几何引导的三维点Transformer，利用优化后的引导编码对稠密点图进行细化，并施加部分几何监督。

Result: 实验表明GGPT能有效融合几何先验与前馈预测，在域内和域外设置下均显著优于当前最先进的前馈三维重建模型，且仅在ScanNet++上训练即可跨架构、跨数据集泛化。

Conclusion: GGPT为前馈三维重建提供了一种原则性的几何引导机制，在保证几何一致性的同时提升了空间完整性和细节恢复能力。
...
Abstract (中文翻译): 近期的前馈网络通过直接从RGB图像预测稠密点图，在稀疏视角三维重建方面取得了显著进展。然而，由于缺乏显式的多视角约束，这些方法常常面临几何不一致性和细粒度精度有限的问题。我们提出了几何引导点Transformer（Geometry-Grounded Point Transformer, GGPT），该框架通过可靠的稀疏几何引导来增强前馈重建。我们首先提出了一种改进的运动结构（Structure-from-Motion）流程，该流程基于稠密特征匹配和轻量级几何优化，能够从稀疏输入视角高效地估计精确的相机位姿和部分三维点云。在此基础上，我们设计了一个几何引导的三维点Transformer，利用优化后的引导编码，在显式部分几何监督下对稠密点图进行细化。大量实验表明，我们的方法为将几何先验与稠密前馈预测相结合提供了一种原则性机制，所生成的重建结果既具有几何一致性，又具备空间完整性，能够恢复精细结构并填补无纹理区域的空缺。GGPT仅使用ScanNet++数据集和VGGT预测进行训练，即可在不同架构和数据集之间良好泛化，在域内和域外设置下均大幅超越当前最先进的前馈三维重建模型。
...
TL;DR: 本文提出了一种基于文本引导的乳腺肿瘤分割模型（TextBCS），通过分阶段视觉-语言交互和证据学习，有效提升在低对比度和模糊边界情况下的分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的乳腺肿瘤分割方法在低对比度和模糊边界情况下难以准确定位肿瘤轮廓，限制了分割性能；引入文本提示有望改善这一问题。

Method: 提出TextBCS模型，包含分阶段的视觉-语言交互机制，在下采样各阶段融合图像与文本特征以增强对病灶区域的定位能力，并引入证据学习，利用变分狄利克雷分布量化边界分割的不确定性。

Result: 在多个公开数据集上的实验表明，TextBCS优于现有分割网络，取得了最优的乳腺肿瘤分割性能。

Conclusion: 结合文本提示、分阶段多模态交互和不确定性建模能显著提升乳腺MRI中肿瘤分割的准确性，尤其在低对比度和边界模糊场景下表现突出。
...
Abstract (中文翻译): 乳腺癌是全球女性死亡的主要原因之一，每年造成数百万人死亡。磁共振成像（MRI）可提供多种序列以刻画肿瘤形态和内部结构，已成为乳腺肿瘤检测与诊断的有效工具。然而，以往基于深度学习的肿瘤分割方法由于癌症区域与正常组织之间对比度低、边界模糊，难以准确勾画肿瘤轮廓。利用文本提示信息有助于通过明确分割区域来改善肿瘤分割效果。受此启发，我们提出了文本引导的乳腺肿瘤分割模型（TextBCS），该模型结合分阶段的视觉-语言交互机制与证据学习。具体而言，所提出的分阶段视觉-语言交互在下采样各阶段促进视觉特征与文本特征之间的信息互融，进一步发挥文本提示在低对比度场景下辅助定位病灶区域的优势。此外，采用证据学习对模型在模糊边界处的分割不确定性进行量化，通过变分狄利克雷分布刻画分割概率的分布，从而有效处理边界分割的不确定性。大量实验验证了TextBCS相较于其他分割网络的优越性，在公开数据集上实现了最佳的乳腺肿瘤分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于预训练视觉-语言模型的增量学习方法存在训练效率低、依赖记忆库、需强主干网络等问题，亟需更高效简洁的解决方案。

Method: 提出SimE框架，在视觉-语言模型中引入专为增量学习设计的适配器，并研究适配器连接数量与模型性能间的非线性关系。

Result: SimE在TinyImageNet上比传统方法高9.6%，在CIFAR-100上比其他CLIP方法高5.3%；同时发现增加Transformer块间适配器可提升性能，但块内过多适配器反而有害。

Conclusion: SimE是一种高效且简单的增量学习方法，通过合理设计适配器结构可有效利用CLIP的零样本能力，且更强的CLIP主干（如ViT-L/14）可进一步提升性能。
...
Abstract (中文翻译): 增量学习（IL）旨在学习新任务的同时保留先前获得的知识。将预训练视觉-语言模型的零样本学习能力融入增量学习方法已取得显著进展。然而，这些方法面临三大挑战：（1）训练效率有待提升；（2）依赖用于存储历史数据的记忆库；（3）需要强大的主干网络以增强模型能力。本文提出SimE——一种简单高效的框架，采用专为增量学习任务设计适配器的视觉-语言模型。我们发现一个显著现象：适配器连接数量与模型增量学习能力之间存在非线性关系。虽然增加Transformer块之间的适配器连接可提升模型性能，但在较小增量步长下，在Transformer块内部添加更多自适应连接不仅无法提升、甚至可能损害模型的增量学习能力。大量实验结果表明，SimE在TinyImageNet上比传统方法高出9.6%，在CIFAR-100上比其他基于CLIP的方法高出5.3%。此外，我们还系统性地研究了如何更好地利用CLIP的零样本能力，建议将SimE的编码器替换为在更大规模数据集（如LAION2B）上训练、采用更强架构（如ViT-L/14）的CLIP模型。
...
TL;DR: Senna-2 improves consistency between vision-language model decisions and end-to-end driving policy planning through a three-stage training approach, significantly boosting safety and decision-following.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-E2E driving systems suffer from misalignment between high-level decisions from VLMs and low-level trajectory planning from E2E policies, weakening top-down guidance and decision adherence.

Method: Senna-2 uses a three-stage consistency-oriented training: (1) driving pre-training with a decision adapter, (2) open-loop alignment of VLM and E2E, and (3) closed-loop alignment via hierarchical reinforcement learning in 3DGS environments.

Result: Senna-2 achieves a 19.3% F1 score improvement in dual-system consistency, 5.7% FDE reduction in open-loop, and 30.6% AF-CR reduction in closed-loop settings.

Conclusion: Explicitly aligning VLM decisions with E2E planning through staged training significantly enhances both consistency and safety in autonomous driving systems.
...
TL;DR: 本文提出FMVR，一种即插即用的频域调制视觉恢复策略，在大幅减少视觉token数量的同时几乎无损地保持LMM性能。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型（LMMs）因视觉token数量庞大而难以适应不同计算预算，而现有减少token的方法会损失视觉语义信息。

Method: FMVR将少量视觉token的表征分解为高低频分量（通过AvgPool和MaxPool），并用轻量可学习参数进行调制：AvgPool的高频作为显著性滤波器增强显著语义，MaxPool的低频作为反显著性滤波器强化弱语义。同时结合Matryoshka表示学习实现推理时弹性调整token数量。

Result: 在10个图像和4个视频基准上，FMVR-LLaVA将LLaVA-1.5-7B的FLOPs降低89%，同时保持近100%原始准确率。

Conclusion: FMVR是一种简单有效的即插即用策略，可在大幅压缩计算开销的同时有效保留和恢复视觉语义，使LMM能灵活适应不同计算预算。
...
Abstract (中文翻译): 大型多模态模型（LMMs）由于大量视觉token的存在，难以适应不同的计算预算。以往的方法尝试在大语言模型（LLM）之前或内部减少视觉token的数量，但这些策略不可避免地导致视觉语义的丢失。为解决这些问题，我们提出了FMVR——一种即插即用且极其简单的频域调制视觉恢复策略，以在视觉token减少的情况下提升LMM的推理能力。具体而言，FMVR通过AvgPool和MaxPool将较少视觉token的视觉表征解耦为低频和高频分量，并使用轻量级可学习参数对这些频率分量进行调制。其中，AvgPool产生的高频分量充当显著性滤波器，用于增强显著的视觉语义；而MaxPool产生的低频分量则作为反显著性滤波器，用于强化较弱的视觉语义。这使得由少量视觉token主导的视觉语义得以保留，同时稀释的视觉语义也能被恢复。此外，我们将FMVR引入套娃表示学习（Matryoshka Representation Learning）中，以学习从粗到细的视觉token集合，从而在推理过程中能够弹性调整视觉token数量，同时保持相当的性能。在10个基于图像和4个基于视频的基准上的实验表明，FMVR-LLaVA将LLaVA-1.5-7B的FLOPs降低了89%，同时几乎保持了100%的原始准确率。代码将开源。
...
TL;DR: 本文提出了一种轻量级的slot合并机制，通过在训练过程中动态合并重叠的slots，提升了基于slot的对象中心学习在对象分解和分割任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有slot-based方法通常使用固定数量的slots，导致多个slots可能竞争同一实体的重叠区域，而非各自关注不同区域，从而影响对象分解效果。

Method: 引入slot merging操作：利用Soft-IoU度量slot注意力图之间的重叠程度，并采用重心更新（barycentric update）合并选定的slot对；合并策略基于固定规则，阈值由重叠统计推断，无需额外可学习模块。

Result: 在DINOSAUR框架中集成该方法后，在对象发现与分割基准上优于其他自适应方法，显著提升了对象分解能力和掩码质量。

Conclusion: slot merging是一种有效且轻量的改进策略，能增强slot-based模型对场景中对象的解耦能力，无需增加模型复杂度。
...
Abstract (中文翻译): 基于slot的对象中心学习方法将图像表示为一组潜在slot，并通过解码器将这些slot组合成图像或特征。解码器规定了如何将slot组合为输出，但slot集合通常是固定的：slot数量需预先设定，且仅进行细化。这可能导致多个slot竞争同一实体的重叠区域，而非各自聚焦于不同区域。我们提出了slot合并（slot merging）：一种即插即用、轻量级的slot集合操作，可在训练过程中合并重叠的slot。我们通过slot注意力图之间的Soft-IoU分数量化重叠程度，并通过保持梯度流的重心更新（barycentric update）来合并选定的slot对。合并遵循固定策略，其决策阈值由重叠统计推断得出，无需引入额外的可学习模块。将该方法集成到DINOSAUR已有的特征重建流程中，显著提升了对象分解和掩码质量，在对象发现与分割基准测试中超越了其他自适应方法。
...
TL;DR: 本文提出利用多次移动激光扫描数据提取城市语义三维模型中物体表面的“辐射指纹”，以揭示其材质特性，并实现了大规模LiDAR观测与高精度语义城市模型的自动关联。


<details>
  <summary>Details</summary>
Motivation: 当前语义三维城市模型虽日益精细，但缺乏对材料及其物理属性的结构化表达，限制了城市数字孪生的分析能力；而重复采集的移动激光扫描数据蕴含丰富的材质相关信息，亟需有效方法加以利用。

Method: 通过将来自不同距离、入射角、环境条件、传感器和扫描任务的LiDAR回波观测按语义对象分组，构建物体表面的辐射指纹；利用A2D2数据集中的3.124亿条激光束自动关联到6368个LOD3级语义城市模型对象。

Result: 成功提取出具有类内一致性的辐射指纹，反映出主导各类别物体的典型材质特征，并基于CityGML 3.0标准构建了厘米级精度的语义三维城市模型。

Conclusion: 该方法有效挖掘了多时相LiDAR数据中的材质信息，拓展了语义三维城市模型的应用潜力，相关模型、方法及3DSensorDB地理数据库已开源发布。
...
Abstract (中文翻译): 尽管语义三维城市模型已在国际上广泛应用且日益精细化，但对材料信息的整合仍处于未充分开发的状态。然而，对材料及其物理属性进行结构化表达，可显著拓宽城市数字孪生的应用范围并增强其分析能力。与此同时，城市及其街道空间的重复移动激光扫描正产生大量受对应表面材质特性影响的观测数据。为充分利用这些信息，我们提出通过将来自不同距离、入射角、环境条件、传感器及扫描任务下、从同一语义对象反射的LiDAR观测进行分组，构建物体表面的辐射指纹。本研究展示了如何将Audi自动驾驶数据集（A2D2）车辆在四次扫描任务中使用五种LiDAR传感器获取的3.124亿条独立激光束自动关联至语义三维城市模型中的6368个独立对象。该模型基于CityGML 3.0标准，包含四条城区街道厘米级精度的LOD3级详尽语义表达，并支持对象的细粒度细分。所提取的物体表面辐射指纹揭示了类内重复出现的模式，表明了各类别占主导地位的材质。相关语义模型、方法实现及所开发的地理数据库解决方案3DSensorDB已开源发布于：https://github.com/tum-gis/sensordb
...
TL;DR: 本文提出一种仅使用RGB图像实现患者注册与解剖引导初始探头放置（PIPG）的自动化框架，用于辅助远程超声检查中的新手或机器人在无专家现场指导下准确定位探头。


<details>
  <summary>Details</summary>
Motivation: 心脏和肺部超声操作技术要求高，需识别个体化的肋间隙声窗并在不同成像平面间调整探头位置、旋转和力度；在远程超声场景中，若由新手或机器人操作且缺乏专家现场指导，初始探头放置尤为困难。

Method: 利用混合现实头戴显示器（HMD）上的校准相机采集患者RGB图像，边缘服务器推断出个体化的体表与骨骼模型，并通过多视角空间平滑优化；基于预测骨骼的骨性标志估计肋间隙区域，并将引导信息投影回重建的体表；通过在多个经胸超声心动图扫描平面上叠加重建体表网格与虚拟探头姿态进行验证。

Result: 对健康志愿者的初步实验表明，所提出的探头放置预测与MR引导可在解剖变异允许范围内实现一致的初始探头定位，适用于远程超声设置。

Conclusion: 该框架有效解决了远程超声中初始探头放置难题，为未来无人或低技能操作下的远程超声应用提供了可行的技术路径。
...
Abstract (中文翻译): 心脏和肺部超声在技术上具有挑战性，因为操作者必须识别患者个体化的肋间隙声窗，并通过调整探头的位置、旋转角度和施加力度，在不同成像平面之间切换以获取标准切面。当在远程超声场景中由新手或机器人操作，且缺乏现场专家协助时，这些挑战尤为突出，尤其是在初始探头放置阶段。本文提出了一种仅利用校准相机拍摄的RGB图像来自动完成患者注册与基于解剖结构的初始探头放置引导（PIPG）的框架。新手首先使用混合现实（MR）头戴式显示器（HMD）上的相机拍摄患者图像，边缘服务器随后推断出患者个体化的体表与骨骼模型，并在多个视角间进行空间平滑处理。利用预测骨骼中的骨性标志，我们估计出肋间隙区域，并将引导信息投影回重建的体表。为验证该框架，我们在多个经胸超声心动图扫描平面上原位叠加了重建的体表网格与虚拟探头姿态引导，并测量了定量放置误差。对健康志愿者的初步实验表明，所提出的探头放置预测与MR引导能够在解剖变异可接受的范围内实现一致的初始探头定位，适用于远程超声设置。
...
TL;DR: InstantHDR 是一种前馈网络，可从未经校准的多曝光低动态范围（LDR）图像中单次前向推理重建高动态范围（HDR）3D场景，在保持与现有优化方法相当性能的同时，速度提升高达700倍。


<details>
  <summary>Details</summary>
Motivation: 现有HDR新视角合成方法依赖已知相机位姿、稠密点云初始化和耗时的逐场景优化；而当前的前馈方法则忽略了HDR问题，假设外观在不同曝光下不变。为解决这一问题，作者提出InstantHDR以实现高效且无需复杂先验的HDR场景重建。

Method: InstantHDR 采用几何引导的外观建模进行多曝光融合，并引入元网络实现可泛化的场景特定色调映射。此外，作者构建了包含168个Blender渲染场景、多样光照类型和多种相机响应函数的HDR-Pretrain预训练数据集。

Result: 实验表明，InstantHDR 在合成质量上与最先进的基于优化的HDR方法相当，同时在单次前向推理和后优化设置下分别实现了约700倍和20倍的速度提升。

Conclusion: InstantHDR 成功实现了高效、无需精确标定的HDR新视角合成，为HDR场景重建提供了实用且可扩展的前馈解决方案。
...
Abstract (中文翻译): 高动态范围（HDR）新视角合成（NVS）旨在从多曝光低动态范围（LDR）图像中重建HDR场景。现有的HDR流程严重依赖已知的相机位姿、良好初始化的稠密点云以及耗时的逐场景优化。当前的前馈替代方法则通过假设外观在不同曝光下不变而忽略了HDR问题。为弥合这一差距，我们提出了 InstantHDR —— 一种前馈网络，可在单次前向传播中从未校准的多曝光LDR图像集合中重建3D HDR场景。具体而言，我们设计了一种几何引导的外观建模用于多曝光融合，以及一个用于可泛化场景特定色调映射的元网络。由于缺乏HDR场景数据，我们构建了一个名为 HDR-Pretrain 的预训练数据集，包含168个Blender渲染场景、多样化的光照类型和多种相机响应函数。全面的实验表明，我们的 InstantHDR 在合成性能上与最先进的基于优化的HDR方法相当，同时在单次前向推理和后优化设置下分别实现了约700倍和20倍的重建速度提升。所有代码、模型和数据集将在审稿流程结束后公开。
