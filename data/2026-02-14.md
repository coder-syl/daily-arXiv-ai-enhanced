...
TL;DR: 本文提出DD-MDN，一种端到端的概率性人类轨迹预测模型，结合去噪扩散机制与混合密度网络，在短观测条件下实现高精度、可靠不确定性建模和多样轨迹生成。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注预测准确性、社会交互建模和轨迹多样性，但对不确定性建模、校准能力以及在短观测窗口下的预测鲁棒性关注不足，而这些对于路径规划和避障等下游任务至关重要。

Method: 提出DD-MDN模型，采用少样本去噪扩散主干网络和双混合密度网络，学习自校准的驻留区域和按概率排序的锚点路径，无需预设锚点或终点即可生成多样化的轨迹假设。

Result: 在ETH/UCY、SDD、inD和IMPTC数据集上，DD-MDN在预测精度、短观测鲁棒性和不确定性建模方面均达到SOTA水平。

Conclusion: DD-MDN有效解决了短观测下的人类轨迹预测问题，同时提供准确且校准良好的不确定性估计，适用于自动驾驶等安全关键场景。
...
Abstract (中文翻译): 人类轨迹预测（HTF）旨在根据过去轨迹和环境上下文预测未来人类运动，广泛应用于自动驾驶、智能监控和人机交互等领域。尽管以往工作聚焦于预测准确性、社会交互建模和轨迹多样性，但对不确定性建模、校准能力以及在短观测窗口下的预测性能关注较少，而这些对于路径规划和碰撞避免等下游任务至关重要。本文提出DD-MDN，一种端到端的概率性HTF模型，结合高位置精度、校准良好的不确定性估计以及对短观测的鲁棒性。该方法利用少样本去噪扩散主干网络和双混合密度网络，学习自校准的驻留区域和按概率排序的锚点路径，从而在无需预设锚点或终点的情况下生成多样化的轨迹假设。在ETH/UCY、SDD、inD和IMPTC数据集上的实验表明，该方法在准确性、短观测鲁棒性和不确定性建模方面均达到当前最优水平。代码已开源：https://github.com/kav-institute/ddmdn。
...
TL;DR: ABot-M0 是一个通用具身智能框架，通过统一数据集构建、动作流形学习和模块化感知机制，实现跨多种机器人形态的高效策略学习与泛化。


<details>
  <summary>Details</summary>
Motivation: 当前机器人领域缺乏统一的数据表示、训练目标和模型架构，导致难以构建适用于多种硬件平台的通用具身智能体（“一脑多形”问题）。

Method: 提出 ABot-M0 框架：1）构建包含600万轨迹的统一数据集 UniACT；2）基于动作流形假设（Action Manifold Hypothesis），引入动作流形学习（AML）方法，使用 DiT 骨干网络直接预测低维连续动作；3）采用双流模块化感知机制，融合 VLM 语义与几何先验，并支持即插即用的 3D 模块。

Result: 实验表明各组件可独立运行并带来叠加性能提升，显著提高动作预测效率、策略稳定性及跨平台泛化能力。

Conclusion: ABot-M0 为构建通用具身智能提供了一套系统性解决方案，涵盖数据、模型与感知，代码和流程将开源以促进后续研究。
...
Abstract (中文翻译): 在机器人领域，构建适用于多种硬件平台的通用具身智能体（即“一脑多形”范式）仍是一个核心挑战。当前进展受限于数据碎片化、表示不一致以及训练目标不匹配等问题。本文提出 ABot-M0 框架，通过构建系统化的数据整理流程，并联合优化模型架构与训练策略，实现从异构原始数据到统一高效表示的端到端转换。我们从六个公开数据集中清洗、标准化并平衡样本，构建了大规模 UniACT 数据集，包含超过600万条轨迹和9500小时的数据，覆盖多样化的机器人形态与任务场景。统一预训练提升了跨平台与跨任务的知识迁移与泛化能力，支持通用具身智能。为提升动作预测的效率与稳定性，我们提出“动作流形假设”：有效的机器人动作并非分布于完整的高维空间，而是位于由物理规律和任务约束所决定的低维光滑流形上。基于此，我们引入动作流形学习（Action Manifold Learning, AML），利用 DiT 骨干网络直接预测干净、连续的动作序列，将学习目标从去噪转变为向可行流形投影，从而提升解码速度与策略稳定性。此外，ABot-M0 通过双流机制支持模块化感知，融合视觉语言模型（VLM）语义与几何先验，并集成 VGGT 和 Qwen-Image-Edit 等即插即用的 3D 模块提供的多视角输入，在不修改主干网络的前提下增强空间理解能力，缓解标准 VLM 在 3D 推理中的局限性。实验表明，各组件可独立运行并带来叠加收益。我们将开源全部代码与流程，以支持可复现性与未来研究。
...
TL;DR: 本文提出了一种基于深度学习的自动化茶树叶病害分类方法，利用茶LeafBD数据集（5278张高分辨率图像，7类）训练DenseNet201和EfficientNetB3模型，结合对抗训练与Grad-CAM可解释性技术，最终EfficientNetB3达到93%准确率，为农业病害管理提供实用方案。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国经济高度依赖茶叶产业，但茶树易受多种叶部病害影响，导致产量下降和品质降低。传统人工检测效率低、易出错，因此亟需一种高效、准确的自动病害识别方法以支持农业生产。

Method: 研究构建了一个包含数据预处理、划分、对抗训练、数据增强、模型训练、评估及可解释性分析的完整流程。采用DenseNet201和EfficientNetB3作为主干网络，并引入对抗训练提升模型鲁棒性，使用Grad-CAM可视化模型决策依据。

Result: 在茶LeafBD数据集上，EfficientNetB3模型取得了93%的最高分类准确率，DenseNet201达到91%，表明所提方法能有效识别茶树叶病害。

Conclusion: 该研究验证了深度学习结合对抗训练与可解释AI技术在茶树叶病害自动识别中的有效性，为现代农业管理提供了可靠、高效的解决方案。
...
Abstract (中文翻译): 茶叶是孟加拉国经济的重要资产，因此茶叶种植在推动经济发展中发挥着关键作用。然而，这些珍贵的植物容易受到多种叶部病害的侵袭，可能导致产量减少和品质下降。人工检测这些病害并不容易，耗时且可能存在误判。因此，本研究旨在基于teaLeafBD数据集开发一种自动化的深度学习模型，用于茶树叶病害分类，使任何人都能更轻松、高效地识别病害。该数据集包含5,278张高分辨率图像，分为七类：其中六类代表不同病害，一类代表健康叶片。所提出的流程包括数据预处理、数据划分、对抗训练、数据增强、模型训练、评估以及借助可解释人工智能（Explainable AI）策略进行模型理解。研究采用了DenseNet201和EfficientNetB3执行分类任务。为增强模型鲁棒性，应用了对抗训练，使其即使在输入存在噪声或干扰的情况下也能有效运行。此外，还通过Grad-CAM可视化技术分析模型预测，识别每张图像中最具影响力的区域。实验结果表明，EfficientNetB3取得了93%的最高分类准确率，而DenseNet201达到了91%。结果证明，所提出的方法能够准确检测茶树叶病害，为先进农业管理提供了切实可行的解决方案。
...
TL;DR: 本文提出Active-Zero框架，通过主动探索视觉环境替代传统自博弈中对静态图像的被动使用，显著提升视觉语言模型的自主学习能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的自博弈方法依赖于静态图像集合，导致对初始数据集依赖性强、学习效率低，无法根据模型能力动态调整学习内容。

Method: Active-Zero引入三个协同演化的智能体：Searcher从开放世界图像库中检索与模型能力前沿匹配的图像，Questioner生成难度适配的推理任务，Solver则通过准确率奖励进行优化，形成闭环自引导课程。

Result: 在Qwen2.5-VL-7B-Instruct模型上，Active-Zero在12个基准测试中推理任务平均准确率达53.97%（提升5.7%），通用理解达59.77%（提升3.9%），均优于现有自博弈基线。

Conclusion: 主动探索是构建可扩展、自适应的视觉语言自演化系统的关键要素。
...
Abstract (中文翻译): 自博弈使大语言模型能够通过自我生成的挑战实现自主提升。然而，现有的视觉语言模型自博弈方法依赖于与静态图像集合的被动交互，导致对初始数据集高度依赖且学习效率低下。由于无法主动获取与其不断演进能力相匹配的视觉数据，智能体往往在过于简单或超出当前能力范围的样本上浪费计算资源。为解决这些问题，我们提出了Active-Zero框架，将交互方式从被动转向对视觉环境的主动探索。Active-Zero采用三个协同演化的智能体：Searcher根据模型的能力前沿从开放世界图像库中检索图像，Questioner合成经过校准的推理任务，而Solver则通过准确率奖励进行优化。这一闭环机制实现了自我引导的自动课程，使模型能够自主构建其学习轨迹。在Qwen2.5-VL-7B-Instruct模型上，Active-Zero在12个基准测试中推理任务平均准确率达到53.97%（提升5.7%），通用理解达到59.77%（提升3.9%），始终优于现有的自博弈基线方法。这些结果表明，主动探索是构建可扩展、自适应的视觉语言自演化系统的关键要素。
...
TL;DR: ReTracing 是一个多智能体具身表演艺术项目，通过考古学方法探索 AI 如何塑造、限制并生成身体动作，并揭示生成系统如何通过编舞编码社会文化偏见。


<details>
  <summary>Details</summary>
Motivation: 探讨人工智能如何影响人类身体动作，并揭示生成式 AI 系统在编舞中隐含的社会文化偏见，同时反思“在会移动、思考并留下痕迹的 AI 之中，何以为人”这一当代关键问题。

Method: 从科幻小说中提取描述人机交互的句子，利用大语言模型（LLMs）为每段摘录生成“应做”与“不应做”的成对提示；再用基于扩散的文本到视频模型将这些提示转化为人类表演者的编舞指导和四足机器人的电机指令。人类与机器人在镜面地板上执行动作，由多摄像头运动捕捉系统记录，并重建为三维点云和运动轨迹，形成动作痕迹的数字档案。

Result: 成功构建了一个融合人类、AI 与机器人互动的沉浸式表演系统，并生成了可分析的动作数字档案，揭示了生成模型在动作生成中嵌入的文化偏见。

Conclusion: ReTracing 提供了一种新颖的方法，通过具身表演艺术揭示 AI 系统中的社会文化编码，并引发对人类在 AI 时代身份与存在意义的深层思考。
...
Abstract (中文翻译): 我们提出了 ReTracing，这是一种多智能体具身表演艺术项目，采用考古学方法来考察人工智能如何塑造、限制并生成身体动作。该项目从科幻小说中提取描述人机交互的句子，并利用大语言模型（LLMs）为每段摘录生成“应做”与“不应做”的成对提示。随后，基于扩散的文本到视频模型将这些提示转化为人类表演者的编舞指导以及四足机器人的电机指令。人类表演者与机器人在镜面地板上执行这些动作，由多摄像头运动捕捉系统记录，并重建为三维点云和运动轨迹，形成动作痕迹的数字档案。通过这一过程，ReTracing 成为一种揭示生成系统如何通过编舞动作编码社会文化偏见的新颖方法。在人工智能、人类与机器人沉浸式互动的过程中，ReTracing 直面我们这个时代的一个关键问题：在同样能够移动、思考并留下痕迹的人工智能之中，何以为人？
...
TL;DR: 本文发现当前视频-语言模型（VidLMs）在处理视频内容、时序和运动方面存在严重缺陷，并提出了REVEAL诊断基准，通过五项压力测试揭示其弱点。


<details>
  <summary>Details</summary>
Motivation: 探究视频-语言模型是否真正理解视频内容、时间顺序和运动，因为现有模型可能依赖语言捷径或忽略关键视觉信息。

Method: 提出REVEAL诊断基准，包含五项受控压力测试：时间预期偏差、仅依赖语言的捷径、视频奉承行为、对摄像机运动的敏感性、以及对时空遮挡的鲁棒性；同时提供自动生成诊断样本的数据管道。

Result: 主流开源和闭源VidLM在所有测试中表现不佳：将倒放视频描述为正向、忽略视频内容作答、认同错误陈述、难以处理基本摄像机运动、且在简单时空遮挡下无法整合时间信息；而人类轻松完成这些任务。

Conclusion: 当前VidLM并未稳健地建模视频中的时空信息，REVEAL基准可推动更全面、可扩展的评估，作者将公开发布该基准和代码以促进后续研究。
...
Abstract (中文翻译): 本研究探讨了一个基本问题：视频-语言模型（VidLMs）是否能够稳健地理解视频内容、时间顺序和运动？我们的研究表明，令人惊讶的是，它们往往不能。我们提出了REVEAL{}诊断基准，通过五项受控压力测试来揭示当前VidLMs的根本弱点，包括时间预期偏差、对纯语言捷径的依赖、视频奉承行为、对摄像机运动的敏感性，以及对时空遮挡的鲁棒性。我们对主流的开源和闭源VidLM进行了测试，发现这些模型会自信地将倒放的场景描述为正向播放，在回答问题时忽略视频内容，认同错误的陈述，难以应对基本的摄像机运动，并且在简单的时空遮挡下无法有效聚合时间信息。相比之下，人类能轻松完成这些任务。除了该基准外，我们还提供了一个数据管道，可自动生成用于压力测试的诊断样本，从而实现更广泛、更具可扩展性的评估。我们将发布该基准和代码以支持未来的研究。
...
TL;DR: 本文提出了一种新方法，通过从高质量3D模型和程序生成的相机姿态合成图像，用于可重复、可量化的摄影测量重建评估。


<details>
  <summary>Details</summary>
Motivation: 当前基于摄影测量法生成数字孪生的方法存在多种设计选择，但其效果多依赖主观定性评价，缺乏系统性、可量化的评估手段。

Method: 构建一个新流程：利用高质量3D模型和程序化生成的虚拟相机姿态，合成逼真的2D图像；然后用这些图像进行摄影测量重建，并将重建结果与已知的虚拟相机参数和物体真值进行对比。

Result: 该方法支持大量可重复、可量化的实验，能够精确评估不同摄影测量策略在重建精度方面的表现。

Conclusion: 所提出的合成图像生成与评估框架为摄影测量和数字孪生技术提供了客观、可复现的量化评测基准。
...
Abstract (中文翻译): 从现实世界物体生成3D模型通常通过摄影测量法实现，即从多个视角拍摄2D照片，然后通过三角化匹配的点特征来构建带纹理的网格。在此框架下，生成数字孪生体存在多种设计选择，而这些方法之间的差异主要依靠定性判断。本文提出并测试了一种新颖的流程，该流程从高质量3D模型和程序化生成的相机姿态中生成合成图像。这使得可以开展大量可重复、可量化的实验，将虚拟相机参数和虚拟物体的真值与重建所得的视角和对象估计结果进行对比。
...
TL;DR: 本文提出SYNC loss，将后验（post-hoc）方法中的softmax响应引入到训练阶段，与先验（ad-hoc）方法SelectiveNet结合，提升模型在选择性预测任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有选择性预测方法分为两类：一类是修改网络结构或目标函数的ad-hoc方法（如SelectiveNet），另一类是基于模型输出概率的post-hoc方法（如softmax响应）。作者发现post-hoc方法隐含生成的选择性先验（selective prior）在训练阶段同样重要，但以往仅用于推理阶段。

Method: 提出SYNC loss，将post-hoc方法（softmax响应）产生的选择性先验信息整合进SelectiveNet的训练过程中，从而联合优化模型的预测与选择能力。

Result: 在CIFAR-100、ImageNet-100和Stanford Cars等多个数据集上实验表明，该方法不仅提升了模型泛化能力，还在选择性预测任务中超越了现有方法，达到新的SOTA水平。

Conclusion: 将选择性先验信息从推理阶段引入训练阶段，能有效提升选择性预测性能；SYNC loss成功融合了ad-hoc与post-hoc方法的优势，为不确定性下的可靠预测提供了新思路。
...
Abstract (中文翻译): 在不确定性下进行预测是深度神经网络实现负责任决策的关键要求。本文聚焦于选择性预测，即允许深度神经网络根据其预测的不确定性水平，自主决定是否进行预测或放弃预测。当前的方法要么是临时设计的（ad-hoc），例如SelectiveNet，侧重于修改网络架构或目标函数；要么是事后分析的（post-hoc），例如softmax响应，通过分析模型的概率输出来实现选择性预测。我们观察到，post-hoc方法隐式地生成了一种称为“选择性先验”（selective prior）的不确定性信息，传统上仅在推理阶段使用。我们认为，由选择机制提供的选择性先验在训练阶段同样至关重要。因此，我们提出了SYNC loss，这是一种新颖的ad-hoc与post-hoc方法的融合方式。具体而言，我们的方法将softmax响应引入SelectiveNet的训练过程中，通过考察选择性先验来增强其选择性预测能力。在包括CIFAR-100、ImageNet-100和Stanford Cars在内的多个数据集上的评估表明，我们的方法不仅增强了模型的泛化能力，还在选择性预测性能上超越了先前的工作，设立了新的最先进性能基准。
...
TL;DR: 本文提出一种将基于Vision Transformer的深度估计先验融入VINS-Mono后端的新框架，在保持边缘设备计算限制的同时，通过仿射不变深度一致性和序数约束显著提升单目视觉惯性里程计在低纹理环境下的精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统单目视觉惯性里程计（VIO）在低纹理环境中因缺乏足够稀疏视觉特征而难以准确估计位姿；虽然基于ViT的密集深度估计模型能提供几何一致的深度信息，但其高计算开销阻碍了在边缘设备上的实时部署。
...
Conclusion: 所提方法在严格遵守边缘设备计算限制的前提下，成功融合了先进深度估计模型的优势，显著提升了单目VIO在低纹理环境中的性能与尺度恢复能力。
...
Abstract (中文翻译): 传统的单目视觉惯性里程计（VIO）系统在低纹理环境中表现不佳，因为稀疏的视觉特征不足以实现精确的姿态估计。为解决这一问题，密集单目深度估计（MDE）被广泛探索作为补充信息源。尽管近期基于视觉Transformer（ViT）的复杂基础模型能够提供密集且几何一致的深度信息，但其计算需求通常使其无法在边缘设备上实现实时部署。我们的工作通过将学习到的深度先验直接集成到VINS-Mono优化后端中，弥合了这一差距。我们提出了一种新颖的框架，该框架施加仿射不变的深度一致性约束和成对序数约束，并通过基于方差的门控机制显式滤除不稳定的伪影。该方法在严格遵守边缘设备计算限制的同时，稳健地恢复了度量尺度。在TartanGround和M3ED数据集上的大量实验表明，我们的方法在具有挑战性的场景中有效防止了发散，并显著提升了精度，绝对轨迹误差（ATE）最多降低了28.3%。代码将公开发布。
...
TL;DR: 本文提出了一个面向流媒体视频的超分辨率新数据集StreamSR，并发布了高效实时模型EfRLFN，显著提升了压缩视频超分辨率的性能与效率。


<details>
  <summary>Details</summary>
Motivation: 现有实时超分辨率方法在处理压缩视频时表现不佳，且常用数据集无法反映真实流媒体视频的特性，导致基准测试缺乏现实意义。

Method: 作者构建了来自YouTube的StreamSR数据集，涵盖多种视频类型和分辨率；提出EfRLFN模型，融合高效通道注意力机制与双曲正切激活函数，并设计复合损失函数以优化训练；同时对11种SOTA模型进行基准测试，并对其他模型在新数据集上进行微调。

Result: EfRLFN在视觉质量和运行效率方面优于现有方法；在StreamSR上微调的模型在多个标准基准上均取得显著且泛化良好的性能提升。

Conclusion: 针对真实流媒体场景构建的数据集和专用模型能有效提升超分辨率效果，所提方法为实际应用提供了有价值的解决方案和资源。
...
Abstract (中文翻译): 近期实时超分辨率技术的进步推动了更高质量的视频流媒体传输，但现有方法在处理压缩视频内容时仍面临独特挑战。当前常用的数据集无法准确反映流媒体视频的特性，限制了现有基准测试的实际相关性。为弥补这一空白，我们提出了一个全面的新数据集——StreamSR，该数据集源自YouTube，涵盖了代表真实世界流媒体场景的多种视频类型和分辨率。我们对11种最先进的实时超分辨率模型进行了基准测试，以评估它们在流媒体应用场景中的表现。此外，我们提出了EfRLFN，一种高效的实时超分辨率模型，它整合了高效通道注意力机制和双曲正切激活函数——这是实时超分辨率领域的一项新颖设计。我们对网络架构进行了充分优化以最大化效率，并设计了一种复合损失函数以改善训练收敛性。EfRLFN结合了现有架构的优点，在视觉质量和运行时性能方面均有提升。最后，我们证明了在我们的数据集上对其他模型进行微调，能够带来显著的性能增益，并且这些增益在多个标准基准上具有良好的泛化能力。我们已在https://github.com/EvgeneyBogatyrev/EfRLFN公开了数据集、代码和基准测试结果。
