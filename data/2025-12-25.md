...
TL;DR: 本文提出了首个大规模视觉-语言模型（VLM）注视理解基准VL4Gaze，包含48.9万条自动生成的问答对，涵盖四种互补任务。实验表明，现有VLM在无特定监督下难以有效理解注视信息，而基于VL4Gaze的训练可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型（VLM）虽在多种视觉任务中表现优异，但缺乏系统评估或训练其注视理解能力的基准，尚不清楚通用视觉-语言预训练能否自然习得注视理解能力。

Method: 构建了名为VL4Gaze的大规模基准数据集，包含12.4万张图像和48.9万个自动生成的问答对，并将注视理解建模为统一的视觉问答（VQA）问题，涵盖四个互补任务：注视物体描述、注视方向描述、注视点定位和模糊问题识别。在上下文学习和微调设置下对多个商业和开源VLM进行了全面评估。

Result: 实验结果显示，即使大规模VLM在没有任务特定监督的情况下也难以可靠地推断注视语义和空间定位；而使用VL4Gaze进行训练则在所有任务上均带来显著且一致的性能提升。

Conclusion: 针对注视理解的多任务监督对提升VLM的注视理解能力至关重要，作者将公开发布VL4Gaze数据集和代码以促进该领域的进一步研究。
...
Abstract (中文翻译): 人类注视为理解视觉场景中的注意力、意图和社会互动提供了关键线索，然而当前的视觉-语言模型（VLM）在注视理解方面仍鲜有探索。尽管近期的VLM在多种视觉任务中展现出强大的场景级推理能力，但尚无系统性评估或训练其注视理解能力的基准，因此尚不清楚注视理解是否能从通用视觉-语言预训练中自然涌现。为填补这一空白，我们提出了VL4Gaze——首个大规模基准，旨在探究、评估并释放VLM在注视理解方面的潜力。VL4Gaze包含12.4万张图像上的48.9万个自动生成的问答对，并通过四项互补任务将注视理解形式化为统一的视觉问答（VQA）问题：(1) 注视物体描述，(2) 注视方向描述，(3) 注视点定位，以及(4) 模糊问题识别。我们在上下文学习和微调设置下对商业和开源VLM进行了全面评估。结果表明，即使大规模VLM在缺乏任务特定监督的情况下也难以可靠地推断注视语义和空间定位；相比之下，在VL4Gaze上进行训练可在所有任务上带来显著且一致的性能提升，凸显了针对性多任务监督对于发展VLM注视理解能力的重要性。我们将发布该数据集和代码以支持该方向的进一步研究与开发。
...
TL;DR: 本文提出一种面向TinyML的硬件感知神经架构搜索框架，用于在TACO数据集上进行垃圾检测。该方法构建ResDets超网并通过迭代进化搜索优化骨干和检测头，生成一系列名为TrashDets的轻量级检测器，在精度、参数量和能效方面显著优于现有TinyML检测器。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的边缘设备和IoT设备上部署高性能垃圾检测模型面临严峻挑战，现有TinyML检测器在精度、参数量和能效方面存在不足，亟需一种兼顾硬件约束与检测性能的高效神经架构搜索方法。

Method: 提出一种迭代式硬件感知神经架构搜索框架：构建Once-for-All风格的ResDets超网，通过交替优化骨干网络与neck/head的进化搜索策略，并引入种群传递机制和精度预测器以降低搜索成本并提升稳定性。

Result: 在TACO五类子集上，最强变体TrashDet-l达到19.5 mAP50，参数量30.5M，比先前模型最高提升3.6 mAP50且参数更少；TrashDet系列覆盖1.2M–30.5M参数范围，mAP50为11.4–19.5。在MAX78002微控制器上，TrashDet-ResNet单次推理能耗7525 μJ、延迟26.7 ms、37.45 FPS；TrashDet-MBNet mAP50提升10.2%；两者相较基线最多降低88%能耗、78%延迟和53%平均功耗。

Conclusion: 所提框架成功生成了适用于不同TinyML部署预算的可扩展垃圾检测器家族TrashDets，在精度、参数效率和硬件能效方面全面超越现有方法，为资源受限设备上的实时垃圾检测提供了实用解决方案。
...
Abstract (中文翻译): 本文针对TACO数据集上的垃圾检测任务，在严格的TinyML约束下，提出了一种面向边缘和物联网设备的迭代式硬件感知神经架构搜索框架。该方法构建了一个Once-for-All风格的ResDets超网，并采用交替优化骨干网络与neck/head的迭代进化搜索策略，辅以种群传递机制和精度预测器，以降低搜索成本并提升稳定性。该框架生成了一系列即用型检测器，称为TrashDets。在包含纸张、塑料、瓶子、易拉罐和烟头五个类别的TACO子集上，性能最强的变体TrashDet-l实现了19.5的mAP50，参数量为30.5M，相比先前检测器最高提升了3.6 mAP50，同时使用了更少的参数。TrashDet系列模型参数量覆盖1.2M至30.5M，mAP50介于11.4至19.5之间，为不同TinyML部署预算的资源受限硬件提供了可扩展的检测器选项。在MAX78002微控制器上使用TrashNet数据集时，两个专用变体TrashDet-ResNet和TrashDet-MBNet共同优于ai87-fpndetector基线：其中TrashDet-ResNet单次推理能耗为7525微焦，延迟26.7毫秒，帧率达37.45 FPS；TrashDet-MBNet则将mAP50提升了10.2%。与现有TinyML检测器相比，二者最多可降低88%的能耗、78%的延迟和53%的平均功耗。
...
TL;DR: 本文提出了OccuFly，首个基于相机的真实世界空中语义场景补全（SSC）基准数据集，并设计了一种无需LiDAR的标签生成框架，利用传统3D重建技术将2D掩码自动提升为3D点云标签，显著减少人工标注成本。该数据集涵盖多种季节、高度和场景类型，为高空视角下的3D感知研究提供了重要资源。


<details>
  <summary>Details</summary>
Motivation: 现有语义场景补全（SSC）研究主要集中在地面场景（如自动驾驶），而空中场景（如无人机飞行）尚未得到充分探索；同时，主流依赖LiDAR传感器的方法受限于无人机的载重、能耗及法规限制，且高空视角下LiDAR点云稀疏，难以适用。

Method: 提出OccuFly数据集，包含在50m、40m、30m高度采集的春、夏、秋、冬四季图像，覆盖城市、工业和乡村场景，提供22类语义标签；并开发一种基于纯相机模态的LiDAR-free数据生成框架，通过传统3D重建方法将部分标注的2D掩码自动映射到重建点云中，实现高效3D标注。

Result: 成功构建了首个真实世界、基于相机的空中SSC基准OccuFly，并在该数据集上对当前先进方法进行了评测，揭示了高空视角下SSC任务所面临的独特挑战。

Conclusion: OccuFly填补了空中语义场景补全领域的空白，其LiDAR-free的数据生成策略为资源受限的无人机平台提供了可行方案，推动了面向空中三维场景理解的整体感知研究。
...
Abstract (中文翻译): 语义场景补全（Semantic Scene Completion, SSC）对于移动机器人中的三维感知至关重要，因为它能够通过联合估计密集体素占用状态和每个体素的语义信息，实现对场景的整体理解。尽管SSC在自动驾驶等地面领域已得到广泛研究，但在自主飞行等空中场景中仍鲜有探索，从而限制了下游应用的发展。此外，LiDAR传感器是目前SSC数据生成的主要模态，但由于飞行法规、质量和能耗限制，以及从高空视角获取的LiDAR点云稀疏性等问题，大多数无人飞行器（UAV）难以使用LiDAR。为解决这些局限性，我们提出了OccuFly——首个基于相机的真实世界空中SSC基准数据集，该数据集在春季、夏季、秋季和冬季分别于50米、40米和30米的高度采集，涵盖城市、工业和乡村场景，提供22个语义类别，且数据格式遵循现有规范，便于与已有研究无缝集成。尤为重要的是，我们提出了一种基于相机模态的无LiDAR数据生成框架，该模态在现代无人机上普遍存在。通过利用传统的三维重建技术，我们的框架可将部分标注的二维掩码自动提升至重建的点云中，从而大幅减少人工三维标注的工作量。最后，我们在OccuFly上对当前最先进的方法进行了基准测试，突出了高空视角所带来的特有挑战，为整体空中三维场景理解提供了一个全面的视觉基准。
...
Method: 提出 NullBUS 框架，引入“可空提示”机制——使用可学习的空嵌入和存在掩码，在元数据缺失时回退到仅图像证据，而在元数据存在时利用文本提示，实现单一模型对有/无提示图像的联合学习。

Result: 在三个公开 BUS 数据集的统一测试池中，NullBUS 达到平均 IoU 0.8568 和平均 Dice 0.9103，展示了在混合提示可用条件下的最先进性能。

Conclusion: NullBUS 有效解决了 BUS 分割中元数据缺失的问题，提升了模型在真实场景中的适用性和鲁棒性，为多模态医学图像分割提供了新思路。
...
Abstract (中文翻译): 乳腺超声（BUS）分割可提供病灶边界，对计算机辅助诊断和治疗规划至关重要。尽管可提示方法在有文本或空间提示可用时能提升分割性能和肿瘤勾画效果，但许多公开的 BUS 数据集缺乏可靠的元数据或报告，导致训练只能局限于小规模多模态子集，从而降低了模型的鲁棒性。我们提出了 NullBUS，一种多模态混合监督框架，可在单一模型中同时从有提示和无提示的图像中学习。为处理缺失的文本信息，我们引入了“可空提示”机制，通过可学习的空嵌入和存在掩码实现：当元数据缺失时回退到仅依赖图像证据，当元数据存在时则利用文本提示。在三个公开 BUS 数据集构成的统一测试池上评估，NullBUS 实现了平均 IoU 0.8568 和平均 Dice 0.9103，展示了在混合提示可用条件下最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统自动驾驶感知流程将相机设计与下游任务解耦，采用固定光学系统和人工设计的图像信号处理（ISP）流程，优先生成适合人类观看的图像而非机器可理解的语义信息。这种分离在去马赛克、去噪或量化过程中丢失关键信息，并迫使模型适应传感器引入的伪影。

Method: 提出一个端到端的RAW-to-task协同设计框架，整合真实手机级镜头模型、可学习彩色滤光阵列（CFA）、泊松-高斯噪声过程和量化模块，并直接针对语义分割目标进行联合优化。

Result: 在KITTI-360数据集上，该方法相比固定流水线持续提升mIoU，其中光学建模和CFA学习带来最大增益，尤其对细长或低光照敏感类别效果显著；模型仅含约100万参数，运行速度达28 FPS，具备边缘部署能力。

Conclusion: 全栈协同优化（光学+传感器+网络）是实现高效、可靠且可部署的自动驾驶感知系统的有效路径；协同设计的传感器能根据语义结构自适应调整图像采集，在模糊、噪声和低位深条件下仍保持边界清晰与精度。
...
Abstract (中文翻译): 传统的自动驾驶感知流程将相机设计与下游感知任务解耦，依赖固定的光学系统和手工设计的图像信号处理器（ISP），其目标是生成适合人类观看的图像，而非面向机器的语义信息。这种分离方式在去马赛克、去噪或量化过程中会丢弃信息，并迫使模型去适应传感器引入的伪影。本文提出一种任务驱动的协同设计框架，将光学系统、传感器建模与轻量级语义分割网络统一为一个端到端的RAW-to-task流水线。该系统基于DeepLens[19]，整合了真实的手机级镜头模型、可学习的彩色滤光阵列（CFA）、泊松-高斯噪声过程以及量化模块，并直接针对分割任务目标进行联合优化。在KITTI-360数据集上的评估表明，该方法相比固定流水线始终取得更高的mIoU，其中光学建模和CFA学习带来了最大性能提升，尤其对细长物体或对低光照敏感的类别效果显著。重要的是，这些鲁棒性提升是在一个紧凑的约100万参数模型上实现的，运行速度约为28 FPS，展示了其在边缘设备上的部署潜力。视觉与定量分析进一步表明，协同设计的传感器能够根据语义结构自适应地调整图像采集策略，在模糊、噪声和低位深条件下仍能锐化边界并保持精度。这些发现共同确立了光学、传感器与网络的全栈协同优化，是迈向高效、可靠且可部署的自动驾驶感知系统的有效路径。
...
TL;DR: 本文提出了CHAMMI-75数据集，包含来自75项不同生物研究的多通道显微图像，用于训练能适应不同成像通道和条件的细胞形态学模型，从而提升跨研究的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有细胞形态学量化模型通常仅针对单一显微成像类型训练，难以在不同生物研究间复用，因技术规格（如通道数）或实验条件差异导致分布外问题。

Method: 构建并利用名为CHAMMI-75的开放数据集，该数据集整合了75项不同生物研究的异构多通道显微图像，用于训练具有通道自适应能力的细胞形态学模型。

Result: 实验表明，使用CHAMMI-75训练的模型在多通道生物成像任务中性能更优，主要得益于其在显微成像模态上的高度多样性。

Conclusion: CHAMMI-75为开发新一代适用于各类生物研究的通用细胞形态学模型奠定了基础。
...
Abstract (中文翻译): 利用图像和机器学习对细胞形态进行量化已被证明是研究细胞对处理响应的强大工具。然而，用于量化细胞形态的模型通常仅使用单一类型的显微成像进行训练，导致这些模型专业化，无法在不同生物研究之间复用，原因在于技术规格不匹配（例如通道数量不同）或目标实验条件超出分布范围。在此，我们提出了CHAMMI-75——一个开放获取的数据集，包含来自75项多样化生物研究的异构多通道显微图像。我们从公开资源中整理了这一数据集，旨在研究具备通道自适应能力、可处理任意显微图像类型的细胞形态模型。我们的实验表明，使用CHAMMI-75进行训练能够提升多通道生物成像任务的性能，这主要归功于其在显微成像模态方面的高度多样性。本研究为创建适用于生物研究的新一代细胞形态模型铺平了道路。
...
TL;DR: 本文提出一种自适应视觉预处理方法，通过动态调整输入分辨率和空间覆盖范围，在不修改模型架构或重新训练的前提下，显著提升视觉语言模型（VLM）的推理效率。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（VLM）在高分辨率图像上推理时存在高延迟和高计算成本的问题。尽管已有如FastVLM等高效架构，但其仍采用静态视觉预处理策略，对视觉内容简单的图像造成冗余计算。

Method: 提出一种自适应视觉预处理方法，结合内容感知图像分析、自适应分辨率选择和内容感知裁剪，在送入视觉编码器前减少视觉冗余，并与FastVLM无缝集成，无需修改模型结构或重新训练。

Result: 在DocVQA子集上的实验表明，该方法将每张图像的推理时间减少超过50%，平均生成时间降低，并使视觉token数量持续减少55%以上。

Conclusion: 内容感知的自适应预处理是一种轻量且有效的策略，可显著提升VLM在部署场景下的效率。
...
Abstract (中文翻译): 视觉语言模型（VLM）在多模态推理任务中表现出色，但由于高推理延迟和计算开销，尤其是在处理高分辨率视觉输入时，其部署仍然具有挑战性。尽管FastVLM等近期架构通过优化视觉编码器提升了效率，但现有流程仍依赖静态视觉预处理，导致对视觉内容简单的输入产生冗余计算。本文提出一种自适应视觉预处理方法，根据图像内容特征动态调整输入分辨率和空间覆盖范围。该方法结合内容感知图像分析、自适应分辨率选择和内容感知裁剪，在视觉编码前减少视觉冗余。重要的是，该方法无需修改FastVLM架构或重新训练即可集成。我们在DocVQA数据集的一个子集上以仅推理方式评估了所提方法，重点关注效率导向的指标。实验结果表明，自适应预处理相比基线流程，将每张图像的推理时间减少超过50%，降低了平均完整生成时间，并使视觉token数量持续减少55%以上。这些结果表明，输入感知的预处理是一种有效且轻量的策略，可提升视觉语言模型在部署场景中的效率。为便于复现，我们的实现以FastVLM仓库的分支形式提供，包含所提方法的相关文件，地址为：https://github.com/kmdavidds/mlfastlm。
...
TL;DR: ALIVE 是一个本地运行的交互式讲座视频引擎，结合神经虚拟人、内容感知检索与多模态交互，将传统被动观看转变为实时互动学习体验。


<details>
  <summary>Details</summary>
Motivation: 传统讲座视频缺乏实时答疑机制，而现有基于大语言模型和虚拟人的交互系统往往缺乏对讲座内容的理解、依赖云端服务，或未能在保护隐私的前提下整合检索与虚拟人讲解。

Method: ALIVE 在本地硬件上运行，整合三项核心技术：(1) 基于 ASR 转录、LLM 优化和神经说话头合成的虚拟人讲解；(2) 结合语义相似性和时间戳对齐的内容感知检索机制；(3) 支持文本/语音提问并以文本或虚拟人形式返回答案的实时多模态交互。系统采用轻量嵌入模型、FAISS 检索和分段预加载策略保障响应速度。

Result: 在完整医学影像课程上验证了 ALIVE 的检索准确率、延迟表现和用户体验，结果表明其能提供准确、内容相关且引人入胜的实时学习支持。

Conclusion: ALIVE 展示了多模态 AI 结合内容感知检索与本地部署，可显著提升录播课程的教学价值，为下一代交互式学习环境提供了可扩展路径。
...
Abstract (中文翻译): 传统的讲座视频虽然灵活，但缺乏实时澄清机制，导致学习者在遇到困惑时不得不借助外部搜索。尽管大型语言模型和神经虚拟人技术的最新进展为交互式学习带来了新机遇，但现有系统通常缺乏对讲座内容的理解，依赖云服务，或未能在一个统一且保护隐私的流程中整合信息检索与虚拟人讲解。

本文提出了 ALIVE（Avatar-Lecture Interactive Video Engine），它将被动的讲座观看转变为动态、实时的学习体验。ALIVE 完全在本地硬件上运行，整合了三大核心组件：(1) 通过自动语音识别（ASR）转录、大语言模型（LLM）优化和神经说话头合成生成的虚拟人讲解；(2) 一种内容感知的检索机制，结合语义相似性与时间戳对齐，以定位上下文相关的讲座片段；(3) 实时多模态交互功能，允许学生暂停讲座并通过文本或语音提问，并获得基于讲座内容的文本或虚拟人形式的回答。

为保证响应速度，ALIVE 采用了轻量级嵌入模型、基于 FAISS 的检索系统，以及分段虚拟人合成与渐进式预加载策略。我们在一门完整的医学影像课程上展示了该系统，并评估了其检索准确率、延迟特性和用户体验，结果表明 ALIVE 能够提供准确、内容相关且引人入胜的实时学习支持。

ALIVE 表明，当多模态人工智能与内容感知检索及本地部署相结合时，可以显著提升录播讲座的教学价值，为构建下一代交互式学习环境提供了可扩展的路径。
...
Method: 1）建立基于B/C/D-Scan三视图联合分析的三维管线特征评估方法；2）提出DCO-YOLO框架，集成DySample、CGLU和OutlookAttention机制以增强小目标边缘特征提取；3）设计3D-DIoU空间特征匹配算法，结合三维几何约束与中心距离惩罚项，实现多视图标注自动关联。

Result: 在真实城市地下管线数据实验中，所提方法在复杂多管线场景下达到96.2%准确率、93.3%召回率和96.7%mAP，分别比基线模型高2.0%、2.1%和0.9%；消融实验和Grad-CAM++可视化验证了模块有效性及对管线几何特征的关注能力提升。

Conclusion: 该研究将深度学习优化策略与3D GPR物理特性相结合，为地下管线智能识别与定位提供了一种高效可靠的新技术框架。
...
Abstract (中文翻译): 为解决利用三维探地雷达（3D GPR）进行地下管线检测时存在的多视图特征相关性弱、小尺度目标识别精度低以及复杂场景下鲁棒性不足等问题，本文提出了一种三维管线智能检测框架。首先，基于B/C/D-Scan三视图联合分析策略，通过FDTD方法获得的正演模拟结果与实际测量数据交叉验证，建立了三维管线三视图特征评估方法。其次，提出了DCO-YOLO框架，将DySample、CGLU和OutlookAttention等跨维度关联机制集成到原始YOLOv11算法中，显著提升了对小尺度管线边缘特征的提取能力。此外，还提出了一种3D-DIoU空间特征匹配算法，该算法融合三维几何约束与中心距离惩罚项，实现了多视图标注的自动关联，三视图融合策略有效解决了单视图检测中的固有模糊性。基于真实城市地下管线数据的实验表明，所提方法在复杂多管线场景下分别达到了96.2%的准确率、93.3%的召回率和96.7%的平均精度均值（mAP），较基线模型分别提高了2.0%、2.1%和0.9%。消融实验证实了动态特征增强模块的协同优化效果，Grad-CAM++热力图可视化也表明改进后的模型显著增强了对管线几何特征的关注能力。本研究将深度学习优化策略与3D GPR的物理特性相结合，为地下管线的智能识别与定位提供了一种高效可靠的新技术框架。
...
TL;DR: NeRV360 是一种面向 360 度视频的高效神经视频表示方法，通过仅解码用户视口区域，大幅降低内存消耗并提升解码速度。
...
Method: 提出 NeRV360 框架，在解码阶段直接提取用户选定视口而非重建整个全景帧，并引入时空仿射变换模块，根据视角和时间进行条件解码。

Result: 在 6K 分辨率视频上，相比 HNeRV，NeRV360 内存消耗减少 7 倍，解码速度提升 2.5 倍，且客观画质指标更优。

Conclusion: NeRV360 有效解决了高分辨率 360 度视频中 NeRV 的效率瓶颈，为实时神经视频压缩提供了可行方案。
...
Abstract (中文翻译): 面向视频的隐式神经表示（NeRV）在视频压缩方面展现出强大潜力。然而，将 NeRV 应用于高分辨率 360 度视频会导致内存占用过高和解码速度缓慢，使得实时应用变得不切实际。我们提出了 NeRV360，这是一种端到端框架，仅解码用户选定的视口区域，而不是重建整个全景帧。与传统流程不同，NeRV360 将视口提取集成到解码过程中，并引入了一个时空仿射变换模块，以根据视角和时间进行条件解码。在 6K 分辨率视频上的实验表明，与代表性先前工作 HNeRV 相比，NeRV360 的内存消耗减少了 7 倍，解码速度提升了 2.5 倍，同时在客观指标上实现了更优的图像质量。
