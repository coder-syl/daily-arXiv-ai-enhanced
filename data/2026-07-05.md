...
TL;DR: 本文提出AnchorSplat，一种端到端、无需原始图像的3D高斯泼溅（3DGS）资产增强方法，通过点锚机制和单次乘法操作实现高效、几何一致的细节恢复，在新构建的3DGS-SR基准上达到SOTA性能，且推理速度比优化方法快10^5倍。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅资产常存在细节缺失和纹理噪声等问题，而基于2D图像处理的增强方法会引入多视角不一致性和高昂计算开销；因此需要一种直接在3D空间操作、无需原始多视图图像且高效的3D原生细化方案。

Method: 提出AnchorSplat，一个端到端的深度网络，直接在3D结构上操作。其核心是点锚机制（Point Anchor Mechanism），通过局部偏移约束保证几何一致性，并用单次乘法机制替代传统的迭代稠密化过程，且完全不需要原始多视图图像（source-free）。

Result: 在新构建的大规模基准数据集3DGS-SR上取得SOTA结果，推理吞吐量比传统优化方法快高达10^5倍，并在生成模型输出和真实扫描等多种数据分布上展现出强大的零样本泛化能力。

Conclusion: AnchorSplat为3D高斯泼溅资产提供了一种高效、鲁棒且无需原始图像的3D原生增强范式，显著提升了渲染质量和处理效率，推动了该领域的实用化发展。
...
Abstract (中文翻译): 3D高斯泼溅（3DGS）已成为一种用于高保真渲染的强大表示方法。然而，现有的资产常常受到质量瓶颈的限制，例如细节缺失和纹理噪声。以往通过2D图像处理来增强这些资产的方法会引入多视角不一致性并带来高昂的计算成本。本文提出了一种新颖的3D原生细化范式——AnchorSplat。AnchorSplat是一个端到端的深度网络，直接在3D结构上进行操作，避免了传统3D-2D-3D流程中昂贵的优化开销。关键的是，AnchorSplat是一种严格的无源（source-free）解决方案，无需原始的多视图图像。该方法的核心是点锚机制（Point Anchor Mechanism），通过局部偏移约束来强制几何一致性，从而缓解不适定映射和梯度混淆问题。此外，AnchorSplat用单次乘法机制取代了迭代稠密化过程。为了促进相关研究，我们构建了首个面向此任务的大规模基准数据集3DGS-SR。实验表明，AnchorSplat在3DGS-SR数据集上取得了最先进的结果，其吞吐量比优化方法最高快达10^5倍。值得注意的是，AnchorSplat在包括生成模型输出和真实世界扫描在内的多种数据分布上均展现出强大的零样本泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有PAD模型在跨域场景下泛化能力有限，主要因为传统基于类别标签优化的提示学习无法对齐细粒度的攻击相关视觉语义，导致模型过度拟合域特有伪影而非可迁移的攻击线索。

Method: 提出CPG-PAD框架，包含两个核心组件：1）视觉概念驱动增强（VCE）模块，利用可解释AI（XAI）技术自动发现与PAD相关的视觉概念并生成对应热力图；2）基于提示的概念注入（PCI）机制，通过视觉-提示解码器（VPD）和概念映射损失，将这些概念融入提示空间，使提示与模型内部概念对齐。
...
Abstract (中文翻译): 呈现攻击检测（PAD）是保护人脸识别系统免受打印照片、重放视频和3D面具等呈现攻击的关键防线。尽管已有显著进展，现有PAD模型在面对传感器、光照和攻击材料变化时，仍难以泛化到未见过的域。近期的视觉-语言模型（VLMs）展现出强大的泛化能力，但其在PAD中的应用仍受限，因为通常在类别标签监督下优化的学习提示未能显式地与细粒度的攻击相关视觉语义对齐，导致所学表征往往过拟合域特定伪影，而非捕获可迁移的攻击线索。为解决此问题，我们提出了概念引导提示的呈现攻击检测（CPG-PAD）框架，在提示学习过程中引入模型级概念引导。具体而言，我们设计了一个视觉概念驱动增强（VCE）模块，利用可解释人工智能（XAI）技术自动发现与PAD相关的视觉概念，并生成关联的概念热力图以提供局部细粒度指导。在这些热力图的引导下，基于提示的概念注入（PCI）机制通过视觉-提示解码器（VPD）和概念映射损失，将这些概念整合到提示空间中，使提示与模型内部的概念空间对齐。该设计使CPG-PAD能够捕获可泛化且域不变的攻击线索，同时有效抑制数据集特定的偏差。在九个基准数据集上的大量实验表明，CPG-PAD在多源、少源和单源设置下均持续实现了最先进的跨域性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉叙事生成模型虽能生成视觉连贯的图像序列，但可能丢失场景间关键的语义过渡信息；而当前评估基准忽视了这种“语义轨迹崩溃”的失败模式。

Method: 作者提出KathaTrace协议，通过文本、图像及图文结合三种条件评估场景过渡的可恢复性，并构建包含25K样本的KathaBench数据集；定义语义轨迹差距（STG）作为核心度量，并开发Semantic Compass探针用于生成后修复。

Result: 人类验证显示高一致性（Fleiss' kappa = 0.845）；在主流生成模型上测得平均STG为23.5 ± 1.3，表明存在显著语义损失；Semantic Compass能有效提升故事板选择质量。

Conclusion: 仅靠视觉连贯不足以保证叙事完整性，需专门评估语义过渡；KathaTrace与STG为诊断和改进视觉叙事生成提供了新工具和方向。
...
Abstract (中文翻译): 视觉叙事是故事板、漫画、儿童媒体和电影预可视化的核心，观众仅凭图像即可理解故事。近期的生成模型（如StoryDiffusion）虽能生成视觉连贯的序列，但视觉连贯并不保证源故事中的场景过渡语义仍可被还原。现有基准主要评估视觉质量、内容忠实度和场景连贯性，却忽略了一种关键失败模式：即故事板中各场景看似视觉连贯，但场景间的语义联系却已消失。我们提出了KathaTrace——一种与生成器无关的协议，用于诊断“语义轨迹崩溃”（即理解一个场景如何接续另一个场景所需的过渡语义的丢失）。KathaTrace在三种证据条件下评估过渡：仅文本、仅图像、以及图文结合，并过滤掉模糊样本。我们贡献了KathaBench-25K数据集，包含来自《伊索寓言》《五卷书》和《故事海》等经典文集的5,000个叙事、20,000个过渡以及28,712个可恢复性问题。我们定义“语义轨迹差距”（Semantic Trajectory Gap, STG）为仅文本与仅图像条件下可恢复性的差值，用以衡量可视化过程中丢失的过渡语义。人工验证获得Fleiss' kappa = 0.845。在多个前沿生成模型上的实验显示出显著的STG值（23.5 ± 1.3）。此外，我们提出的“语义指南针”（Semantic Compass）作为一种可操作性探针，利用KathaTrace信号进行生成后修复，有效提升了故事板的选择效果。
...
TL;DR: 本文提出一种新颖的输入感知可扩展专家模块，通过在相似样本子集上训练专家，并结合输入感知专家选择机制与时空选择机制，提升视频行人重识别中对细粒度特征的挖掘能力，在两个大规模数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理外观相似的行人身份时难以有效区分，因此需要更强的细粒度特征提取能力。传统方法通常对整个数据集统一更新网络参数，无法专注于相似样本间的细微差异。

Method: 提出输入感知可扩展专家模块：1）输入感知专家选择机制，在相似样本子集上动态激活特定专家以学习细微差异；2）时空选择机制，增强专家对空间和时间维度细粒度差异的敏感性；3）设计可扩展方案，支持按需灵活添加新专家。

Result: 所提方法在两个大规模视频行人重识别数据集上取得了卓越的性能表现。

Conclusion: 通过在相似样本子集上训练专用专家并引入时空细粒度感知机制，能显著提升视频Re-ID的准确性，且模块具备良好的可扩展性。
...
Abstract (中文翻译): 基于视频的行人重识别（Re-ID）旨在从图库视频片段中检索出与查询视频片段中相同身份的目标。为解决该问题，挖掘细粒度特征至关重要，尤其是在区分外观相似的身份时。本文提出了一种新颖的输入感知可扩展专家模块，以增强模型探索细粒度信息的能力。不同于对数据集中每个样本统一更新网络参数的做法，我们旨在仅包含相似样本的特定子集上训练专家，从而提升其在这些相似样本中挖掘细粒度信息的能力。为实现这一目标，该模块融合了两种机制：输入感知专家选择机制和时空选择机制。前者针对相似样本子集动态激活一组专家，促使专家学习这些相似样本之间的细微差异；后者进一步增强专家对空间和时间维度上细粒度差异的敏感性，并允许专家根据不同输入样本动态利用这些差异。此外，为支持该专家模块，我们设计了一种可扩展方案，使其能够在必要时灵活地添加新专家。最终，我们的方法在两个大规模数据集上取得了出色的性能。
...
TL;DR: MapDreamer 是一种基于扩散模型的方法，能从单张航拍图像中自动生成具有显式拓扑结构的车道级矢量地图，显著减少人工标注需求。


<details>
  <summary>Details</summary>
Motivation: 高精地图对自动驾驶至关重要，但大规模构建仍依赖大量人工。现有方法难以高效生成具备精确几何与拓扑结构的车道级地图。

Method: MapDreamer 结合变分自编码器（VAE）学习车道中心线及其拓扑关系的紧凑潜在表示，并使用基于 Transformer 的潜在扩散模型预测图结构；通过交叉注意力机制在每一步去噪过程中注入密集航拍特征以对齐场景；引入车道数量模块和背景“幽灵”车道潜在变量应对不同场景车道数量变化；采用滑动窗口全局图聚合策略拼接局部瓦片以生成城市尺度且保持连通性的地图。

Result: 在 Argoverse 2 衍生的 UrbanLaneGraph 数据集上的实验表明，MapDreamer 在几何和拓扑保真度方面优于非生成式基线方法。

Conclusion: MapDreamer 能有效从单张航拍图像自动生成高精度、拓扑一致的车道级矢量地图，为高精地图构建提供了一种可扩展的新范式。
...
Abstract (中文翻译): 高精地图生成对自动驾驶至关重要，但其大规模构建仍是一个劳动密集型过程。我们提出了 MapDreamer，一种生成式扩散模型，能够直接从单张航拍图像中合成具有显式拓扑结构的车道级矢量地图。MapDreamer 利用变分自编码器学习车道中心线及其拓扑关系的紧凑潜在表示，并通过基于 Transformer 的潜在扩散模型预测图结构。为了使生成的地图与观测场景对齐，我们在每个去噪步骤中通过交叉注意力机制注入密集的航拍特征。为应对不同场景中车道数量的变化，我们提出了一种车道基数模块，并配以背景“幽灵”车道潜在变量——这是一种可学习的缓冲机制，可在扩散过程中防止槽位坍塌。此外，我们引入了一种滑动窗口全局图聚合策略，通过编码的车道边界将局部瓦片拼接成城市尺度的地图，同时保持连通性。在源自 Argoverse 2 的 UrbanLaneGraph 数据集上的实验表明，MapDreamer 在几何和拓扑保真度方面优于非生成式基线方法。
...
TL;DR: 本文提出MIBE框架，包含多主体交互基准（MIB）和评估器（MIE），用于解决多主体个性化图像生成中的身份遗漏、外观失真和交互错配问题，并提供更符合人类偏好的评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的多主体个性化图像生成模型难以准确渲染所有指定参考身份及其交互关系，且现有评估指标主要针对单主体场景，在多主体情况下无法可靠反映生成质量或与人类偏好对齐。

Method: 作者构建了MIB基准，包括一个60K对由视觉语言模型标注的Silver Set用于可扩展训练，以及一个4K对经双盲人工评估的Gold Set用于评测；并在此基础上训练轻量级评估器MIE，采用双头结构进行排序与诊断。

Result: MIE在Gold Set上展现出强大的跨生成器泛化能力，整体与人类偏好的成对准确率达0.922（已见生成器0.982，未见生成器0.884），显著优于CLIP、DINO等基线指标。

Conclusion: 通过引入诊断性监督信号，MIE有效保持了多主体场景下的排序区分度和与人类偏好的一致性，解决了传统评估方法在此类任务中失效的问题。
...
Abstract (中文翻译): 多主体个性化图像生成要求根据引导提示精确渲染所有请求的参考身份及其指定的交互关系。然而，当前最先进的模型在此过程中仍面临挑战，经常遗漏主体、无法保留参考外观，或错误分配交互关系。此外，现有指标主要针对单主体保真度设计，难以可靠捕捉这些错误，在主体数量增加时排名区分度严重下降，且无法与人类偏好保持一致。为填补这一空白，我们提出了多主体交互基准与评估器（MIBE），这是一个统一框架，包含多主体交互基准（MIB）和多主体交互评估器（MIE）。MIB通过解耦的数据机制系统覆盖多种关系类型和场景复杂度，包括一个用于可扩展指标训练的60K对视觉语言模型（VLM）标注的Silver Set，以及一个涵盖多种先进生成器的4K对双盲人工评估Gold Set；其中Silver Set在不同VLM间的偏好一致性达到95.1%。为展示该基准的实用性，我们提出了MIE——一种轻量级、基于参考条件的评估器，仅使用Silver Set进行训练，并采用双头结构实现排序与诊断目标。MIE在Gold Set上展现出强大的跨生成器泛化能力，与人类偏好的整体成对准确率达到0.922（已见生成器为0.982，未见生成器为0.884）。通过超越包括CLIP和DINO变体在内的广泛基线指标，MIE证明了诊断性监督能够在传统评估器失效的情况下，有效保持排名区分度并与人类偏好对齐。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪模型在面对目标形变、复杂干扰、环境变化或训练未见类别时表现不佳，其泛化与在线适应能力仍是瓶颈，难以媲美人类视觉的连续性与鲁棒性。

Method: 提出一系列方法，系统性地增强跟踪模型的目标判别能力、鲁棒自适应能力和几何推理能力。

Result: 所提方法有效提升了跟踪器在复杂动态场景中的鲁棒性和泛化性能，更接近人类视觉对场景变化的理解与适应能力。

Conclusion: 通过融合先验知识、空间几何与语义上下文，可显著提升通用目标跟踪系统的性能，使其更贴近人类视觉感知机制。
...
Abstract (中文翻译): 人类视觉感知的核心在于能够对外部世界保持连续且一致的理解。通过将观察结果与积累的经验相结合，人类视觉系统能够持续适应目标及其周围环境的变化，并在场景动态演变过程中保持稳健的视觉连续性。因此，人类视觉可以整合先验知识、空间几何和语义上下文，以理解复杂场景及其变化。作为计算机视觉中的核心问题，视觉目标跟踪旨在使机器感知更接近人类视觉感知。这一能力正是通用目标跟踪（Generic Object Tracking, GOT）任务的核心：在该任务中，视觉跟踪器仅在首帧中给定任意指定目标的边界框，随后需在动态变化的视觉流中持续定位该目标。然而，未来事件、观测结果以及现实世界的变化本质上是不可预测的，因此模型的泛化能力和在线适应能力仍是主要瓶颈。当目标发生严重形变、受到复杂干扰物影响、遭遇显著环境变化，或属于训练阶段未见过的类别时，跟踪可靠性往往会下降。本论文旨在通过提出一系列方法，系统性地增强跟踪模型的目标判别能力、鲁棒自适应能力和几何推理能力，从而缩小机器视觉跟踪系统与人类视觉感知之间的差距。
...
TL;DR: 本文提出利用计算机视觉（YOLOv10）自动检测树冠桥上褐吼猴的活动，通过引入辅助数据提升模型性能，以优化城市扩张背景下对树栖物种的保护监测。


<details>
  <summary>Details</summary>
Motivation: 城市扩张导致森林生境破碎化，威胁树栖物种生存；树冠桥虽可缓解此问题，但依赖相机陷阱监测会产生大量误报图像，耗费人力。因此需借助计算机视觉技术实现高效、自动的目标物种检测。

Method: 使用YOLOv10框架，结合不同比例的辅助数据对褐吼猴（Alouatta guariba）在树冠桥上的相机陷阱视频进行目标检测模型微调。

Result: 引入辅助数据可有效提升自动检测模型的性能，减少人工审核负担。

Conclusion: 该方法为监测树栖动物使用树冠桥提供了高效自动化工具，有助于减轻人类活动对其栖息地的干扰，支持生物多样性保护。
...
Abstract (中文翻译): 城市扩张威胁全球生物多样性，尤其因森林生境破碎化而影响树栖物种。树栖动物穿越不相连的森林斑块会增加死亡风险，从而危及其保护成效。在此背景下，架设树冠桥可能是一种可行策略；然而，为确保其有效性，必须持续监测树栖物种的使用情况，通常借助红外相机完成。但该方法常产生大量误报图像，耗费保护工作者大量时间进行人工审核。为此，计算机视觉算法可用于优化树冠桥上目标物种的自动检测。本研究探索了利用相机陷阱视频自动检测褐吼猴（Alouatta guariba）的方法。鉴于训练此类算法需要大量标注图像，我们测试了引入辅助数据以改进检测模型的效果，并使用不同比例的辅助数据对YOLOv10框架进行微调。这些自动检测技术的改进有助于保护工作，为监测减轻人类干扰对动物栖息地影响的措施提供了自动化工具。
...
TL;DR: G-CBM 是一种基于图的可解释模型，通过无监督方式发现概念并构建每张图像的概念图，利用图注意力网络建模概念间依赖关系，在多个医学和通用视觉数据集上优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有 Concept Bottleneck Models (CBMs) 存在依赖预定义概念词表或监督标注、缺乏显式概念定位、仅用单个图像级分数表示概念等问题，无法捕捉空间重复性和概念间依赖。

Method: 提出 G-CBM 框架：使用非负矩阵分解（NMF）进行无监督概念发现；将每个图像中的概念表示为图节点；通过区域特征匹配实现概念定位并记录空间复现；引入可调阈值 τ 过滤弱区域特征；采用图注意力网络（GAT）建模概念间非线性依赖。

Result: 在 ImageNet、HAM10000、PH2 和 Derm7pt 上平均 AUC 相对提升 3.7%；通过概念过滤机制，仅用少量概念即可达到高性能（如 PH2 上用 2/10 概念达 AUC 0.96）；在皮肤镜数据集上媲美需外部标注的监督方法；删除/插入实验验证了概念排序与模型预测的一致性。

Conclusion: G-CBM 提供了一种无需监督标注、具备显式概念定位和结构化推理能力的可解释模型，在性能和可解释性之间取得良好平衡。
...
Abstract (中文翻译): 概念瓶颈模型（CBM）提供了一种内在可解释的替代方案，避免了事后解释的局限性。然而，现有的 CBM 通常依赖于预定义的概念词汇表或监督标注，缺乏显式的概念定位，并且仅用单一的图像级分数来概括每个概念，忽略了概念在空间上的重复出现及其相互依赖关系。我们提出了一种基于图的概念瓶颈模型（G-CBM），该框架通过非负矩阵分解（NMF）进行无监督概念发现，并将所发现的概念表示为每张图像对应的概念图中的节点。G-CBM 将区域级特征与这些概念节点进行匹配，从而实现概念定位并捕捉概念在整个图像中的重复出现；同时引入一个可调节的概念过滤阈值 τ，以抑制较弱的区域级特征。随后，图注意力网络（GAT）通过建模节点间的非线性依赖关系来进行概念层面的推理。在 ImageNet、HAM10000、PH2 和 Derm7pt 数据集上，G-CBM 相较于 ResNet-50 基线模型平均实现了 3.7% 的相对 AUC 提升。概念过滤机制不仅经常提升预测性能，还能促使模型选择性地使用概念：例如在 PH2 上仅使用 10 个概念中的 2 个即达到 0.96 的 AUC，在 HAM10000 上使用 9 个概念中的 3.8 个达到 0.92 的 AUC。在皮肤镜基准测试中，G-CBM 的性能可与需要外部标注的监督方法相媲美。通过带有随机消融控制的删除/插入分析表明，所学习到的概念排序能够真实反映模型的预测结果。
...
TL;DR: 提出了一种基于空中签名的3D行为认证方法“Sign in the Air to Unlock”，利用点-体素交叉注意力网络（PV-Net）从3D轨迹中建模局部动态与全局结构，在两个数据集上验证了其有效性，展示了在沉浸式环境中实现无缝、自然且安全认证的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有身份认证方式（如密码、PIN码或依赖外部设备的登录）会破坏VR/AR等沉浸式体验；而现有的3D行为认证方法（如手势、眼动或脑电）常需专用传感器或限制自然动作，难以适用于动态沉浸环境。因此，亟需一种既安全又符合自然交互的新型认证接口。

Method: 设计了一种名为“Sign in the Air to Unlock”的空中签名认证接口，用户可在3D空间中自然书写签名；为识别该行为，提出了点-体素交叉注意力网络（PV-Net），联合建模3D轨迹中的局部运动动态和全局空间结构。

Result: 在公开数据集DeepAirSig（40名用户，1800个样本）上达到2.5%的等错误率（EER）；在新收集的沉浸式VR数据集ImmAirSig（22名用户，880个样本，使用Meta Quest 2采集）上达到76%的分类准确率。

Conclusion: 3D行为认证接口（如空中签名）能够在沉浸式环境中实现安全、直观且符合人体自然交互的用户认证，具有良好的应用前景。
...
Abstract (中文翻译): 沉浸式技术（如虚拟现实和增强现实，VR/AR）的显著进步及其在现代生活中的广泛应用，亟需一种安全、直观且兼容具身交互的身份认证界面。传统的密码、PIN码及基于设备的登录方式会破坏沉浸感，并依赖外部硬件。近期针对3D环境的行为认证方法（如手势识别、眼动追踪和脑电图EEG）虽具潜力，但通常需要专用传感器或限制用户的自然动作，从而在动态环境中可用性受限。本文提出“空中签名解锁”（Sign in the Air to Unlock）——一种空中签名认证界面，允许用户在3D空间中自然地书写其个性化且可重复的签名以完成身份验证。为实现该界面，我们设计了一种点-体素交叉注意力网络（PV-Net），可从3D轨迹中联合建模局部运动动态与全局空间结构。该模型在两个数据集上进行了评估：公开的DeepAirSig数据集（40名用户，共1800个签名）以及新采集的沉浸式VR数据集ImmAirSig（使用Meta Quest 2设备收集，22名用户，共880个样本）。PV-Net在DeepAirSig上达到了2.5%的等错误率（EER），在ImmAirSig上实现了76%的分类准确率。这些结果凸显了3D行为认证界面在沉浸式环境中融合安全性与自然交互、实现以用户为中心的无缝认证的巨大潜力。
