...
TL;DR: 本文提出了MV2数据集和基准，用于评估动态城市场景中大视角变化下的新视角合成（NVS）方法。该数据集包含汽车、滑板车和无人机同步采集的多轨迹图像，共50个高质量场景、12000张图像，并通过SfM与人工标注验证相机位姿。实验表明，现有NVS方法在视角差异增大时性能显著下降，且前馈位姿估计器明显弱于基于优化的方法。


<details>
  <summary>Details</summary>
Motivation: 现有新视角合成方法在真实驾驶场景中面临稀疏视角、动态物体和缺乏多轨迹数据等挑战，难以应对大视角变化，因此需要一个更具挑战性的数据集和基准来推动该领域发展。

Method: 作者构建了Multi-View Multi-Vehicle (MV2) 数据集，利用汽车、滑板车和无人机沿不同但同步的轨迹采集同步图像；使用Structure-from-Motion进行序列配准，并通过人工像素级对应标注验证相机位姿；在该数据集上对多种NVS和相机位姿估计方法进行基准测试。

Result: 在MV2上的实验显示，随着视角差异增大，NVS性能显著下降；前馈式位姿估计方法明显落后于基于优化的方法，说明MV2可作为评估驾驶场景下NVS能力的严格测试平台。

Conclusion: MV2数据集填补了动态城市场景中多视角、大视角变化下NVS研究的空白，为未来方法提供了可靠的评估基准，并揭示了当前方法在视角泛化和位姿估计方面的局限性。
...
Abstract (中文翻译): 可微渲染已推动新视角合成（NVS）的发展，但在真实驾驶场景中的应用仍面临诸多挑战，例如稀疏的采集视角、动态物体以及有限的多轨迹数据。为此，我们提出了多视角多车辆（MV2）数据集及基准，用于在动态城市场景中大视角变化条件下评估NVS模型。MV2包含来自汽车、滑板车和无人机的同步采集数据，各平台沿不同但同步的轨迹运行。通过在一个载具的相机流上训练NVS方法并在另一载具上测试，可在比现有单轨迹数据集大得多的视角变化下进行评估。所有序列均通过运动恢复结构（Structure-from-Motion）进行配准，并利用人工像素级对应标注验证相机位姿，最终得到包含12000张图像的50个高质量场景。对近期NVS和相机位姿估计方法的基准测试表明，随着视角差异增大，NVS性能显著下降，且前馈式位姿估计器明显落后于基于优化的方法，凸显了MV2作为驾驶场景下NVS严格测试平台的价值。该数据集、基准协议及相关项目资源已在 https://mv2-dataset.github.io/ 公开。
...
TL;DR: 本文提出HIMEC方法用于遥感图像变化描述生成（RSICC），通过方向性变化表示（DCR）和固定接口解码机制提升性能，在LEVIR-CC和SECOND-CC数据集上均优于基线。
...
Method: 提出HIMEC框架：1）使用方向性变化表示（DCR）将带符号的变化分解为“出现”、“消失”和“共享上下文”三个流后再融合；2）引入学习查询编码器，将融合特征转化为仅依赖样本的视觉条件变化查询token，作为场景解码器唯一的记忆；3）训练时使用辅助短语解码器提供监督；4）场景解码器在训练和推理时保持固定零输入接口。此外还评估了局部到全局级联策略，并研究不同条件输入对性能的影响。

Result: 在LEVIR-CC上，HIMEC取得CIDEr得分142.81±0.60，优于直接融合特征方法的139.51±3.40；在SECOND-CC上，固定零输入和匹配条件分别达到75.67和76.99 CIDEr，显著优于不匹配级联的60.77。实验还发现状态对应顺序对性能影响不大，但该结论仅限所评估级联结构。

Conclusion: HIMEC通过显式建模变化方向性和固定解码接口，有效提升了遥感图像变化描述生成的性能与一致性，验证了中间变化结构建模和接口一致性的重要性。
...
Abstract (中文翻译): 遥感图像变化描述生成（RSICC）旨在将双时相影像转换为描述语义变化的句子。大多数RSICC方法直接将融合后的视觉特征作为描述解码器的条件输入，较少关注中间变化结构以及解码器在训练与推理阶段接口的一致性。本文提出HIMEC方法，结合方向性变化表示（DCR）与固定接口解码机制。DCR在融合前将带符号的差异分离为面向“出现”、面向“消失”和共享上下文三个流。一个可学习查询编码器将融合后的表示转换为视觉条件化的“变化查询”token，这些token构成场景解码器唯一的样本依赖记忆。训练阶段额外引入一个辅助短语解码器，提供来自描述的监督信号。场景解码器在训练和推理时均以固定的零输入运行，从而保持接口一致。此外，我们还评估了一种局部到场景的级联结构：训练时以教师强制的局部状态为条件，推理时则使用自回归状态。在LEVIR-CC的变化验证对上，这些状态的平均余弦距离为0.69；采用匹配条件策略可恢复大部分性能损失，而打乱状态对应关系则未观察到明显性能下降——但该发现仅适用于所评估的级联结构。在三组随机种子的公平比较中，HIMEC在LEVIR-CC上达到CIDEr得分142.81±0.60，优于直接使用融合特征作为记忆的139.51±3.40；在SECOND-CC上，固定零输入和匹配条件策略分别达到75.67和76.99 CIDEr，远高于不匹配级联的60.77。代码将在发表后公开于https://github.com/ayshaashra/HIMEC。
...
TL;DR: 本文评估了三种开源视觉语言模型在从机器人第一人称视角判断近距离危险等级任务中的表现，发现未经微调的模型性能接近随机基线，微调后整体提升有限，但Qwen-VL配合高级提示在高危场景召回率上显著优于其他模型；同时发现正确分类并不依赖于对人物位置的准确感知，说明当前VLM在精细空间推理方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 为实现机器人在人类环境中的安全导航，需从其第一人称视角评估近距离（proxemic）危险，这要求模型兼具视觉与上下文推理能力。现有视觉语言模型在此类细粒度空间安全判断任务中的能力尚不明确，因此有必要系统评估其性能及改进策略。

Method: 作者在四类危险等级的机器人第一人称图像数据集上，评估InternVL、Qwen-VL和SmolVLM三种开源视觉语言模型。比较了三种提示策略和两轮QLoRA微调的效果，并以分层随机基线作为对照。

Result: 未微调时所有模型性能接近基线；微调后整体提升有限，但Qwen-VL结合高级提示在高危样本上的召回率显著更高。人物定位分析显示，正确危险分类并不对应更好的空间定位能力，表明模型可能未关注相关区域却仍输出正确标签。

Conclusion: 当前视觉语言模型在细粒度近距离推理和空间定位方面仍存在明显局限，但通过针对性提示设计和微调可在特定模型中提升高危场景的检测能力。
...
Abstract (中文翻译): 从机器人第一人称视角评估近距离危险对于在人类环境中实现安全的具身导航至关重要，这需要结合视觉与上下文推理能力。我们在将第一人称机器人图像分类为四个危险等级的任务中，评估了三种开源视觉语言模型（InternVL、Qwen-VL 和 SmolVLM），比较了三种提示策略以及两轮QLoRA微调相对于分层随机基线的效果。结果显示，未经微调时所有模型性能均接近基线水平，而微调仅带来有限的整体提升。然而，采用高级提示的Qwen-VL在高危案例上的召回率显著高于其他模型。进一步的人物定位分析表明，正确的危险分类并不对应更优的空间定位能力，说明模型可能在未关注场景相关区域的情况下仍生成有用的安全标签。这些结果表明，当前视觉语言模型在细粒度近距离推理和空间定位方面仍存在局限，但通过有针对性的提示工程和微调，可在特定模型中改善高危场景的检测效果。
...
TL;DR: 本文提出一种基于CT扫描的表面到骨骼标志点推断方法，在控制多种干扰因素的前提下，验证了从外部软组织几何结构推断内部骨骼标志点的可行性。
...
Method: 利用240例来自两家医院的临床CT扫描数据，构建一个锁定的回顾性协议，将CT导出的外部软组织点云与21个骨骼标志点及3个可见软组织标志点配对；采用集成的层次化点云模型进行预测，并通过患者错配对照和覆盖消融实验分析模型性能。

Result: 在40例留出患者上，模型对骨骼标志点的平均径向误差为2.97 mm，对深层或表面不可见标志点为3.03 mm；患者错配对照表明存在超出固定人群构型或全局相似性的个体特异性信号，覆盖消融显示模型依赖非前部几何信息。

Conclusion: 研究肯定地回答了在受控条件下从外部软组织推断隐藏骨骼标志点的可行性问题，为后续相关应用提供了基础。
...
Abstract (中文翻译): 现有的3D面部标志点方法仅定位可见皮肤上的点，但从外部软组织几何结构是否能够推断出CT定义的内部骨骼标志点仍不清楚。我们利用同次采集的CT导出表面，构建了一个坐标一致的表面到骨骼任务，将估计过程与光学到CT的配准、扫描仪域和采集状态效应分离，并单独分析覆盖范围的影响。基于来自两家医院的240例临床CT扫描，我们构建了一个锁定的回顾性协议，将CT导出的外部软组织点云与21个骨骼标志点及3个可见软组织标志点进行配对。一个集成的层次化点云模型在40例留出患者上对骨骼标志点实现了2.97 mm的平均径向误差，对深层或表面不可见标志点则为3.03 mm。患者错配对照支持存在超出固定人群构型或仅靠全局相似性所能解释的个体特异性信号，而覆盖范围的消融实验表明模型依赖于非前部几何信息。光学迁移诊断揭示了显著的覆盖范围相关成分和全局构型成分，尽管可部署的光学推断问题仍未解决。这些结果肯定地回答了在受控条件下的可行性问题，并为隐藏骨骼标志点的推断提供了基础。
...
TL;DR: 本文提出了StrAD，一个面向长视频的自动音频描述（AD）生成新基准，并将AD任务重新定义为流式密集视频描述。该方法无需真实时间戳即可在完整视频上插入AD，支持微调模型和视觉语言模型的零样本提示，在多个指标上达到SOTA或建立新基线。


<details>
  <summary>Details</summary>
Motivation: 现有自动AD生成方法依赖视频片段字幕，需真实时间戳和额外上下文信息，且当前基准仅涵盖短片段，无法满足对完整长视频（如电影、纪录片等）进行无障碍访问的实际需求。因此亟需一种适用于全视频、无需人工标注时间戳的AD生成框架。

Method: 作者将AD生成重构为流式密集视频描述任务，使用滑动窗口处理完整视频，在已有转录文本中插入AD，无需真实时间戳。提出两种实现方式：微调模型StrAD-FT和零样本提示模型StrAD-Zero，并构建了涵盖多种类型长视频的新基准StrAD。

Result: 在给定时间戳的片段级任务中，StrAD-FT在CMD-AD上以36.3 CIDEr刷新SOTA（+10.0），在StrAD上取得51.0 CIDEr，在MAD-Eval上达24.9 CIDEr。在全视频流式任务中，StrAD-FT的SODA得分为2.4，优于零样本基线StrAD-Zero的1.1，但两者在时序定位与叙事连贯性方面仍有不足。

Conclusion: 本文首次提出面向完整视频的流式AD生成方法，无需真实时间戳即可实时生成AD，推动了全视频AD生成的可衡量进展，为大规模提升视障人士的内容可访问性奠定基础。
...
Abstract (中文翻译): 视觉内容是主要的传播媒介，但若无音频描述（AD），盲人和低视力人群将难以获取其中信息。AD会在自然音频停顿期间叙述与上下文相关的视觉事件。然而，人工制作AD成本高昂，导致仅有少量内容被覆盖。目前大多数自动AD生成方法将任务视为视频片段字幕生成，依赖真实时间戳及角色数据库等额外上下文线索。现有基准也延续这一设定，仅包含配以自动或任务不匹配标注的短视频片段。我们提出了StrAD——一个面向完整长视频（涵盖电影、纪录片、短片、演出和电子游戏等多种类型）的AD生成新基准，并将AD生成任务重新定义为流式密集视频描述。我们的方法通过滑动窗口处理完整视频，在已有转录文本中插入AD，无需真实时间戳，并同时支持微调模型与视觉语言模型的零样本提示。在给定时间戳的片段级任务中，我们的微调模型StrAD-FT在CMD-AD上以36.3 CIDEr刷新SOTA（较Shot-by-shot提升10.0），在StrAD上建立51.0 CIDEr的参考基线，在MAD-Eval上也取得24.9 CIDEr的竞争力表现。在完整视频流式任务中，StrAD-FT的SODA得分为2.4，显著优于零样本基线StrAD-Zero的1.1，但两者在时序定位与叙事连贯性方面仍存在局限。此前工作多以离线、多阶段方式处理完整视频AD生成，而我们的方法是首个流式方案，可在无真实时间戳的情况下实时生成AD。StrAD使完整视频AD生成的进展变得可衡量，这是实现无障碍内容规模化扩展的前提。
...
TL;DR: 本文提出MM-slotgate方法，将Fashion-CLIP的图文嵌入分解为四个命名属性槽（类别、颜色、图案、人群），每个槽通过学习图文门控机制实现属性特异性控制，在H&M数据集上显著提升多属性约束检索性能，尤其在颜色属性上取得巨大提升。


<details>
  <summary>Details</summary>
Motivation: 现有时尚检索方法要么使用单一嵌入向量混合所有属性信号，难以在检索时进行属性级控制；要么使用离散语义ID，但这些ID通常作为整体项或残差地址优化，无法提供可独立操控的命名属性槽。

Method: 提出MM-slotgate多模态槽编码器，将Fashion-CLIP的文本和图像嵌入分解为四个命名属性槽，每个槽学习自己的图文门控机制，使视觉导向属性（如颜色、图案）更依赖图像证据，而分类导向属性（如类别、人群）更依赖文本信息。

Result: 在H&M数据集上，使用槽相似度和槽logit组合检索得分，MM-slotgate达到0.7566的macro ConstraintSatisfied@10，优于等权重多模态融合（0.7142）和纯文本fCLIP检索（0.4755）。颜色属性表现最佳，从0.321提升至0.889（+0.568），因其门控机制赋予图像证据57.4%权重。线性探针显示无额外信息泄露，量化槽码支持定向干预，颜色干预效果提升15.3倍。

Conclusion: 可控时尚检索应采用类型化、属性条件化的多模态槽机制，而非单一全局嵌入或不透明的项级语义ID。
...
Abstract (中文翻译): 时尚检索通常需要同时满足多个属性，例如类别、颜色、图案和人群特征。单体嵌入将这些信号混合到一个向量中，使得在检索时难以实现针对特定属性的控制。许多现有的语义ID方法提供了离散的商品编码，但这些编码通常被优化为商品级别或残差地址，并未暴露可独立控制的命名属性槽位。我们提出了MM-slotgate，一种多模态槽位编码器，它将Fashion-CLIP的文本和图像嵌入分解为四个命名的属性槽位。每个槽位学习其自身的文本-图像门控机制，从而使颜色和图案等视觉基础属性更多地依赖图像证据，而类别和人群等分类导向属性则更多地保持文本驱动。在H&M数据集上，通过结合槽位相似度和槽位logit的检索得分，MM-slotgate实现了0.7566的macro ConstraintSatisfied@10，优于等权重多模态融合（0.7142）和仅使用文本的fCLIP检索（0.4755）。最大的提升出现在颜色属性上，其得分从0.321提高到0.889（绝对提升+0.568），因为所学习的颜色门控赋予了图像证据57.4%的权重。所学习的门控机制无需模态监督即可解释：颜色偏向图像，类别偏向文本，而图案和人群则处于中间位置。所得槽位也保持可控性：线性探针显示超出标签相关性基线的额外信息泄露为零，且量化后的槽位编码支持定向干预，包括颜色属性上15.3倍的效果提升。这些结果表明，与单一全局嵌入或不透明的商品级语义ID相比，类型化、属性条件化的多模态槽位机制更有利于实现可控的时尚检索。
...
TL;DR: 提出一种基于教师-学生半监督学习框架，通过置信度感知的地图优化生成高质量伪标签，在标注数据稀缺的情况下显著提升在线高精地图构建性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中在线高精地图构建系统面临标注训练数据稀缺的问题，限制了模型在多样化环境中的泛化能力。

Method: 首先在有限标注数据上训练教师模型；利用基于Beta分布的置信度图评估时序观测中预测地图元素的可靠性；引入空间裁剪技术，仅保留高置信区域；将优化后的地图元素作为先验，在第二轮中提升教师模型对无标签数据的预测，生成伪标签用于从头训练学生模型，并在原始标注数据上微调。

Result: 在nuScenes数据集上的实验表明，在低标注条件下，该方法相比仅使用标注数据训练的模型，mAP提升了+6.1。

Conclusion: 该教师-学生框架结合优化后的伪标签，有效缓解了在线高精地图构建中标注数据稀缺的问题，具有实际应用价值。
...
Abstract (中文翻译): 在现实场景中部署在线高精地图构建系统面临的一个关键挑战是标注训练数据的稀缺，这限制了模型在多样化环境中的泛化能力。为解决这一问题，我们提出了一种教师-学生半监督学习（SSL）框架，通过置信度感知的地图优化从无标签数据中生成高质量伪标签。我们的方法首先在有限的标注数据上训练一个教师模型，然后利用基于Beta分布的置信度图来评估时序观测中预测地图元素的可靠性。与传统方法直接丢弃整个地图元素不同，我们引入了一种空间裁剪技术，选择性地保留高置信区域并移除不可靠部分。这些优化后的地图元素作为地图先验，在第二轮推理中提升教师模型对无标签数据的预测准确性。这些增强后的预测结果被用作伪标签，从头训练一个学生模型，随后在原始标注数据上进行微调。在nuScenes数据集上的实验结果表明，与仅使用标注数据训练相比，我们在低标注条件下将性能提升了+6.1 mAP，为在线高精地图构建中的标注数据稀缺问题提供了一种实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有截图到代码系统在灵活性和可控性之间存在权衡：直接多模态生成易产生幻觉，而结构化流水线虽减少错误但限制输出范围。作者希望探索选择性工具接地是否能改善这一权衡。

Method: 提出WidgetGen，一个轻量级工具接地框架，提取可观察的文本与颜色证据，进行高层布局（及可选图表）推理，并直接生成可执行JSX代码，避免依赖组件级生成和固定UI模式。

Result: 在6个模型和1000个保留widget上，WidgetGen在多数视觉重建指标（如面积、可读性、样式）上优于直接提示和结构化Widget2Code流水线；利用其生成的图像-代码对微调Qwen系列模型，在所有指标上均有提升。
...
Abstract (中文翻译): 现有的截图到代码系统在灵活性与可控性之间面临权衡：直接的多模态生成可能会臆造可见细节，而结构化流水线则通过组件分解、预定义模板和定制中间表示来减少此类错误。然而，这些结构引入了额外的生成协调开销，并将输出限制在中间表示所覆盖的设计范围内。我们研究了选择性工具接地是否能够改善直接从控件到代码生成中的保真度—效率权衡。我们提出了WidgetGen——一种轻量级的工具接地方案，它提取可观察的文本与颜色证据，执行高层布局推理（以及可选的图表推理），并直接生成可执行的JavaScript XML（JSX）。该设计减少了对组件级生成的依赖，同时避免了固定UI模式的限制。在六个多模态模型和1,000个保留控件上的实验表明，WidgetGen在大多数视觉重建指标上优于直接提示方法和结构化的Widget2Code流水线，在面积、可读性和样式方面均取得一致提升。此外，利用WidgetGen生成的图像-代码对通过监督微调显著提升了六个Qwen系列开源模型在所有报告指标上的表现。这些结果确立了WidgetGen作为强有力的轻量级基线，并表明选择性证据接地是一种有效替代繁重表示约束的方法。
...
TL;DR: EgoCITE is a new framework for long-horizon egocentric memory that improves QA accuracy and retrieval alignment by using context-rich indexing and time-aware evidence retrieval, outperforming baselines by 4.4–14.2% with 36× lower cost than long-context LLM agents.


<details>
  <summary>Details</summary>
Motivation: Existing egocentric memory systems suffer from two key limitations: (1) indices derived from context-poor captions are unreliable for agentic search, and (2) retrieval methods ignore the temporal intent of user questions.

Method: EgoCITE includes three components: EgoScheme generates atomic memory indices using local multimodal context; EgoIndex constructs multi-view, multi-granularity searchable indices from action, activity, utterance, and conversation representations; EgoRetrv performs semantic search enhanced with question-conditioned temporal relevance scoring and evidence curation.

Result: Evaluated on EgoLifeQA, EgoMem, and EgoR1-Bench, EgoCITE improves answer accuracy by 4.4–14.2% over existing agentic memory baselines and achieves 36× lower computational cost compared to long-context LLM agents.

Conclusion: EgoCITE effectively addresses key bottlenecks in egocentric memory by integrating contextual richness into indexing and temporal awareness into retrieval, enabling more accurate and efficient long-horizon egocentric QA.
...
Abstract (中文翻译): 长时程自我中心记忆将连续的第一人称视频和音频转化为可搜索的过往经历记录。我们指出现有系统存在两个瓶颈：基于缺乏上下文信息的字幕构建的索引在智能体搜索中不可靠，而检索过程忽略了问题的时间意图。为解决这两个问题，我们提出了 EgoCITE（自我中心上下文增强索引与时间感知证据检索），这是一个面向自我中心问答的长时程智能体记忆框架。EgoCITE 包含三个组件：EgoScheme 利用局部多模态上下文将碎片化的视频字幕和语音转录转化为自包含的原子记忆索引；EgoIndex 将互补的动作、活动、话语和对话表征组织成多粒度、可搜索的多视角记忆索引；EgoRetrv 结合语义搜索、问题条件化的时间相关性评分以及检索证据的筛选。我们在 EgoLifeQA、EgoMem 和 EgoR1-Bench 上评估了 EgoCITE 的回答准确率和目标事件检索对齐效果。结果表明，EgoCITE 相较于现有智能体记忆基线方法，准确率提升至少 4.4–14.2%，同时计算成本比长上下文大语言模型智能体低 36 倍。
...
TL;DR: 本文提出使用三个正交切片（轴向、冠状、矢状）作为种子进行标签传播，并通过无标签融合规则提升3D医学图像分割性能，显著优于仅使用单一轴向种子的方法。


<details>
  <summary>Details</summary>
Motivation: 密集体素级标注在3D医学图像分割中成本高昂。现有单切片传播方法（如Sli2Vol）仅沿轴向传播标签，忽略了冠状和矢状方向的信息，导致远离种子切片时误差累积，尤其影响表面距离指标。

Method: 研究不同训练与推理策略对切片传播模型的影响，包括单轴vs多轴无标签配准、单种子vs多种子传播及正交种子配置。采用三个正交种子切片分别传播标签，并通过简单无标签规则融合结果。

Result: 在多器官CT数据集上，相比单轴基线，正交种子策略使Dice系数提升21.9%，归一化表面Dice提升25.5%，平均Hausdorff距离降低53.5%。训练范式影响有限，推理时种子的正交几何结构是关键。

Conclusion: 在切片传播方法中，推理阶段采用正交种子配置比增加标注切片数量或进行三轴联合训练更有效，能显著提升分割精度，尤其改善表面一致性。
...
Abstract (中文翻译): 密集的体素级标注仍然是3D医学图像分割中的主要瓶颈。单切片传播方法（如Sli2Vol）通过无标签配准将一个标注好的种子切片在整个体积中传播，从而减轻标注负担。然而，仅沿轴向传播会随着与种子距离的增加而累积误差，尤其是在表面距离度量方面，因为它忽略了冠状面和矢状面的信息，未能充分利用CT/MRI体积中的三维信息。为了更好地利用体积几何结构，我们研究了关键的训练和推理选择如何影响切片传播模型，包括单轴与多轴无标签配准、单种子与多种子传播以及正交种子配置。我们不再仅从单一轴向种子传播，而是使用三个正交种子——一个轴向、一个冠状和一个矢状——并通过简单的无标签规则融合其传播的标签。我们的结果表明，训练范式的影响有限：一个仅在轴向训练的网络应用于非轴向种子时几乎能获得全部性能提升，而显式的三轴联合训练带来的增益很小。相反，性能主要由推理阶段的种子几何结构驱动，尤其是正交性而非标注切片的数量，因为预算匹配的三个轴向种子对照组未带来任何收益，甚至可能降低性能。在多器官CT队列上，使用轴向Sli2Vol主干结合正交种子策略，相比单轴基线，Dice系数提升了21.9%，归一化表面Dice提升了25.5%，平均Hausdorff距离降低了53.5%。
