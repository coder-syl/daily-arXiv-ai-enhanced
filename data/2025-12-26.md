...
TL;DR: 本文提出了 VL4Gaze，首个用于评估和提升视觉语言模型（VLMs）对人类凝视理解能力的大规模基准数据集，包含 489K 个自动生成的问答对，涵盖四种凝视相关任务。实验表明，现有 VLMs 在缺乏针对性监督的情况下难以有效理解凝视语义与空间定位，而基于 VL4Gaze 的训练可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（VLMs）虽在多种视觉任务中表现出强大的场景级推理能力，但尚未系统研究其对人类凝视（gaze）的理解能力。由于缺乏专门的评测基准，尚不清楚凝视理解能否通过通用视觉-语言预训练自然涌现。

Method: 作者构建了 VL4Gaze 基准，包含 124K 张图像上的 489K 个自动生成问答对，并将凝视理解建模为统一的视觉问答（VQA）问题，涵盖四个互补任务：凝视对象描述、凝视方向描述、凝视点定位和模糊问题识别。在上下文学习和微调设置下，对多个商用和开源 VLM 进行全面评估。

Result: 实验结果显示，即使大规模 VLM 在没有任务特定监督的情况下也难以可靠地推断凝视语义和空间位置；而使用 VL4Gaze 进行训练则能在所有任务上带来显著且一致的性能提升。

Conclusion: 凝视理解不能仅靠通用视觉语言预训练自动获得，需要针对性的多任务监督。VL4Gaze 数据集和代码将公开发布，以推动该方向的研究。
...
Abstract (中文翻译): 人类凝视为理解视觉场景中的注意力、意图和社会互动提供了关键线索，然而当前的视觉语言模型（VLMs）在凝视理解方面仍鲜有探索。尽管近期的 VLMs 在多种视觉任务中展现出强大的场景级推理能力，但尚无系统性评估或训练其凝视理解能力的基准，因此尚不清楚凝视理解是否能从通用视觉-语言预训练中自然涌现。为填补这一空白，我们提出了 VL4Gaze——首个旨在探究、评估并释放 VLMs 凝视理解潜力的大规模基准。VL4Gaze 包含 124K 张图像上自动生成的 489K 个问答对，并通过四项互补任务将凝视理解形式化为统一的视觉问答（VQA）问题：(1) 凝视对象描述，(2) 凝视方向描述，(3) 凝视点定位，以及 (4) 模糊问题识别。我们在上下文学习和微调两种设置下，对多个商用和开源 VLM 进行了全面评估。结果表明，即使大规模 VLM 在缺乏任务特定监督的情况下也难以可靠地推断凝视语义和空间定位；相比之下，在 VL4Gaze 上进行训练可在所有任务上带来显著且一致的性能提升，凸显了针对性多任务监督对于发展 VLM 凝视理解能力的重要性。我们将公开发布该数据集和代码，以支持该方向的进一步研究与开发。
...
TL;DR: 本文提出TrashDets——一种面向TinyML设备的垃圾检测模型族，通过硬件感知的迭代神经架构搜索，在TACO和TrashNet数据集上显著优于现有方法，在精度、参数量、能耗和延迟方面取得全面改进。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的边缘和物联网设备上部署高效的垃圾检测模型面临挑战，现有TinyML检测器在精度、能耗和延迟之间难以兼顾，亟需一种可扩展且硬件友好的神经架构搜索方法来优化模型性能。

Method: 提出一种迭代式硬件感知神经架构搜索框架：构建Once-for-All风格的ResDets超网，通过交替优化主干网络与颈部/头部结构的进化搜索策略，并引入种群传递机制和精度预测器以降低搜索成本并提升稳定性。

Result: 在五类TACO子集上，最强变体TrashDet-l达到19.5 mAP50，比先前模型提升最多3.6 mAP50且参数更少；模型族覆盖1.14–19.5 mAP50与1.2M–30.5M参数。在MAX78002微控制器上，TrashDet-ResNet和TrashDet-MBNet分别在能效与精度上超越基线，最高降低88%能耗、78%延迟和53%平均功耗。

Conclusion: 所提出的TrashDets系列模型为TinyML场景下的垃圾检测提供了高精度、低功耗、可扩展的解决方案，显著优于现有方法，适用于多样化的资源受限硬件部署需求。
...
Abstract (中文翻译): 本文针对TACO数据集上的垃圾检测任务，在严格的TinyML约束下，提出了一种面向边缘和物联网设备的迭代式硬件感知神经架构搜索框架。该方法构建了一个Once-for-All风格的ResDets超网，并采用交替优化主干网络与颈部/头部结构的迭代进化搜索策略，辅以种群传递机制和精度预测器，以降低搜索成本并提升稳定性。该框架生成了一系列可直接部署的检测器，称为TrashDets。在包含纸张、塑料、瓶子、易拉罐和烟头五个类别的TACO子集上，性能最强的变体TrashDet-l达到了19.5 mAP50，参数量为30.5M，相比以往检测器精度最高提升3.6 mAP50，同时使用更少的参数。TrashDet模型族的参数量范围为1.2M至30.5M，mAP50介于11.4至19.5之间，为不同TinyML部署预算提供了可扩展的选择。在MAX78002微控制器上使用TrashNet数据集时，两种专用变体TrashDet-ResNet和TrashDet-MBNet共同优于ai87-fpndetector基线：TrashDet-ResNet单次推理能耗为7525微焦，延迟26.7毫秒，帧率达37.45 FPS；TrashDet-MBNet则将mAP50提升了10.2%。两者相较现有TinyML检测器，最高可减少88%的能耗、78%的延迟和53%的平均功耗。
...
<details>
  <summary>Details</summary>
Motivation: 现有语义场景补全（SSC）研究主要集中在地面场景（如自动驾驶），而空中场景（如无人机飞行）尚未被充分探索；同时，主流SSC依赖LiDAR传感器，但无人机受法规、重量、能耗及高空LiDAR点云稀疏性限制，难以部署LiDAR，亟需基于相机的替代方案。

Method: 提出OccuFly数据集：在30–50米高度采集涵盖城市、工业和乡村场景的四季图像，包含22个语义类别；开发无LiDAR的数据生成框架，通过传统3D重建将部分2D掩码标注自动提升到重建点云中，实现高效3D标注。

Result: 构建了首个真实世界、基于相机的空中SSC基准OccuFly；实现了自动化3D标注流程，大幅降低人工成本；在该基准上评估了现有SOTA方法，揭示了高空视角下SSC的独特挑战。

Conclusion: OccuFly填补了空中语义场景补全研究的空白，其基于相机的数据生成框架为资源受限的无人机平台提供了可行方案，推动了高空3D场景理解的发展。
...
Abstract (中文翻译): 语义场景补全（SSC）对于移动机器人中的三维感知至关重要，因为它能够通过联合估计密集体素占用率和每个体素的语义信息，实现对场景的整体理解。尽管SSC在自动驾驶等地面领域已得到广泛研究，但在自主飞行等空中场景中仍鲜有探索，从而限制了下游应用的发展。此外，LiDAR传感器是SSC数据生成的主要模态，但由于飞行法规、质量和能耗限制，以及从高视角获取的LiDAR点云稀疏性，大多数无人飞行器（UAV）难以部署LiDAR。为解决这些问题，我们提出了OccuFly——首个基于相机的真实世界空中SSC基准数据集，该数据集在春季、夏季、秋季和冬季分别于50米、40米和30米高度采集，涵盖城市、工业和乡村场景，提供22个语义类别，且数据格式遵循既有规范，便于与现有研究无缝集成。尤为重要的是，我们提出了一种无需LiDAR、仅基于相机模态的数据生成框架，该模态在现代UAV上普遍存在。通过利用传统三维重建技术，我们的框架将部分标注的二维掩码自动提升至重建的点云中，从而显著减少人工三维标注的工作量。最后，我们在OccuFly上对当前最先进的方法进行了基准测试，揭示了高视角下特有的挑战，为整体空中三维场景理解提供了一个全面的视觉基准。
...
TL;DR: NullBUS 是一种新型多模态混合监督框架，通过引入可为空的提示（nullable prompts）机制，在乳腺超声图像分割任务中有效利用有/无文本提示的数据，显著提升了在提示信息缺失情况下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有可提示分割方法依赖于文本或空间提示，但许多公开乳腺超声（BUS）数据集缺乏可靠的元数据或报告，导致只能在小规模多模态子集上训练，限制了模型的鲁棒性。

Method: 提出 NullBUS 框架，采用可为空提示（nullable prompts），即通过可学习的空嵌入和存在掩码，在元数据缺失时回退到仅使用图像信息，而在元数据存在时利用文本提示，从而在一个统一模型中同时学习带提示和不带提示的图像。

Result: 在三个公开 BUS 数据集组成的统一测试池上，NullBUS 达到平均 IoU 为 0.8568、平均 Dice 为 0.9103，展现了在混合提示可用条件下的最先进性能。

Conclusion: NullBUS 有效解决了乳腺超声图像分割中提示信息缺失的问题，提高了模型在实际临床场景中的适用性和鲁棒性。
...
Abstract (中文翻译): 乳腺超声（BUS）分割为计算机辅助诊断和治疗规划提供了关键的病灶边界信息。尽管可提示方法在存在文本或空间提示时能够提升分割性能和肿瘤轮廓描绘效果，但许多公开的 BUS 数据集缺乏可靠的元数据或报告，这限制了训练只能在小规模多模态子集上进行，从而降低了模型的鲁棒性。我们提出了 NullBUS，这是一种多模态混合监督框架，能够在单一模型中同时从带提示和不带提示的图像中学习。为应对文本缺失问题，我们引入了“可为空提示”机制，通过可学习的空嵌入和存在掩码实现：当元数据缺失时，模型可回退至仅依赖图像证据；当元数据存在时，则利用文本信息。在三个公开 BUS 数据集组成的统一数据池上的评估表明，NullBUS 实现了平均 IoU 为 0.8568、平均 Dice 为 0.9103 的性能，展示了在混合提示可用条件下最先进的分割效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统自动驾驶系统将相机设计与下游感知任务解耦，采用固定光学系统和人工设计的图像信号处理（ISP）流程，优先生成适合人类观看的图像而非机器可理解的语义信息。这种分离在去马赛克、去噪或量化过程中会丢失关键信息，并迫使感知模型适应传感器引入的伪影。

Method: 提出一个端到端的RAW-to-task协同设计框架，整合了基于DeepLens的现实手机级镜头模型、可学习彩色滤光阵列（CFA）、泊松-高斯噪声过程和量化操作，并直接针对语义分割目标进行联合优化。

Result: 在KITTI-360数据集上的评估表明，该方法相比固定流水线在mIoU上持续提升，其中光学建模和CFA学习带来最大增益，尤其对细长或低光照敏感类别效果显著。模型仅含约100万参数，运行速度达28 FPS，具备边缘部署能力。可视化和定量分析显示，协同设计的传感器能根据语义结构自适应调整图像采集，在模糊、噪声和低位深条件下仍能锐化边界并保持精度。

Conclusion: 全栈协同优化（包括光学、传感器和网络）是实现高效、可靠且可部署的自动驾驶感知系统的有效途径。
...
Abstract (中文翻译): 传统的自动驾驶系统将相机设计与下游感知任务解耦，依赖固定的光学系统和手工设计的图像信号处理器（ISP），其优先考虑的是人类可观看的图像，而非机器所需的语义信息。这种分离方式在去马赛克、去噪或量化过程中会丢弃信息，并迫使模型去适应传感器引入的伪影。我们提出了一种任务驱动的协同设计框架，将光学、传感器建模和轻量级语义分割网络统一为单一的端到端RAW到任务（RAW-to-task）流水线。该系统基于DeepLens[19]，整合了逼真的手机级镜头模型、可学习的彩色滤光阵列（CFA）、泊松-高斯噪声过程以及量化操作，并直接针对分割目标进行优化。在KITTI-360上的评估表明，该方法相比固定流水线在mIoU上持续取得提升，其中光学建模和CFA学习带来了最大的性能增益，尤其对于细长或对低光照敏感的类别效果更为显著。重要的是，这些鲁棒性提升是在一个紧凑的约100万参数模型上实现的，运行速度约为28 FPS，证明了其在边缘设备上的可部署性。视觉和定量分析进一步突显了协同设计的传感器如何根据语义结构自适应地调整图像采集过程，在模糊、噪声和低位深条件下仍能锐化边界并保持准确性。这些发现共同确立了光学、传感器和网络的全栈协同优化，是迈向高效、可靠且可部署的自动驾驶感知系统的有效路径。
...
TL;DR: 本文提出了CHAMMI-75，一个包含75项不同生物研究的多通道显微图像开放数据集，用于训练能适应不同成像通道和条件的细胞形态学模型，从而提升跨研究的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有细胞形态学量化模型通常针对单一显微成像类型训练，难以在不同技术规格（如通道数不同）或分布外实验条件下复用，限制了其在多样化生物研究中的应用。

Method: 构建并利用名为CHAMMI-75的开放数据集，该数据集整合了来自75项不同生物研究的异构多通道显微图像，用于训练具有通道自适应能力的细胞形态学模型。

Result: 实验表明，使用CHAMMI-75训练的模型在多通道生物成像任务中性能更优，主要得益于该数据集中显微成像模态的高度多样性。

Conclusion: CHAMMI-75为开发下一代适用于各类生物研究的通用细胞形态学模型奠定了基础。
...
Abstract (中文翻译): 利用图像和机器学习对细胞形态进行量化已被证明是研究细胞对处理响应的强大工具。然而，用于量化细胞形态的模型通常使用单一类型的显微成像进行训练，导致这些模型因技术规格不匹配（例如通道数量不同）或目标实验条件超出分布范围而无法在不同生物研究间复用。在此，我们提出了CHAMMI-75——一个开放获取的数据集，包含来自75项多样化生物研究的异构多通道显微图像。我们从公开资源中整理了这一数据集，旨在研究具有通道自适应能力、可处理任意显微图像类型的细胞形态学模型。我们的实验表明，使用CHAMMI-75进行训练能够提升多通道生物成像任务的性能，这主要归功于其在显微成像模态方面的高度多样性。本研究为创建适用于生物研究的新一代细胞形态学模型铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）在多模态推理任务中表现优异，但部署时面临高推理延迟和计算成本问题，尤其在处理高分辨率图像时。现有方法采用静态视觉预处理，对视觉内容简单的图像仍进行冗余计算。

Method: 提出一种自适应视觉预处理方法，结合内容感知图像分析、自适应分辨率选择和内容感知裁剪，在视觉编码前减少视觉冗余，并与FastVLM无缝集成，无需修改其架构或重新训练。

Result: 在DocVQA数据集子集上的实验表明，该方法将每张图像的推理时间减少超过50%，平均生成时间降低，视觉token数量减少超过55%。

Conclusion: 内容感知的自适应预处理是一种轻量且有效的策略，可显著提升视觉语言模型在部署场景下的效率。
...
Abstract (中文翻译): 视觉语言模型（VLMs）在多模态推理任务中表现出色，但其部署仍面临高推理延迟和计算成本的挑战，尤其是在处理高分辨率视觉输入时。尽管近期如FastVLM等架构通过优化视觉编码器提升了效率，但现有流程仍依赖静态视觉预处理，导致对视觉内容简单的输入产生冗余计算。本文提出一种自适应视觉预处理方法，根据图像内容特征动态调整输入分辨率和空间覆盖范围。该方法结合内容感知图像分析、自适应分辨率选择和内容感知裁剪，在视觉编码前减少视觉冗余。重要的是，该方法无需修改FastVLM架构或重新训练即可集成。我们在DocVQA数据集的一个子集上以仅推理方式评估了所提方法，重点关注面向效率的指标。实验结果表明，自适应预处理相比基线流程，将每张图像的推理时间减少超过50%，降低了平均完整生成时间，并使视觉token数量持续减少55%以上。这些结果表明，基于输入内容的预处理是一种有效且轻量的策略，可提升视觉语言模型在部署中的效率。为便于复现，我们的实现作为FastVLM仓库的一个分支提供，包含所提方法的相关文件，地址为：https://github.com/kmdavidds/mlfastlm。
...
Result: 在完整医学影像课程上验证了 ALIVE 的检索准确率、延迟表现与用户体验，结果表明其能提供准确、内容相关且具吸引力的实时学习支持。

Conclusion: ALIVE 展示了多模态 AI 与内容感知检索、本地部署相结合可显著提升录播课程的教学价值，为下一代交互式学习环境提供了可扩展路径。
...
Abstract (中文翻译): 传统的讲座视频虽然灵活，但缺乏实时澄清机制，导致学习者在遇到困惑时不得不向外搜索答案。尽管大型语言模型和神经化身的最新进展为交互式学习带来了新机遇，但现有系统通常缺乏对讲座内容的理解，依赖云服务，或未能在统一且保护隐私的流程中整合检索与化身讲解功能。  
我们提出了 ALIVE（Avatar-Lecture Interactive Video Engine），它将被动的讲座观看转变为动态、实时的学习体验。ALIVE 完全在本地硬件上运行，整合了三项核心技术：(1) 通过 ASR 转录、大语言模型优化和神经说话头合成生成的化身讲解；(2) 结合语义相似性与时间戳对齐的内容感知检索机制，用于定位上下文相关的讲座片段；(3) 实时多模态交互，允许学生暂停讲座并通过文本或语音提问，并接收以文本或化身形式呈现的、基于讲座内容的解释。  
为保持响应速度，ALIVE 采用了轻量级嵌入模型、基于 FAISS 的检索机制，以及带有渐进预加载的分段化身合成策略。我们在一门完整的医学影像课程上展示了该系统，并评估了其检索准确率、延迟特性和用户体验，结果表明 ALIVE 能够提供准确、内容相关且引人入胜的实时学习支持。  
ALIVE 表明，当多模态 AI 与内容感知检索和本地部署相结合时，可以显著提升录播讲座的教学价值，为构建下一代交互式学习环境提供了可扩展的路径。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D探地雷达在地下管线检测中多视图特征关联弱、小尺度目标识别精度低以及复杂场景下鲁棒性不足的问题。

Method: 1）建立基于B/C/D-Scan三视图联合分析的三维管线特征评估方法；2）提出DCO-YOLO框架，融合DySample、CGLU和OutlookAttention机制增强小目标边缘特征提取；3）设计3D-DIoU空间特征匹配算法，结合三维几何约束与中心距离惩罚项，实现多视图标注自动关联。

Result: 在真实城市地下管线数据上的实验表明，所提方法在复杂多管线场景中达到96.2%准确率、93.3%召回率和96.7%mAP，分别比基线模型高2.0%、2.1%和0.9%；消融实验和Grad-CAM++热力图验证了模型对管线几何特征的关注能力提升。

Conclusion: 该研究将深度学习优化策略与3D GPR物理特性相结合，为地下管线的智能识别与定位提供了一种高效可靠的新型技术框架。
...
TL;DR: NeRV360 是一种面向 360 度视频的高效神经视频表示方法，通过仅解码用户视口区域，显著降低内存消耗并提升解码速度。


<details>
  <summary>Details</summary>
Motivation: 传统 NeRV 方法在高分辨率 360 度视频上存在内存占用高、解码速度慢的问题，难以支持实时应用。

Method: 提出 NeRV360 框架，将视口提取集成到解码过程中，并引入时空仿射变换模块，实现基于视角和时间的条件解码。

Result: 在 6K 分辨率视频上，相比 HNeRV，NeRV360 内存消耗减少 7 倍，解码速度提升 2.5 倍，且客观画质指标更优。

Conclusion: NeRV360 有效解决了高分辨率 360 度视频神经表示中的效率瓶颈，为实时应用提供了可行方案。
...
Abstract (中文翻译): 面向视频的隐式神经表示（NeRV）在视频压缩方面展现出强大潜力。然而，将 NeRV 应用于高分辨率 360 度视频会导致内存占用过高和解码速度缓慢，使得实时应用变得不切实际。我们提出了 NeRV360，这是一种端到端框架，仅解码用户选定的视口区域，而非重建整个全景帧。与传统流程不同，NeRV360 将视口提取集成到解码过程中，并引入了一个时空仿射变换模块，以根据视角和时间进行条件解码。在 6K 分辨率视频上的实验表明，与代表性先前工作 HNeRV 相比，NeRV360 的内存消耗减少了 7 倍，解码速度提升了 2.5 倍，同时在客观指标上实现了更优的图像质量。
