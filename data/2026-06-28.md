...
TL;DR: 本文提出DocArena，一个全自动的多模态文档数据构建流水线，用于训练无需人工标注的搜索智能体；该方法通过MLLM进行文档结构化、跨页信息建模和质量控制，生成高质量QA对，并设计了解耦视觉感知与策略模型的Doc-Search架构，在多个多模态和文本问答基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于（问题，答案，证据）元组训练搜索智能体的方法主要局限于纯文本环境，难以构建可控、可扩展且支持多模态数据的训练环境。为解决这一问题，作者提出DocArena，以满足实际应用中对多模态文档搜索与问答的需求。

Method: DocArena流水线首先利用多模态大语言模型（MLLM）对原始文档进行视觉感知和结构化索引，然后分析跨页信息分布以构建需要推理能力的问答对，并通过MLLM执行级联的质量保障操作。此外，作者设计了Doc-Search智能体架构，将视觉感知模块与策略模型解耦，使纯文本LLM能作为多模态检索与问答的推理核心。

Result: 在包含8,336份文档、覆盖16个领域和49种语言的DocArena-79K数据集上训练的智能体，在六个多模态文档场景和七个文本问答基准测试中，在检索准确率和问答质量方面均优于现有方法。

Conclusion: DocArena提供了一种高效、自动化的多模态训练环境构建方案，实验证明其生成的数据能有效提升搜索智能体的性能与可控性，为多模态文档理解与检索任务提供了新范式。
...
Abstract (中文翻译): 近期的方法通过（问题，答案，证据）三元组以强化学习方式训练搜索智能体，无需专家轨迹。这些三元组构成训练环境，其特性直接影响智能体所能发展的搜索策略与泛化能力。尽管先前工作在提升训练数据质量方面取得了可喜进展，但现有环境仍主要基于文本，且难以构建可控、可扩展并兼顾多模态数据的训练环境。为此，我们提出了DocArena——一个完全自动化的数据构建流水线，基于多模态文档搜索与问答的实际需求，无需任何人工标注即可将原始文档集合转化为搜索智能体的训练环境。该流水线首先通过基于多模态大语言模型（MLLM）的视觉感知对文档进行结构化与索引，随后分析并利用跨页信息分布来构建需要复杂推理的问答对，并通过MLLM执行级联的质量保障操作。我们发布了DocArena-79K数据集，包含来自8,336份文档的问答对，涵盖16个领域和49种语言。此外，我们设计了Doc-Search智能体架构，将视觉感知与策略模型解耦，使基于文本的大语言模型（LLM）能够作为多模态文档检索与问答的推理主干。在统一评估框架下（仅策略模型不同），在六个多模态文档场景和七个文本问答基准上的实验表明，基于DocArena数据训练的智能体在检索准确率和问答质量方面均达到最佳性能。对智能体搜索行为的进一步分析也验证了所构建训练环境的有效性与可控性。
...
TL;DR: 本文提出一种结合图像处理与深度学习的混合方法，用于高效、低成本地评估水果新鲜度，在苹果和橙子数据集上实现超过90%的准确率，并支持实时应用。


<details>
  <summary>Details</summary>
Motivation: 水果腐烂造成严重的农业经济损失，现有方法可能计算成本高或难以部署。因此，作者旨在开发一种低资源消耗、可实时运行且准确的新鲜度评估方案。

Method: 首先设计一个图像处理算法，输出0–100的新鲜度评分；同时训练一个CNN进行二分类（新鲜/腐烂）。将两者结果通过逻辑回归融合以提升预测精度，并进一步利用该逻辑回归模型使图像处理算法能直接输出二分类结果，从而在实际部署中无需使用CNN。

Result: 该方法在包含苹果和橙子的数据集上达到超过90%的准确率，且可在低计算资源下实现实时运行。

Conclusion: 结合简单图像处理与机器学习能为农业领域提供实用、高效的新鲜度检测解决方案；未来可通过引入先进分割模型以支持复杂背景下的水果识别。
...
Abstract (中文翻译): 水果腐烂是农业中的一个重大问题，会造成巨大的经济损失。针对这一问题，本研究提出了一种结合图像处理与深度学习的混合方法来评估水果的新鲜度。我们开发了一种图像处理算法，能够将腐烂程度量化为0（完全新鲜）到100（完全腐烂）之间的数值。同时，我们利用大量水果图像数据集训练了一个卷积神经网络（CNN），用于执行二分类任务（新鲜或腐烂）。随后，通过逻辑回归将两种方法的结果进行融合，以提高新鲜度预测的准确性。接着，该逻辑回归模型被用于使图像处理算法能够基于其百分比输出直接进行二分类，从而在实时应用中无需依赖CNN。该方法无需高计算资源，实现了实时性能，并在包含苹果和橙子的数据集上验证了超过90%的准确率。主要局限在于要求水果必须置于白色或透明背景上，未来改进方向可包括引入先进的分割模型以自动去除背景。本研究结果表明，将简单的图像处理技术与机器学习相结合，有望为农业领域提供切实可行的解决方案。
...
TL;DR: 本文提出长上下文生成（LCG）框架，通过稀疏关系注意力（SRA）和路由一致性约束（RCC）提升多图像生成中跨帧的一致性，并构建了包含60万训练序列的LCCD数据集进行验证。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型在单图合成上表现优异，但在漫画、故事板等需多图一致性的任务中难以维持角色与语义的一致性，尤其在长序列和多角色场景下问题更严重。

Method: 提出LCG框架，包含稀疏关系注意力机制（SRA）用于高效建模长视觉上下文中的关键特征，以及路由一致性约束（RCC）利用身份感知掩码对齐不同生成分支的结构模式；同时构建LCCD数据集支持训练与评估。

Result: 实验表明LCG在长上下文图像生成任务中，在提示对齐和角色一致性方面优于现有基线方法，尤其在复杂多角色场景中表现突出。

Conclusion: LCG有效解决了长上下文多图像生成中的一致性问题，为视觉叙事类应用提供了可扩展且高效的解决方案。
...
TL;DR: 本文提出一种基于深度学习与遥感数据的冠层级城市地上生物量（AGB）估算框架，利用LiDAR和高分辨率影像，在无需人工标注的情况下实现了对加拿大安大略省810 km²区域内单株树木的生物量估算，并构建了公开的双时相AGB数据库。


<details>
  <summary>Details</summary>
Motivation: 城市树木生物量的空间显式量化不足，现有方法多依赖于无法解析单个树冠或细尺度异质性的清查数据或粗分辨率产品，限制了城市森林碳储量的精细评估与管理。

Method: 结合2018年和2023年的落叶期机载LiDAR（8–10 pulses/m²）与近红外RGB正射影像（0.16–0.20 m），使用基于规则伪标签训练的双流交叉注意力网络进行语义标记（建筑、针叶树、阔叶树），支持树冠分割与功能类型分配；通过多尺度分水岭算法在识别出的树木区域中提取树冠，并基于冠幅-高度幂律关系（校准自21,921棵清查树木的物种特异性异速生长方程）估算AGB；采用深度集成方法生成不确定性地图以指导不确定树冠的处理。

Result: 在独立验证集上，语义分割的全局/平均精确率、召回率和Dice分数分别为0.86、0.83和0.84；在90,726棵树的测试集中，18,713对清查-分割匹配样本的AGB预测R²为0.609（使用清查冠层几何）和0.570（使用实际分割结果）；聚合至30 m分辨率后，2018年和2023年总AGB分别为1.73 Tg和1.81 Tg（相当于811–850 Gg C），尼亚加拉悬崖沿线局部密度高达约140 Mg/ha，五年净碳增益为39 Gg C。

Conclusion: 该框架仅使用省级标准遥感数据、无需人工标注，即可在管理相关分辨率下生成城市非林地树木的双时相冠层级AGB数据库，为城市碳汇监测与生态管理提供高精度工具，并指出树冠分割仍是主要不确定性来源。
...
Abstract (中文翻译): 由于许多估算依赖于无法解析单个树冠或细尺度异质性的清查数据或粗分辨率产品，城市树木生物量的空间显式量化程度仍低于人工林。我们提出一个冠层级地上生物量（AGB）估算框架，应用于加拿大安大略省810 km²的景观区域，使用2018年和2023年的落叶期机载LiDAR（8–10脉冲/m²）和近红外RGB正射影像（0.16–0.20 m）。一个基于规则伪标签训练的双流交叉注意力网络生成了建筑、针叶树和阔叶树的语义标记，用于支持树冠分割和功能类型分配。在独立标注的保留瓦片上，全局/平均精确率、召回率和Dice分数分别为0.86、0.83和0.84。在已识别的树木区域内，采用多尺度分水岭分割法提取树冠，并基于冠幅-高度幂律代理模型估算AGB，该模型根据Lambert等（2005）针对21,921棵清查树木建立的物种特异性异速生长方程进行校准。在包含90,726棵树的保留测试集中，18,713对清查-分割匹配样本的AGB预测在使用清查冠层几何时R²为0.609，在实际操作分割下R²为0.570，表明树冠分割仍是主要不确定性来源。将结果聚合至30 m分辨率后，估算得出2018年和2023年总AGB分别为1.73 Tg和1.81 Tg（相当于811–850 Gg碳），尼亚加拉悬崖沿线局部密度高达约140 Mg/ha，五年间净碳增益为39 Gg碳。深度集成不确定性地图突出了与代表性不足的土地覆盖类型相关的高认知不确定性区域，并指导将不确定树冠分配至统一的异速生长方程。该框架仅使用省级标准数据，无需人工标注，即可在管理相关分辨率下生成首个公开的双时相城市非林地树木冠层级AGB数据库。
...
TL;DR: 本文提出一种多任务深度学习模型，利用焊接过程中的熔池图像和工艺参数，高精度预测激光深熔焊的熔透状态、熔深及焊缝形貌。


<details>
  <summary>Details</summary>
Motivation: 在激光深熔焊中，准确评估熔透状态和焊缝形貌对保证焊接质量至关重要，但现有方法难以同时高效、精确地实现多指标预测。
...
Conclusion: 该研究为激光深熔焊系统的在线质量控制提供了新思路与有效方法。
...
Abstract (中文翻译): 在激光深熔焊中，熔透状态与焊缝形貌的评估对确定焊接质量起着关键作用。本文全面介绍了一种创新的多任务深度学习模型，该模型能够高精度地预测熔透状态、熔深以及焊缝形貌。监测平台依赖于在激光焊接过程中使用互补金属氧化物半导体（CMOS）相机捕获的熔池图像。所提出的模型整合了从熔池顶部图像中提取的时空特征以及焊接工艺参数，构建了一个基于卷积神经网络和状态空间模型的深度学习框架，以更高效地提取和处理时空信息。此外，本文还提出了一种可靠的数据集构建方法，以增强所开发模型的鲁棒性和泛化能力。在测试集上的验证结果表明，熔透状态的预测准确率可达99.35%，熔深预测误差为1.79毫米，焊缝横截面重建的准确率为95.65%。本研究为激光深熔焊系统中的在线质量控制策略提供了新的见解与方法。
...
TL;DR: 该研究将基于fMRI重建的认知任务分类学从单源迁移扩展到多源迁移，分析了23种任务状态间的迁移关系，并利用布尔整数规划在监督预算约束下优化任务分配，发现工作记忆任务具有高优先级，而运动任务簇表现出范式内强迁移但跨范式迁移受限。


<details>
  <summary>Details</summary>
Motivation: 现有基于fMRI重建的任务分类学主要关注单源到单目标的一对一迁移，难以全面刻画多源协同对目标任务的支持关系，也缺乏对监督资源有限情况下的任务分配策略的系统研究。

Method: 在23个HCP任务状态下训练1,127个任务特定和迁移模型，采用布尔整数规划（BIP）分析不同监督预算下的最优任务分配，并比较单源与多源迁移的表现差异。

Result: 单源迁移呈现方向性和范式结构：运动任务内部迁移效果好，但对非运动任务支持有限；多源迁移效果依赖于源任务组合，表明成对任务关系不足以完全描述多对一迁移；BIP在多种预算下均优先分配监督资源给0-back和2-back工作记忆任务。

Conclusion: 研究揭示了跨范式迁移受限的运动任务簇以及在全局分配目标下具有高优先级的工作记忆状态，将fMRI任务分类学从一对一迁移拓展至多对一迁移和预算约束下的任务依赖分析。
...
Abstract (中文翻译): 认知任务由共享和特化的神经过程组织而成。掩码fMRI重建为量化任务状态之间的迁移关系提供了一个通用的自监督目标，但现有的基于重建的任务分类学主要研究从单一源任务到目标任务的一对一迁移。本文将fMRI认知任务分类学从单源迁移扩展到多源迁移，涵盖人类连接组计划（Human Connectome Project）中的23种任务状态，并使用布尔整数规划（BIP）分析监督预算约束下的任务分配策略。我们训练了1,127个任务特定模型和迁移模型。结果表明，单源迁移具有方向性且受任务范式结构影响：运动状态在运动范式内部迁移效果良好，但对大多数非运动目标的支持有限，这与共享的感觉运动执行系统和效应器特异性表征一致。多源迁移的效果则取决于源任务集合的组成，表明多对一的任务关系无法仅通过成对任务分类学完全捕捉。在不同监督预算下，BIP反复将直接监督资源分配给多个0-back和2-back工作记忆状态，尽管这些状态并非始终是最强的个体源任务。这一模式可能反映了工作记忆任务中感知、注意和执行过程的整合。综上所述，这些发现揭示了一个跨范式迁移受限的运动任务簇，以及在指定全局分配目标下具有高优先级的工作记忆状态。本研究将基于重建的fMRI任务分类学从一对一迁移扩展到了多对一任务关系及预算约束下的任务依赖分析。
...
TL;DR: 本文提出GeMoE，一种基于门控熵的不确定性感知自适应路由方法，将MoE中的token路由视为信息编码任务，通过最小描述长度（MDL）原则动态决定每个token激活的专家数量，在保持99.5%性能的同时提升专家激活稀疏性36.5%。


<details>
  <summary>Details</summary>
Motivation: 传统MoE架构采用静态Top-k路由策略，无法根据输入变化自适应调整激活专家数量，导致资源利用效率低下。因此，需要一种能动态平衡模型复杂度与性能的路由机制。

Method: 将token路由建模为信息编码问题，利用最小描述长度（MDL）原理，通过门控熵衡量token复杂度，提出Gating Entropy-based Uncertainty-aware Adaptive Routing（GeMoE），动态决定每个token应激活的专家数量。

Result: 在多种骨干网络和基准测试中，GeMoE相比原始静态路由平均保留99.5%的性能，同时将专家激活稀疏性平均提升36.5%。

Conclusion: GeMoE通过显式建模模型复杂度与性能之间的权衡，实现了高效且自适应的专家路由策略，显著提升了MoE架构的资源利用效率。
...
Abstract (中文翻译): 随着模型参数和训练数据的增加，大视觉语言模型（LVLMs）的指令遵循能力和泛化能力显著提升。基于混合专家（MoE）架构的LVLMs在保持推理成本的同时扩展了参数容量。然而，传统的MoE方法采用Top-k静态路由策略，未能考虑输入的变化，也无法自适应地选择专家数量，导致资源利用效率低下。本文提出将token路由视为信息编码任务，将动态路由构建为编码中的最小描述长度（MDL）问题。通过验证MDL与MoE场景中门控熵之间的联系，我们提出了基于门控熵的不确定性感知自适应路由方法（GeMoE）。与传统的静态或基于启发式的动态路由方法不同，GeMoE显式建模了模型复杂度与性能之间的权衡，并利用门控熵评估token的复杂度，从而自适应地决定每个token应激活的专家数量。在多种骨干网络和基准测试上，该方法相比原始静态路由平均保留了99.5%的性能，同时将专家激活稀疏性平均提升了36.5%。
...
TL;DR: 本文提出了一个真实浑浊水下图像数据集TUB和一种新指标PCD，用于评估浑浊条件下结构信息损失，并验证其与实例分割模型性能高度相关。


<details>
  <summary>Details</summary>
Motivation: 当前水下计算机视觉研究多依赖合成浑浊数据集，可能无法准确反映真实浑浊环境中的信息损失，因此亟需真实数据和更有效的评估指标。

Method: 构建包含1,320张极端浑浊条件下拍摄图像及超过16,000个高置信度分割掩码的TUB数据集；提出基于相位一致性图、对对比度不变的PCD指标，用于衡量真实浑浊中的结构信息损失。

Result: 实验表明，PCD指标与实例分割模型在真实和合成浑浊图像上的性能高度相关，而现有常用指标几乎无相关性。

Conclusion: PCD能有效表征浑浊导致的结构信息退化，TUB数据集为水下视觉研究提供了更贴近现实的基准。
...
Abstract (中文翻译): 在浑浊条件下，水下环境的可见度迅速下降，但这种退化对计算机视觉模型的影响尚不明确。这一问题因当前研究依赖于合成浑浊数据集而更加复杂，这些合成数据可能无法准确反映真实世界中的信息损失。为填补这一空白，我们提出了浑浊水下基准（Turbid Underwater Baseline, TUB）数据集，包含在极端浑浊条件下采集的1,320张图像以及超过16,000个高置信度的真实分割掩码。我们还提出了一种名为PCD的新指标，该指标基于相位一致性图，对对比度变化具有不变性，旨在捕捉真实浑浊环境中结构信息的损失。我们证明，PCD与实例分割模型在真实和合成浑浊图像上的性能高度相关，而领域内常用指标则表现出微弱甚至无相关性。该数据集及相关代码可在项目页面获取：https://vap.aau.dk/pcd。
...
<details>
  <summary>Details</summary>
Motivation: 研究视觉提示调优中图像与提示标记的融合方式是否影响性能，并探索是否混合融合策略能带来更好效果。

Method: 将融合策略选择建模为双层优化问题，利用可微架构搜索联合优化可学习提示及其融合方式；在原有拼接和加法基础上引入仿射变换和交叉注意力两种新融合机制。

Result: 在VTAB-1k、FGVC和HTA共34个数据集上显著优于基线方法，在冻结ViT主干下实现了更优的准确率-延迟-参数权衡。

Conclusion: 提示与图像标记的融合方式对视觉提示调优至关重要，混合融合策略能更有效地利用ViT各层语义，为该领域提供新视角。
...
Abstract (中文翻译): 视觉提示调优已成为一种参数高效的微调方法，用于将大规模视觉Transformer（ViT）适配到下游任务。由于其可学习提示被应用于输入空间和特征空间，并在进入Transformer层的注意力机制前与图像标记融合，目前最常用的融合方式是拼接或相加。本文旨在研究视觉提示调优中一个基础但关键的问题：单一融合方式是否更优，以及开发混合融合策略是否有益。为此，我们将该任务形式化为一个双层优化问题，并借助可微架构搜索进行求解，从而联合优化可学习提示及其融合方式。为丰富架构搜索空间，我们在拼接和加法之外提出了两种新的融合机制：仿射变换和交叉注意力。在涵盖VTAB-1k、FGVC和HTA的34个数据集上的大量实验表明，本方法始终优于提示调优基线。在冻结ViT主干的情况下，相比VPT-Deep及其近期变体，本方法在准确率、延迟和参数数量之间取得了更优的平衡。我们的研究揭示了提示与图像标记的融合方式在视觉提示调优中具有重要作用，而混合融合策略能更有效地利用ViT各层的语义信息，为视觉提示调优研究提供了新视角。
...
TL;DR: 本文指出当前深度伪造检测基准可能主要衡量的是通用模态理解能力，而非真正的取证能力，并提出使用冻结的自监督表征上的线性探针作为诊断工具来评估基准的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着深度伪造生成器日益逼真，可靠的检测方法变得至关重要。然而，许多在标准基准上表现良好的检测器在现实场景中却频频失效。作者质疑这些基准是否真正反映了现实世界中的深度伪造威胁，并希望厘清这些基准究竟在衡量什么。

Method: 作者采用一种简单但有效的诊断方法：在冻结的、通用的自监督表征上训练线性探针，并将其性能与专门设计的深度伪造检测器进行比较。如果线性探针能接近专用检测器的性能，则说明该基准主要奖励的是通用模态理解能力，而非特定的取证知识。此外，作者还分析了生成器难度与表征空间中Frechet几何之间的关系。

Result: 在视频、图像和音频三种模态中，基于通用自监督表征的线性探针均能接近专用检测器的性能；同时，生成器级别的难度部分可通过同一表征空间中的Frechet几何解释。

Conclusion: 研究结果支持对深度伪造检测基准采取“审计”视角：在将高分解读为具备取证理解能力之前，应先评估该基准在多大程度上已被通用表征所解决。这提示当前基准可能未能准确反映真实威胁模型。
...
Abstract (中文翻译): 随着深度伪造生成器逐渐达到人类难以察觉的程度，可靠的检测方法变得至关重要。然而，在基准测试中表现优异的检测器在现实场景中却经常失效。一个令人担忧的反馈循环已经出现：基准推动了越来越复杂、精心设计的检测器的发展，但如果这些基准不能反映现实世界中的深度伪造情况，那么这种复杂性可能是在解决错误的问题。这就引出了一个更根本的问题：这些基准实际上在衡量什么？我们使用一种刻意简化的诊断方法对视频、图像和音频深度伪造基准进行了审计。如果在冻结的、通用的自监督表征上训练的线性探针能够逼近专用检测器的性能，那么该基准主要奖励的是通用模态理解能力，而非取证理解能力。这带来两个影响：该基准可能无法反映真实的威胁模型，并且引发对那些被线性探针逼近的专用检测器是否真正学习到了取证理解能力的质疑。我们在三种模态中均观察到，基于通用自监督表征的线性探针性能接近专用检测器。我们进一步表明，生成器级别的难度部分可以通过同一表征空间中的Frechet几何来解释。综合来看，这些结果支持对深度伪造检测采取基准审计的观点：在将高分视为具备取证理解能力的证据之前，有必要先评估该基准在多大程度上已经被通用表征所解决。
